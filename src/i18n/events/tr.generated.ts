import type { EventTranslationCatalog } from './types';

// Auto-generated from scripts/localization/event-source-catalog.json
export const trEventTranslationsGenerated: EventTranslationCatalog = {
  tr_ilk_adim: {
    text: "Ayaklarının üstünde duruyorsun. Bir adım... iki adım... YÜRÜYORSUN! Annen sevinçten ağlıyor!",
    choices: {
      adim_devam: {
        text: "Koşmaya çalış!",
        feedback: "Düştün ama hemen kalktın! Azimlisin küçük kahraman!",
      },
    },
  },
  tr_ilk_kelime: {
    text: "Ağzından bir ses çıkıyor... 'Anne!' mı 'Baba!' mı? Ev halkı nefesini tutmuş bekliyor!",
    choices: {
      kelime_anne: {
        text: "Anne!",
        feedback: "ANNE! Annen seni havaya kaldırdı, gözleri doldu!",
      },
      kelime_baba: {
        text: "Baba!",
        feedback: "BABA! Baban gururdan patladı, herkese anlattı!",
      },
    },
  },
  tr_kre_ilk_gun: {
    text: "Bugün kreşe ilk gidişin. Annen seni bırakıp gidecek. Yabancı yüzler, garip sesler...",
    choices: {
      kres_agla: {
        text: "Ağla ve anneye sarıl",
        feedback: "Biraz ağladın ama öğretmen teselli etti. Yavaş yavaş alışacaksın.",
      },
      kres_oyun: {
        text: "Oyuncaklara koş",
        feedback: "Oyuncaklar dikkatini çekti! Annen gittiğini fark etmedin bile.",
      },
    },
  },
  tr_oyuncak_paylasim: {
    text: "Parkta bir çocuk senin oyuncağını istiyor. Vermek istemiyorsun ama annen bakıyor...",
    choices: {
      oyuncak_paylas: {
        text: "Paylaş",
        feedback: "Zor oldu ama verdin. Çocuk mutlu oldu, sen de biraz...",
      },
      oyuncak_tutma: {
        text: "HAYIR! BENİM!",
        feedback: "Çocuk ağladı, annen utandı. Ama oyuncak senin!",
      },
    },
  },
  tr_gece_korkusu: {
    text: "Gece karanlık çok korkunç. Yatağın altında bir şey var gibi...",
    choices: {
      korku_anne: {
        text: "Anneee! Babaaaa!",
        feedback: "Anne geldi, sarıldı, şarkı söyledi. Rahatladın, uyudun.",
      },
      korku_cesur: {
        text: "Yatağın altına bak",
        feedback: "Baktın... toz ve eski oyuncaklar! Korkulacak bir şey yok. Cesur oldun!",
      },
    },
  },
  tr_sebze_yemek: {
    text: "Annen tabağına brokoli koydu. Yeşil, garip görünüyor. Yemelisin ama...",
    choices: {
      sebze_ye: {
        text: "Ye (yüzünü buruşturarak)",
        feedback: "İğrenç değildi aslında... Annen çok mutlu oldu!",
      },
      sebze_tukur: {
        text: "TÜKÜÜÜR!",
        feedback: "Brokoli duvarda! Annen çıldırdı. Ama en azından yemedin.",
      },
      sebze_gizle: {
        text: "Peçetenin altına sakla",
        feedback: "Kimse görmedi... sanıyorsun. Zeki misin yoksa kurnaz mı?",
      },
    },
  },
  tr_tuvalet_egitimi: {
    text: "Artık bez yok! Tuvaleti kullanma zamanı. Lazımlık hazır...",
    choices: {
      tuvalet_basari: {
        text: "Lazımlığı kullan",
        feedback: "BRAVO! Alkış, şarkı, ödül! Artık büyük çocuksun!",
      },
    },
  },
  tr_kardes_gelmis: {
    text: "Eve yeni bir bebek geldi. Artık tek çocuk değilsin. Annen ona çok bakıyor...",
    choices: {
      kardes_sev: {
        text: "Bebeğe sarıl",
        feedback: "Yavaşça dokundun. Bebek sana baktı! Yeni en iyi arkadaşın olabilir.",
      },
      kardes_kiskan: {
        text: "Ben de bebek istiyorum!",
        feedback: "Kıskanmak normal. Ama annen seni de çok seviyor.",
      },
    },
  },
  tr_el_opme_zorlanma: {
    text: "Akraba ziyareti. Annen 'Gel hala/dayının elini öp' diyor. Ama onları pek sevmiyorsun...",
    choices: {
      el_op_zorla: {
        text: "Öp (istemeye istemeye)",
        feedback: "Zoraki öptün. Hala 'Maşallah büyümüş!' dedi, yanağını sıktı. İçin sıkıldı ama ailen memnun.",
      },
      el_opme_reddet: {
        text: "HAYIR, İSTEMİYORUM!",
        feedback: "Direnebildin! Ama hala kırıldı, annen utandı. 'Terbiyesiz çocuk' dediler.",
      },
    },
  },
  tr_misafir_oyuncak: {
    text: "Misafir geldi. Küçük bir çocuk var ve en sevdiğin oyuncağı istiyor. Annen bakıyor...",
    choices: {
      misafir_ver: {
        text: "Ver (gönülsüzce)",
        feedback: "Verdin ama için acıdı. Çocuk çok mutlu oldu. Annen sana sarıldı - 'Ne kadar cömert çocuk!'",
      },
      misafir_gizle: {
        text: "Oyuncağı gizle",
        feedback: "Oyuncağı yastığın altına sakladın. Çocuk ağladı, annen kızdı ama oyuncak seninle kaldı.",
      },
      misafir_baska: {
        text: "Başka oyuncak öner",
        feedback: "'Bu daha güzel!' dedin ve başka oyuncak verdin. İkisi de mutlu - çözüm buldun!",
      },
    },
  },
  tr_market_inat: {
    text: "Markette çikolata gördün. İSTİYORSUN! Ama annen 'Hayır' dedi. Yere yatacak mısın?",
    choices: {
      market_agla: {
        text: "Yere yat, ağla!",
        feedback: "AĞLAAA! Herkes baktı, annen utandı. Çikolatayı alamadın ama tüm marketin dikkati senindi.",
      },
      market_sabret: {
        text: "Üzül ama kabul et",
        feedback: "Zor oldu ama sabredebildin! Annen çok şaşırdı ve gururlandı. 'Büyüdün' dedi.",
      },
      market_pazarlik: {
        text: "'Lütfen' de, gözlerini aç",
        feedback: "Gözlerini açıp 'Lüüüüüütfen' dedin. Annen yumuşadı. Çikolatayı aldın - kazandın!",
      },
    },
  },
  tr_kopek_korkusu: {
    text: "Sokakta büyük bir köpek! Kuyruğunu sallıyor ama korkuyorsun. Babandaysa yaklaşıyor...",
    choices: {
      kopek_kac: {
        text: "Ağla ve kaç!",
        feedback: "Kaçtın! Güvendesin. Ama baban 'Korkak' dedi ve üzüldün.",
      },
      kopek_cesur: {
        text: "Babanın elini tut, yaklaş",
        feedback: "Titreyerek ama yaklaştın. Köpek senin elini yaladı! Sevimli miymiş! Cesur oldun!",
      },
    },
  },
  tr_abi_zorbalik: {
    text: "Ağabeyin/ablan yine oyuncağını aldı. Ağlıyorsun ama o gülüyor. Ne yapacaksın?",
    choices: {
      abi_anne_soyle: {
        text: "ANNEEEE!",
        feedback: "Anne geldi, ağabeyini/ablanı azarladı. Oyuncağı geri aldın ama 'ispiyoncu' dedi sana.",
      },
      abi_kavga: {
        text: "Geri almaya çalış!",
        feedback: "Cesurca savaştın! Yenildin ama hiç pes etmedin. Ağabeyin/ablan biraz saygı duydu.",
      },
      abi_pazarlik: {
        text: "Ağla ve 'Lütfen' de",
        feedback: "Gözyaşlarına dayanamadı. 'Tamam tamam al' dedi. Duygusal zeka kazandın!",
      },
    },
  },
  tr_sofra_konusma: {
    text: "Yemek masasında büyükler konuşuyor. Söze karışmak istiyorsun ama 'Çocuklar konuşmaz' dediler...",
    choices: {
      sofra_sus: {
        text: "Sus ve dinle",
        feedback: "Sustun. İçin sıkıldı ama 'Terbiyeli çocuk' dediler. Kuralları öğreniyorsun.",
      },
      sofra_konus: {
        text: "Karış yine de!",
        feedback: "'Sus bakalım!' dediler ama söyleyeceğini söyledin. Cesursun ama 'terbiyesiz' dediler.",
      },
    },
  },
  tr_apartman_hopla: {
    text: "Evde zıplamak çok eğlenceli! Ama alt komşu tavana vurdu. Annen 'Dur' diyor...",
    choices: {
      hopla_dur: {
        text: "Dur (İstemeye istemeye)",
        feedback: "Enerjin taşıyor ama durdun. Zor oldu! Annen 'İyi çocuk' dedi. Sabır öğreniyorsun.",
      },
      hopla_devam: {
        text: "Devam et!",
        feedback: "ZIIIP ZIIP! Komşu geldi çıldırdı, annen çok kızdı. Ama çok eğlendin!",
      },
    },
  },
  tr_komsu_bonbon: {
    text: "Komşu teyze sana şeker verdi. Ama onu sevmiyorsun, hep öpüyor seni. Almak zorundasın...",
    choices: {
      bonbon_al_tesekkur: {
        text: "Al ve 'Teşekkür ederim' de",
        feedback: "Teyze çok sevindi! Seni öptü, 'Ne tatlı çocuk!' dedi. Annen gurur duydu.",
      },
      bonbon_red: {
        text: "Alma ve kaç",
        feedback: "Teyze kırıldı, annen çok utandı. 'Terbiyesiz!' diye azarlandın.",
      },
    },
  },
  tr_kaybolma_panik: {
    text: "Çarşıdayken annenle gözgöze gelemedin. Etrafta yabancılar var. KAYBOLMUŞSUN!",
    choices: {
      kaybol_agla: {
        text: "Ağla ve yerinde dur",
        feedback: "Korkunç bir panik! Ağladın ama sonunda annen buldu seni. Sarıldı, hem ağladı hem azarladı.",
      },
      kaybol_ara: {
        text: "Sakin kal, etrafı ara",
        feedback: "Panik yerine düşündün! Girdiğiniz mağazayı hatırladın ve geri döndün. Anne oradaydı! Çok cesur oldun!",
      },
      kaybol_yardim: {
        text: "Bir teyzeye 'Annem kayıp' de",
        feedback: "Teyze yardım etti, annen bulundu. Doğru kişiden yardım istemek akıllıca!",
      },
    },
  },
  tr_inat_krizi: {
    text: "Annen diş fırçalaman gerektiğini söylüyor. Ama sen İNAT EDİYORSUN! Kimse seni zorlayamaz!",
    choices: {
      inat_direne: {
        text: "İNAT ET! HAYIR!",
        feedback: "HAYIIIIR! Ağladın, bağırdın, yere yattın! Sonunda pes ettin ama herkes yoruldu.",
      },
      inat_kabul: {
        text: "Tamam, kabul et",
        feedback: "Zor oldu ama söz dinledin. Dişlerini fırçaladın. 'Büyüdün' dediler! Gurur duydun.",
      },
    },
  },
  tr_baskasinin_oyuncak: {
    text: "Parkta başka çocuğun oyuncağı çok güzel. Sen de istiyorsun. O çocuk seninle oynamıyor...",
    choices: {
      oyuncak_cap: {
        text: "Çal ve kaç!",
        feedback: "Çaldın! Çocuk ağladı, annesi geldi ve senin annen çok utandı. Oyuncağı geri verdin.",
      },
      oyuncak_iste: {
        text: "'Benimle paylaşır mısın?' diye sor",
        feedback: "Nazikçe sordun! Çocuk paylaştı ve beraber oynadınız. Yeni arkadaş edindin!",
      },
      oyuncak_vazgec: {
        text: "Üzül ama vazgeç",
        feedback: "Herkese her şey ait değil. Zor bir ders ama öğrendin.",
      },
    },
  },
  tr_tuvalet_acil: {
    text: "Parkta tuvalete çok sıkıştın! Ama tuvalet yok. Ne yapacaksın?",
    choices: {
      tuvalet_agac: {
        text: "Ağacın arkasına gizlen",
        feedback: "Ağacın arkasında hallettin! Rahatladın ama annen 'Ayıp!' dedi.",
      },
      tuvalet_tutma: {
        text: "Tut, eve kadar dayan!",
        feedback: "ÇOOOOK zordu ama eve kadar tuttun! Koşa koşa tuvalete gittin. Tam zamanında!",
      },
    },
  },
  tr_tablet_sure: {
    text: "Çizgi film izliyorsun. Annen 'Tamam süre doldu!' diyor. Ama en heyecanlı yerindeydi!",
    choices: {
      tablet_devam: {
        text: "'Biraz daha!' diye yalvar",
        feedback: "Ağladın, yalvardın. Sonunda tableti aldılar. Çizgi filmin sonunu göremedin.",
      },
      tablet_kapat: {
        text: "Kapat ve başka şey yap",
        feedback: "Zor oldu! Ama tableti kapattın. Annen çok şaşırdı ve ödül olarak yarın daha fazla izletebilirsin!",
      },
    },
  },
  tr_dis_fircalama: {
    text: "Yatma vakti! Ama dişlerini fırçalaman lazım. Üşeniyorsun...",
    choices: {
      dis_fircala: {
        text: "Fırçala (İstemeye istemeye)",
        feedback: "Fırçaladın! Ağzın mis gibi. Annen 'İyi çocuk' dedi ve öpücük verdi.",
      },
      dis_atla: {
        text: "Atla, direk yat!",
        feedback: "Fırçalamadan yattın! Annen yakaladı ve azarladı. 'Dişlerin çürüyecek!' dedi.",
      },
    },
  },
  tr_park_kavga: {
    text: "Parkta başka bir çocuk seni itti! Düştün, acıdı. O çocuk kaçıyor...",
    choices: {
      kavga_vur: {
        text: "Kalk, onun da seni it!",
        feedback: "Kavga başladı! İkiniz de ağladınız, anneler ayırdı. 'Kavgacı çocuk!' dediler.",
      },
      kavga_agla: {
        text: "Ağla, anneye koş",
        feedback: "Ağlayarak anneye koştun. Sarıldı, teselli etti. O çocuğa kızdı.",
      },
      kavga_affet: {
        text: "Acıdı ama affet, devam et",
        feedback: "Affettin! Olgun bir davranış. O çocuk özür diledi ve beraber oynadınız.",
      },
    },
  },
  tr_yemek_inat: {
    text: "Annen yemek yaptı. Ama beğenmedin! Yemeyeceksin!",
    choices: {
      yemek_red: {
        text: "HAYIR! İSTEMİYORUM!",
        feedback: "İnat ettin! Saatlerce sofradan kalkmadın. Sonunda aç yattın.",
      },
      yemek_ye: {
        text: "Ye (İstemeye istemeye)",
        feedback: "Zor oldu ama yedin. Aslında kötü değilmiş! Annen çok mutlu oldu.",
      },
    },
  },
  tr_oyuncak_magazasi: {
    text: "Oyuncak mağazasındasın! Etraf oyuncak dolu! Bir tane almak istiyorsun AMA...",
    choices: {
      magaza_al: {
        text: "'Alabilir miyim?' diye sor",
        feedback: "Sordun! Annen 'Tamam bir tane' dedi. Seçim yapmak zor oldu ama mutlusun!",
      },
      magaza_agla: {
        text: "Yere yat, ağla!",
        feedback: "Dramaaaa! Herkes baktı, annen seni dışarı çıkardı. Oyuncak alamadın.",
      },
      magaza_sabir: {
        text: "Sadece bak ve çık",
        feedback: "İnanılmaz sabır! Baktın, dokundun ama isteğe direnebildin. Annen çok şaşırdı - 'Büyümüşsün' dedi!",
      },
    },
  },
  tr_bebek_yardim: {
    text: "Annen bebekle uğraşıyor, çok yorgun. Sana 'Biberon getirir misin?' diyor...",
    choices: {
      bebek_yardim_et: {
        text: "Yardım et!",
        feedback: "Biberon getirdin! Annen çok mutlu oldu. 'Büyük kardeş olmuşsun!' dedi ve sarıldı.",
      },
      bebek_red: {
        text: "'Oynuyorum!' de, reddet",
        feedback: "Annen üzüldü. 'Bencil çocuk' dedi. Oyununa devam ettin ama için buruk.",
      },
    },
  },
  tr_top_cama_vurma: {
    text: "Top oynuyorsun. Top kaydı ve KOMŞUNUN CAMI! CAAAAAM! Top patladı ama cam kırılmadı...",
    choices: {
      cam_kac: {
        text: "Kaç saklan!",
        feedback: "Kaçtın! Ama komşu görmüş. Gelip söyledi. Hem azarlandın hem korkak oldun.",
      },
      cam_ozur: {
        text: "Komşuya git, özür dile",
        feedback: "Çok korkuyordun ama gittin, özür diledin! Komşu affetti - 'Dürüst çocuk' dedi!",
      },
    },
  },
  tr_ev_yardimi: {
    text: "Annen evi topluyor. Çok yorgun görünüyor. Oyuncaklarını toplar mısın?",
    choices: {
      ev_yardim: {
        text: "Yardım et, oyuncakları topla",
        feedback: "Topladın! Annen sana sarıldı - 'Sen benim en büyük yardımcımsın!' dedi.",
      },
      ev_oyna: {
        text: "Oynamaya devam et",
        feedback: "Annen tek başına topladı. Üzgün görünüyor. 'Bencil' diyen sesini duydun.",
      },
    },
  },
  tr_bayram_sabahi: {
    text: "Bayram sabahı! Ev mis gibi kokuyor, baklava hazır. Büyüklerin elini öpme zamanı.",
    choices: {
      bayram_full_respect: {
        text: "Tüm büyüklerin elini öp (Zor - İçe kapanık için)",
        feedback: "Her el öpüşünde içinden 'ne zaman bitecek' dedin. Ama ceplerin harçlıkla doldu ve aile mutlu.",
      },
      bayram_minimal: {
        text: "Sadece anne-babayı öp, köşede otur",
        feedback: "Minimum etkileşimle atlattın. Harçlık az oldu ama rahatın yerinde.",
      },
      bayram_social_butterfly: {
        text: "Herkesi kucakla, sohbet et!",
        feedback: "'Maşallah ne terbiyeli!' Herkes seni sevdi. Ceplerin taştı, ruhun doydu.",
      },
      bayram_sleep: {
        text: "Uyumaya devam et",
        feedback: "'Bu çocuk terbiyesiz!' Harçlık alamadın, azarlandın. Ama uyudun en azından.",
      },
    },
  },
  tr_ramazan_iftar: {
    text: "Ramazan ayı! Akşam iftara misafirler geliyor. Annen mutfakta çorba, pilav, kebap hazırlıyor.",
    choices: {
      iftar_help: {
        text: "Sofra hazırlamaya yardım et",
        feedback: "Tabakları, bardakları dizedin. Annen gururla baktı. Misafirler 'ne yardımsever' dedi.",
      },
      iftar_fast: {
        text: "Oruç tutmayı dene (Zor seçim)",
        feedback: "Akşama kadar dayandın! Açlık zor ama başardın. Herkes seninle gurur duydu.",
      },
      iftar_avoid: {
        text: "Odanda bekle",
        feedback: "Yardım etmedin ama en azından ayak altında dolaşmadın.",
      },
    },
  },
  tr_kurban_bayrami: {
    text: "Kurban Bayramı! Baban kurban kestirdi. Etler pay edilecek.",
    choices: {
      kurban_distribute: {
        text: "Et dağıtımına yardım et",
        feedback: "Fakir ailelere et dağıttın. Teşekkür eden gözler, dualar... İyilik yapmak güzel.",
      },
      kurban_cousins: {
        text: "Kuzenlerle oyna",
        feedback: "Bayramda en güzel şey kuzenlerle oynamak!",
      },
      kurban_witness: {
        text: "Kesime şahit ol (Zor seçim)",
        feedback: "Hayatın gerçekleriyle yüzleştin. Zor ama olgunlaştırıcı bir deneyimdi.",
      },
    },
  },
  tr_karne_gunu: {
    text: "Karne günü geldi. Notların ortada - ne çok iyi ne çok kötü.",
    choices: {
      karne_proud: {
        text: "Gururla eve git",
        feedback: "Karneni masaya koydun. Ailenin tepkisi notlarına bağlı...",
      },
      karne_compare: {
        text: "Arkadaşlarla karşılaştır",
        feedback: "Herkes karneleri gösteriyor. Kimi sevinçli, kimi üzgün. Sen neredesin?",
      },
      karne_hide: {
        text: "Karneyi sakla, sonra göster",
        feedback: "Bir süre sakladın ama eninde sonunda ortaya çıkacak... ve daha kötü olacak.",
      },
      karne_honest: {
        text: "Kötü de olsa hemen göster (Zor seçim)",
        feedback: "Korkarak gösterdin ama dürüst oldun. Ailen en azından bunu takdir etti.",
      },
    },
  },
  tr_lgs_hazirlik: {
    text: "LGS'ye 6 ay kaldı. Herkes çalışıyor. Sen ne yapacaksın?",
    choices: {
      lgs_dershane: {
        text: "Dershaneye yazıl",
        feedback: "Dershane yoğun ama etkili. Notların yükselmeye başladı.",
      },
      lgs_self_study: {
        text: "Evde kendi çalış (Zor - Sabırsız için)",
        feedback: "YouTube'dan konuları izledin, testler çözdün. Kendi başına yapmak zor ama öğreniyorsun.",
      },
      lgs_ignore: {
        text: "Umurumda değil",
        feedback: "Arkadaşlarınla takıldın. Notlar düştü ama eğlendin. Sonra ne olacak?",
      },
    },
  },
  tr_yks_stresi: {
    text: "YKS'ye az kaldı. Türkiye'nin en önemli sınavlarından biri. Hazır mısın?",
    choices: {
      yks_grind: {
        text: "Sabahla ve çalış",
        feedback: "Gözlerin kan çanağı ama paragraflar, formüller kafana girdi. Değer mi? Göreceğiz.",
      },
      yks_tutor: {
        text: "Özel ders al",
        feedback: "Birebir ders çok etkili. Eksiklerin kapandı.",
      },
      yks_rest: {
        text: "Mola ver, dinlen (Zor - Çalışkanlar için)",
        feedback: "Bir gün hiç çalışmadın. Suçluluk var ama beynin teşekkür ediyor.",
      },
    },
  },
  tr_akraba_sorgusu: {
    text: "Bayramda teyzeler sorguya çekti: 'Derslerin nasıl? Kaç alacaksın?'",
    choices: {
      akraba_polite: {
        text: "Kibar cevap ver",
        feedback: "'Çalışıyorum teyze, inşallah iyi olur.' Klasik ama işe yarıyor.",
      },
      akraba_snap: {
        text: "Sert çık: 'Sizi ilgilendirmez!'",
        feedback: "Ortam buz kesti. Annen sonra fırça attı. Ama içini döktün.",
      },
      akraba_polite_introvert: {
        text: "Kibar cevap ver (Zor - İçe kapanık için)",
        feedback: "Her kelime işkence gibiydi ama başardın. Sosyal becerin gelişiyor.",
      },
      akraba_deflect: {
        text: "Konuyu değiştir",
        feedback: "Ustalıkla konuyu teyzenin oğluna çevirdin. Taktik işe yaradı.",
      },
    },
  },
  tr_evlilik_baskisi: {
    text: "'Sen hâlâ bekâr mısın?' Akrabalar yine başladı. Nişan, düğün, çocuk derken...",
    choices: {
      evlilik_smile: {
        text: "Gülerek geçiştir",
        feedback: "'Önce okul bitsin teyze!' İşe yaradı.",
      },
      evlilik_boundary: {
        text: "Sınır koy: 'Benim hayatım!'",
        feedback: "Ortam soğudu ama sınırlarını koydun. Herkes seni ciddiye almaya başladı.",
      },
      evlilik_lie: {
        text: "Biri var de (yalan)",
        feedback: "Herkes heyecanlandı! Şimdi her bayram 'ne oldu' diye soracaklar...",
      },
    },
  },
  tr_mahalle_futbolu: {
    text: "Mahallede çocuklar toplandı. Taşlarla kale yapıldı, takımlar belli. 'Bir kişi eksik!'",
    choices: {
      futbol_join: {
        text: "Hemen katıl!",
        feedback: "Toz toprak içinde kaldın ama müthiş eğlendin!",
      },
      futbol_join_introvert: {
        text: "Katıl (Zor seçim)",
        feedback: "Başta garip hissettin ama oyunun içine girince unutun. Eğlendin!",
      },
      futbol_goalkeeper: {
        text: "Kaleci ol (daha az temas)",
        feedback: "Kalecilik zor iş ama daha az koşuşturma. Kritik kurtarışlar yaptın!",
      },
      futbol_watch: {
        text: "İzle sadece",
        feedback: "Kenarda oturdun. Arkadaşların biraz kırgın.",
      },
    },
  },
  tr_internet_kafe: {
    text: "Arkadaşların internet kafeye gece kaçamağı planlıyor. Evde haberleri yok...",
    choices: {
      kafe_go_rebel: {
        text: "Git! Kurallar çiğnenmek için var!",
        feedback: "Sabaha kadar oynadın! Eve gizlice girdin ama anneni uyandırdın. Feci azarlandın. Değdi mi?",
      },
      kafe_go_conformist: {
        text: "Git (Zor - Uyumcu için)",
        feedback: "Tüm gece suçluluk duygusuyla oynadın. Eğlendin mi? Emin değilsin.",
      },
      kafe_permission: {
        text: "Aileden izin iste",
        feedback: "'Evde kal' dediler. Arkadaşların gitti ama en azından dürüst oldun.",
      },
      kafe_online: {
        text: "Evde online katıl",
        feedback: "Evde online katıldın. Aynı değil ama güvendesin.",
      },
    },
  },
  tr_sigara_teklifi: {
    text: "Okulun arkasında büyük çocuklar sigara içiyor. Biri sana uzattı: 'Dene bir tane!'",
    choices: {
      sigara_refuse_brave: {
        text: "Kesinlikle hayır",
        feedback: "Baskıya boyun eğmedin. Dalga geçtiler ama doğru olanı yaptın.",
      },
      sigara_refuse_easy: {
        text: "Hayır, bana göre değil",
        feedback: "Rahatça reddettin. Senin tarzın değil, herkes biliyor.",
      },
      sigara_try: {
        text: "Bir tane dene",
        feedback: "Öksürdün, gözlerin yaşardı. Pek sevmedin ama 'cool' göründün.",
      },
      sigara_excuse: {
        text: "Bahane uydur ve kaç",
        feedback: "'Annem çağırıyor' dedin ve uzaklaştın. Korkak mı, akıllı mı?",
      },
    },
  },
  tr_farkli_olmak: {
    text: "Herkes bir kalıba uyuyor. Sen farklı hissediyorsun. Ne yapacaksın?",
    choices: {
      farkli_be_yourself: {
        text: "Kendin ol, ne derlerse desinler",
        feedback: "Bazıları garip buldu, bazıları hayranlık duydu. Yalnız kaldın ama özgürsün.",
      },
      farkli_conform: {
        text: "Uyum sağla, hayat kolaylaşsın",
        feedback: "Herkes gibi oldun. Kabul gördün ama aynaya baktığında tanıdığın kişiyi görüyor musun?",
      },
      farkli_rebel: {
        text: "Tam tersini yap, isyan!",
        feedback: "Herkesin tersine gittin. Dikkat çektin ama... bu gerçekten sen misin yoksa sadece zıtlık mı?",
      },
    },
  },
  tr_anne_yemegi: {
    text: "Dışarıda arkadaşlarla buluştun. McDonald's mı, eve dönüp annenin yemekleri mi?",
    choices: {
      yemek_fastfood: {
        text: "Fast food! 🍔",
        feedback: "Burger, kola, patates... Lezzetli ama annen üzülecek.",
      },
      yemek_home: {
        text: "Eve, annemin yemekleri",
        feedback: "Kuru fasulye, pilav, ayran... Hiçbir şey anne yemeği gibi değil.",
      },
      yemek_home_hard: {
        text: "Eve dön (Zor - Bencil için)",
        feedback: "Arkadaşlarını bırakıp eve döndün. Annen mutlu oldu. Garip bir sıcaklık hissettin.",
      },
    },
  },
  tr_milli_mac: {
    text: "Türkiye milli maç oynuyor! Mahalle coşkulu, kornalar çalıyor.",
    choices: {
      mac_balkon: {
        text: "Balkondan izle ve bağır",
        feedback: "'GOOOL!' Tüm mahalle çığlık attı. Birlik duygusu muhteşem.",
      },
      mac_kafe: {
        text: "Kafede izle",
        feedback: "Kafe tıklım tıklıktı. Tanımadığın insanlarla kucaklaştın! Garip ama güzeldi.",
      },
      mac_ignore: {
        text: "İlgilenmiyorum",
        feedback: "Sessiz bir gece geçirdin. Gürültü rahatsız etti ama kendi halinde kaldın.",
      },
    },
  },
  tr_23_nisan: {
    text: "23 Nisan Çocuk Bayramı! Okulda kutlama var. Sınıf şiir okuyacak çocuk arıyor.",
    choices: {
      "23nisan_poem": {
        text: "Ben okurum! (Zor - İçe kapanık için)",
        feedback: "'Türk çocuğu her şeyin en iyisine layıktır!' Alkışlar koptu. Başardın!",
      },
      "23nisan_poem_easy": {
        text: "Sahneye çık!",
        feedback: "Muhteşem bir performans! Bu senin doğal ortamın.",
      },
      "23nisan_flag": {
        text: "Bayrak taşıyayım",
        feedback: "Kortejin önünde bayrak taşıdın. Gururla yürüdün.",
      },
      "23nisan_watch": {
        text: "Seyirci kalayım",
        feedback: "Arkadaşlarını izledin. Belki gelecek sene...",
      },
    },
  },
  tr_oyun_parki: {
    text: "Parkta çocuklar oyun oynuyor! Salıncaklar, kaydıraklar, kum havuzu... Hangisine koşsam?",
    choices: {
      park_swing: {
        text: "Salıncağa koş!",
        feedback: "Gökyüzüne doğru uçuyorsun! Rüzgar yüzünde, kahkahalar havada.",
      },
      park_friends: {
        text: "Çocuklarla tanış",
        feedback: "'Merhaba, ben senin adın. Oynayalım mı?' İlk adımı attın!",
      },
      park_sandbox: {
        text: "Kum havuzunda oyna",
        feedback: "Kum kaleler yaptın. Sanat eserin güneşte parlıyor!",
      },
    },
  },
  tr_ilk_bisiklet: {
    text: "Baban sana bisiklet aldı! Ama binmeyi bilmiyorsun. Yardımcı tekerlekler var... takılsın mı?",
    choices: {
      bike_helper: {
        text: "Yardımcı tekerleklerle başla",
        feedback: "Güvenli ama eğlenceli! Yavaş yavaş öğreniyorsun.",
      },
      bike_brave: {
        text: "Direk dene! (Zor)",
        feedback: "Birkaç kez düştün, dizlerin sıyrıldı... AMA SÜRDÜÜÜn! Baban alkışlıyor!",
      },
    },
  },
  tr_cizgi_film: {
    text: "Sabah erkenden kalktın. Çizgi filmler başlıyor! Ama annen kahvaltı hazırladı...",
    choices: {
      cizgi_tv: {
        text: "Önce çizgi film!",
        feedback: "Pokemon kaçmaz! Annen homurdandı ama izin verdi.",
      },
      cizgi_breakfast: {
        text: "Önce kahvaltı",
        feedback: "Zor oldu ama sabrettin. Annen mutlu, karnın tok. Çizgi film de başlamadı daha!",
      },
      cizgi_both: {
        text: "TV önünde ye",
        feedback: "Tabağı kucağına aldın. Pratik çözüm ama annen pek hoşlanmadı.",
      },
    },
  },
  tr_okul_ilk_gun: {
    text: "Okulun ilk günü! Yeni arkadaşlar, yeni maceralar! Heyecanlı mısın?",
    choices: {
      okul_brave: {
        text: "Cesurca gir",
        feedback: "Ağlamadın, koşmadın. Öğretmen 'aferin' dedi. Bu yeni başlangıç!",
      },
      okul_excited: {
        text: "Heyecanla koş!",
        feedback: "Sınıfa koştun, herkesle tanıştın. Bu senin ortamın!",
      },
      okul_cry: {
        text: "Ağla ve sarıl",
        feedback: "Annen seni teselli etti. Herkes ağlar ilk gün, normal.",
      },
    },
  },
  tr_har_l_k: {
    text: "Hafta sonu harçlık günü! Baban sana 20 TL verdi. Ne yapacaksın?",
    choices: {
      har_l_k_save: {
        text: "Kumbaraya at",
        feedback: "Harcamak için elledin titirdi ama koydun. Gelecekte işe yarayacak!",
      },
      har_l_k_candy: {
        text: "Bakkalda harca",
        feedback: "Cips, çikolata, gazoz... Mutlusun ama para gitti!",
      },
      har_l_k_half: {
        text: "Yarısını biriktir",
        feedback: "Akıllıca! Hem eğlendin hem biriktirdin.",
      },
    },
  },
  tr_sabah_uyanis: {
    text: "Sabah oldu... Alarm çalıyor. Yataktan çıkmak o kadar zor ki!",
    choices: {
      sabah_erken: {
        text: "Hemen kalk",
        feedback: "Erken kalktın, güne enerjik başladın!",
      },
      sabah_ertele: {
        text: "5 dakika daha...",
        feedback: "5 dakika 30 dakika oldu... Yine geç kaldın!",
      },
      sabah_spor: {
        text: "Sabah sporu yap",
        feedback: "Şınav, mekik, koşu... Zordu ama harika hissediyorsun!",
      },
    },
  },
  tr_ev_isler: {
    text: "Annen senin odanı temizlemeni istiyor. Ama arkadaşların dışarıda bekliyor...",
    choices: {
      ev_temizle: {
        text: "Önce temizle",
        feedback: "Sızlanarak da olsa temizledin. Annen mutlu, vicdan rahat.",
      },
      ev_sonra: {
        text: "Sonra yaparım!",
        feedback: "Çıktın ama eve dönünce hesap var...",
      },
      ev_hizli: {
        text: "Hızlıca topla",
        feedback: "Her şeyi yatağın altına tıktın. Sayılır... sanırım?",
      },
    },
  },
  tr_bakkal: {
    text: "Annen seni bakkala gönderdi. Alışveriş listesi var ama fazla para da verdi...",
    choices: {
      bakkal_honest: {
        text: "Sadece listedekileri al",
        feedback: "Para üstünü eksiksiz verdin. Annen güveniyor sana.",
      },
      bakkal_snack: {
        text: "Bir şeyler de kendine al",
        feedback: "Cips aldın ama suçluluk duyuyorsun. Annen fark eder mi?",
      },
      bakkal_save: {
        text: "Fazlayı biriktir",
        feedback: "Para cebinde... ama bu doğru mu?",
      },
    },
  },
  tr_komsu_ziyaret: {
    text: "Komşular geldi! Çay, pasta, sohbet. Ne güzel bir kalabalık!",
    choices: {
      komsu_servis: {
        text: "Çay servisi yap",
        feedback: "'Maşallah ne terbiyeli!' Herkes seni övdü. Zordu ama değdi.",
      },
      komsu_sohbet: {
        text: "Sohbete katıl",
        feedback: "Büyüklerle sohbet ettin. Dünya hakkında yeni şeyler öğrendin!",
      },
      komsu_kac: {
        text: "Odana kaç",
        feedback: "Sessizce kayboldun. Rahat ama annen kırgın.",
      },
    },
  },
  tr_yagmurlu_gun: {
    text: "Bugün hava yağmurlu. Dışarı çıkmak mümkün değil. Evde ne yapacaksın?",
    choices: {
      yagmur_kitap: {
        text: "Kitap oku",
        feedback: "Yağmurun sesi ve kitabın kokusu... Huzurlu bir gün.",
      },
      yagmur_oyun: {
        text: "Video oyunu oyna",
        feedback: "Saatlerce oynadın. Eğlenceli ama zaman uçtu!",
      },
      yagmur_tv: {
        text: "Film izle",
        feedback: "Aile filmi izlediniz. Güzel bir gün geçirdiniz.",
      },
      yagmur_yagmur: {
        text: "Yağmurda oyna!",
        feedback: "Sırılsıklam oldun ama çok eğlendin! Annen fırçayı bastı ama gözleri gülüyordu.",
      },
    },
  },
  tr_aile_yemegi: {
    text: "Akşam yemeği vakti. Aile masada toplandı. Bugün ne konuşulacak?",
    choices: {
      yemek_anlat: {
        text: "Gününü anlat",
        feedback: "Herkes seni dinledi. Aile sohbetleri güzel!",
      },
      yemek_dinle: {
        text: "Sessizce ye",
        feedback: "Sessiz bir yemek ama huzurluydu.",
      },
      yemek_soru: {
        text: "Sorular sor",
        feedback: "'Baba sen küçükken ne yapardın?' Hikayeler dinledin.",
      },
    },
  },
  tr_ders_calis: {
    text: "Ders çalışman lazım ama telefon çok çekici görünüyor...",
    choices: {
      ders_fokus: {
        text: "Odaklan ve çalış",
        feedback: "Zordu ama 2 saat çalıştın. Konular oturdu!",
      },
      ders_telefon: {
        text: "Biraz telefona bak",
        feedback: "'5 dakika' 2 saat oldu. Yine erteleme...",
      },
      ders_pomodoro: {
        text: "25dk çalış, 5dk mola",
        feedback: "Pomodoro tekniği işe yaradı! Hem çalıştın hem dinlendin.",
      },
    },
  },
  tr_hasta_olma: {
    text: "Bugün kendini iyi hissetmiyorsun. Boğazın ağrıyor, ateşin var...Okula gitmeli misin?",
    choices: {
      hasta_kal: {
        text: "Evde kal",
        feedback: "Çorba, battaniye, uyku... İyileşiyorsun.",
      },
      hasta_git: {
        text: "Yine de git",
        feedback: "Zor bir gün geçirdin ama devamsızlık yapmadın.",
      },
      hasta_numara: {
        text: "Hasta numarası yap",
        feedback: "Aslında o kadar hasta değilsin ama gün boş geçti.",
      },
    },
  },
  tr_sinav_gunu: {
    text: "Bugün sınav var! Yeterince çalıştın mı? Stres yapıyorsun...",
    choices: {
      sinav_sakin: {
        text: "Sakin ol ve yap",
        feedback: "Derin nefes, odaklan, yaz. Sandığından iyi geçti!",
      },
      sinav_panik: {
        text: "Panik yap",
        feedback: "Aklın karıştı, sorular bulanıklaştı. Keşke sakin olabilseydin...",
      },
      sinav_kopya: {
        text: "Kopya çek",
        feedback: "Yakalanmadın... bu sefer. Ama içindeki ses susmadı.",
      },
    },
  },
  tr_tatil_plani: {
    text: "Yaz tatili başlıyor! 3 ay boş zaman. Ne yapacaksın?",
    choices: {
      tatil_kamp: {
        text: "Yaz kampına git",
        feedback: "Yeni arkadaşlar, doğa, macera! Hayatının yazı oldu!",
      },
      tatil_kurs: {
        text: "Kurs al (yüzme/müzik)",
        feedback: "Yeni bir yetenek öğrendin. Tatil boşa geçmedi!",
      },
      tatil_ev: {
        text: "Evde takıl",
        feedback: "Oyun, uyku, TV... Rahat ama sıkıcı geçti.",
      },
      tatil_koy: {
        text: "Köye git",
        feedback: "Nine'nin bahçesi, taze meyve, temiz hava. Şehir stresi eridi!",
      },
    },
  },
  tr_kayip_esya: {
    text: "En sevdiğin kalemini/oyuncağını kaybettin! Ne yapacaksın?",
    choices: {
      kayip_ara: {
        text: "Sabırla ara",
        feedback: "Her yere baktın ve BULDUN! Sabır meyvesini verdi.",
      },
      kayip_agla: {
        text: "Ağla ve vazgeç",
        feedback: "Üzüldün ama annen teselli etti. Belki yenisini alır...",
      },
      kayip_sucla: {
        text: "Başkasını suçla",
        feedback: "'Kardeşim aldı!' dedin ama... gerçekten öyle mi?",
      },
    },
  },
  tr_yeni_komsular: {
    text: "Yan daireye yeni komşular taşındı. Onlarla tanışacak mısın?",
    choices: {
      komsu_merhaba: {
        text: "Git ve tanış",
        feedback: "'Merhaba, ben komşunuz!' Yeni bir arkadaşlık başlayabilir!",
      },
      komsu_bekle: {
        text: "Anne babam tanışsın önce",
        feedback: "Aileler tanıştı. Sen de tanıştırıldın. Daha kolaydı böyle.",
      },
      komsu_kac: {
        text: "Uzak dur",
        feedback: "Tanışmadın. Kim bilir nasıl insanlar...",
      },
    },
  },
  tr_kavga_sahit: {
    text: "Okulda iki çocuk kavga ediyor. Sen de oradasın. Ne yapacaksın?",
    choices: {
      kavga_ayir: {
        text: "Ayırmaya çalış",
        feedback: "Araya girdin! Birkaç yumruk yedin ama kavga bitti. Kahraman!",
      },
      kavga_ogretmen: {
        text: "Öğretmen çağır",
        feedback: "Öğretmen geldi, kavga bitti. Doğru olanı yaptın... galiba.",
      },
      kavga_izle: {
        text: "Sadece izle",
        feedback: "İzledin... Vicdanın rahat değil ama güvendesin.",
      },
    },
  },
  tr_sinif_gorev: {
    text: "Öğretmen sınıf başkanı/temsilci arıyor. Aday olacak mısın?",
    choices: {
      gorev_aday: {
        text: "Aday ol!",
        feedback: "Sınıfın önünde konuştun! Seçildin mi bilmiyorsun ama cesaretin alkışlandı!",
      },
      gorev_destek: {
        text: "Başkasını destekle",
        feedback: "Arkadaşını destekledin. O kazandı, sen de mutlusun.",
      },
      gorev_hayir: {
        text: "Hayır, teşekkürler",
        feedback: "Sorumluluk istemiyorsun. Rahat ol.",
      },
    },
  },
  tr_bos_gun: {
    text: "Bugün hiçbir planın yok. Evde ne yapacaksın?",
    choices: {
      bos_macera: {
        text: "Dışarı çık, keşfet",
        feedback: "Mahallede dolaştın, yeni yerler keşfettin. Güzel bir gün!",
      },
      bos_hobi: {
        text: "Hobi yap",
        feedback: "Resim yaptın, müzik dinledin. Yaratıcı bir gün geçti.",
      },
      bos_uyku: {
        text: "Tüm gün uyu",
        feedback: "Sabah 6'dan akşam 6'ya uyudun. Dinlendin ama gün kayıp!",
      },
    },
  },
  tr_hediye_secimi: {
    text: "Annenin doğum günü yaklaşıyor. Bir şey almak istiyorsun ama paran az...",
    choices: {
      hediye_el: {
        text: "El yapımı bir şey yap",
        feedback: "Kart yaptın, resim çizdin. Annen ağladı mutluluktan!",
      },
      hediye_satin: {
        text: "Tüm birikimini harca",
        feedback: "Güzel bir çiçek aldın. Paran bitti ama annenin gözleri parlıyordu.",
      },
      hediye_unut: {
        text: "Unuttum desem?",
        feedback: "Annen üzgün görünmedi ama içten içe kırıldı...",
      },
    },
  },
  dilemma_sinav_kopya_gorme: {
    text: "Sınavda en yakın arkadaşın kopya çekiyor. Öğretmen tam o anda sana döndü: 'Bir şey mi gördün?' Arkadaşın sana yalvaran gözlerle bakıyor.",
    choices: {
      tell_truth: {
        text: "Doğruyu söyle (Arkadaşını sat)",
        feedback: "Öğretmen arkadaşını disipline gönderdi. 'Dürüst çocuk' dedin ama koridorda herkes senden uzak duruyor. Arkadaşın bir daha seninle konuşmadı.",
      },
      protect_friend: {
        text: "Yalan söyle: 'Görmedim hocam'",
        feedback: "Arkadaşın kurtuldu ve sana minnettar. Ama öğretmen şüpheleniyor, ailen de senin 'o çocukla' takılmanı sorguluyor.",
      },
      stay_silent: {
        text: "Sessiz kal, omuz silk",
        feedback: "Öğretmen ikna olmadı. 'Sen de biliyordun' dedi. Ne arkadaşını korudun, ne de dürüst oldun. İkisi de sana kırgın.",
      },
    },
  },
  dilemma_ispiyoncu_damgasi: {
    text: "Sınıfta yeni bir olay oldu. Herkes sana bakıyor - 'Bu ispiyoncuya söylemeyin' diye fısıldaşıyorlar. Geçmişte arkadaşını ihbar ettiğin herkesin aklında.",
    choices: {
      choice_0: {
        text: "Kendini savun: 'Doğru olanı yaptım'",
        feedback: "Sınıf ikiye bölündü. Bazıları sana hak verdi ama çoğunluk hâlâ mesafeli. Yalnızlık ağır geliyor.",
      },
      choice_1: {
        text: "Özür dile ve pişmanlık göster",
        feedback: "Herkesin önünde 'Hata yaptım' dedin. Bazıları affetti ama 'zayıf' göründün. Baban duyunca kızdı.",
      },
      choice_2: {
        text: "Yeni arkadaş grubu bul",
        feedback: "Eski grubu bıraktın. Yeni arkadaşlar farklı - daha çalışkan ama daha sıkıcı. Geçmişi geride bıraktın.",
      },
    },
  },
  dilemma_arkadaslik_testi: {
    text: "O gün kopya çekerken koruduğun arkadaşın şimdi popüler grubun lideri. Sen ise hâlâ aynı yerdesin. Bir gün seni 'loser' diye çağırdığını duydun.",
    choices: {
      choice_0: {
        text: "Yüzleş: 'O gün seni kurtardım!'",
        feedback: "'Evet, teşekkürler ama artık farklı dünyalardayız' dedi. Acı gerçek: İyilik her zaman karşılık bulmaz.",
      },
      choice_1: {
        text: "Sessiz kal, ders al",
        feedback: "Acı bir ders öğrendin: Bazı insanlar sadece ihtiyaçları olduğunda yanında. Bu seni daha temkinli yaptı.",
      },
      choice_2: {
        text: "O günkü sırrı ifşa et",
        feedback: "İntikam aldın ama herkes seni 'güvenilmez' olarak gördü. İki yüzlü damgası yedin.",
      },
    },
  },
  dilemma_aile_beklentisi: {
    text: "Ailen senin için planlar yapıyor - iyi bir üniversite, güvenli bir meslek. Ama sen farklı bir şey istiyorsun. Sanat, müzik, belki de seyahat...",
    choices: {
      obey_family: {
        text: "Aileye itaat et (Hayallerinden vazgeç)",
        feedback: "Baban gururla baktı. Ama geceleri yatakta, gitarın duvarda asılı duruyor - hiç çalmıyorsun artık. İçinde bir şey söndü.",
      },
      follow_dreams: {
        text: "Hayallerinin peşinden git (Aileyle çatış)",
        feedback: "Baban kapıyı çarptı. Annen ağladı. Ama gitar eline aldığında, parmaklarından dökülen notalar sana 'Doğru yoldasın' diyor.",
      },
      compromise: {
        text: "Orta yol ara (İkisini de yarım yap)",
        feedback: "Hem ders çalışıyorsun hem gitar. Ama ikisinde de 'ortalama'sın. Ne ailen tam mutlu, ne sen tam tatmin.",
      },
    },
  },
  dilemma_supressed_dreams: {
    text: "17 yaşındasın. Tıp fakültesine hazırlanıyorsun - ailenin istediği gibi. Ama her gece aynı rüyayı görüyorsun: Sahnede, elinde gitar, binlerce kişi alkışlıyor...",
    choices: {
      choice_0: {
        text: "Devam et, artık çok geç",
        feedback: "Kitaplara gömüldün. Notların iyi ama içindeki ses hiç susmadı. Belki bir gün... belki asla.",
      },
      choice_1: {
        text: "Son bir şans: Gizlice yarışmaya katıl",
        feedback: "Yarışmada sahneye çıktın. Kazanamadın ama jüri 'Potansiyel var' dedi. Baban öğrenince çıldırdı. Değdi mi?",
      },
    },
  },
  dilemma_family_reconciliation: {
    text: "Hayallerinin peşinden gittiğin için ailen seninle soğuk. Ama annenin doğum günü yaklaşıyor. Bir karar vermelisin.",
    choices: {
      choice_0: {
        text: "Git ve özür dile",
        feedback: "Annen seni kucakladı, baban hâlâ mesafeli. Ama en azından kapı aralandı.",
      },
      choice_1: {
        text: "Gitme, gururunu koru",
        feedback: "Doğum gününde evde oturup duvara baktın. Haklı olsan bile, yalnızlık ağır.",
      },
      choice_2: {
        text: "Hediye gönder ama gitme",
        feedback: "Annen hediyeyi aldı ve ağladı. Baban 'Kendisi gelseydi' dedi. Yarım adım.",
      },
    },
  },
  dilemma_kardes_hirsizlik: {
    text: "Küçük kardeşinin çantasından komşunun kaybolan telefonu düştü. Kardeşin ağlayarak 'Söyleme' diyor. Komşu kapı kapı soruyor.",
    choices: {
      choice_0: {
        text: "Kardeşini koru, sessiz kal",
        feedback: "Kardeşin rahat bir nefes aldı. Ama sen rahat uyuyamıyorsun. Komşu hâlâ arıyor...",
      },
      choice_1: {
        text: "Kardeşini zorlayarak itiraf ettir",
        feedback: "Kardeşin özür diledi, telefon iade edildi. Komşu anlayışlıydı. Ama kardeşin sana 'Hain' diyor.",
      },
      choice_2: {
        text: "Kendin götür, kardeşini ele verme",
        feedback: "'Yolda buldum' dedin. Komşu teşekkür etti. Ama yalan söyledin ve cebinden para çıktı.",
      },
    },
  },
  dilemma_truth_comes_out: {
    text: "Komşu sonunda öğrendi - kardeşinin çaldığını. Şimdi hem kardeşin hem sen töhmet altındasın. 'Sen biliyordun değil mi?' diye soruyorlar.",
    choices: {
      choice_0: {
        text: "Tamamen itiraf et",
        feedback: "Her şey ortaya çıktı. Ailen utandı, komşularla ilişkiler bozuldu. Ama en azından yük kalktı.",
      },
      choice_1: {
        text: "Bilmiyordum de",
        feedback: "Bir yalan daha... Kimse tam inanmadı. Güvenilirliğin azaldı.",
      },
    },
  },
  dilemma_yarisma_sabotaj: {
    text: "Büyük bilgi yarışmasının finalisin. Rakibin - aynı zamanda arkadaşın - çok iyi. Ama onun hazırlık dosyasını/stratejisini çalabilirsin. Kimse bilmez.",
    choices: {
      choice_0: {
        text: "Dosyayı çal ve kazan",
        feedback: "Yarışmayı kazandın! Kupa elinde, herkes alkışlıyor. Ama arkadaşın ağlıyor ve sen nedenini biliyorsun.",
      },
      choice_1: {
        text: "Adil yarış, ne olursa olsun",
        feedback: "İkinci oldun. Kupa onun. Ama tokalaştığınızda gözlerinin içine bakabildin.",
      },
      choice_2: {
        text: "Yarışmadan çekil",
        feedback: "Baskı çok fazlaydı. Çekildin. Rahatladın mı? Yoksa kaçtın mı?",
      },
    },
  },
  dilemma_hollow_victory: {
    text: "Aylar sonra, o yarışmada kazandığın kupaya bakıyorsun. Parlak ama... içi boş gibi hissettiriyor. Arkadaşın hâlâ seninle konuşmuyor. Vicdanın sızlıyor.",
    choices: {
      choice_0: {
        text: "İtiraf et ve kupayı iade et",
        feedback: "Herkesin önünde gerçeği söyledin. Kupa gitti, itibarın zedelendi. Ama içindeki yük kalktı. Arkadaşın şok oldu ama... belki bir gün affeder.",
      },
      choice_1: {
        text: "Sessiz kal, geçmişte kalsın",
        feedback: "Kupayı dolaba kaldırdın. Kimse bilmiyor. Ama sen biliyorsun ve bu düşünce seni her gece ziyaret ediyor.",
      },
    },
  },
  dilemma_zorbalik_populer: {
    text: "Sınıfın 'cool' grubu seni aralarına almak istiyor. Tek şart: Onlarla birlikte 'garip' çocukla dalga geçeceksin. O çocuk aslında zararsız, sadece farklı.",
    choices: {
      join_bullies: {
        text: "Gruba katıl ve dalga geç",
        feedback: "Artık 'cool' grubun bir parçasısın. Herkes seninle takılmak istiyor. Ama o çocuğun yüzündeki ifadeyi unutamıyorsun.",
      },
      reject_group: {
        text: "Reddet: 'Bu benim tarzım değil'",
        feedback: "'Ezik' dediler ve gittiler. Yalnızsın ama vicdanın rahat. O 'garip' çocuk sana teşekkür etti.",
      },
      passive_observer: {
        text: "Gruba katıl ama dalga geçme",
        feedback: "Grupta sessiz takıldın. Ne tam içlerinden oldun, ne de dışarıda. Herkes için 'o garip sessiz çocuk' oldun.",
      },
    },
  },
  dilemma_bully_karma: {
    text: "Yıllar sonra, o 'garip' çocuk şimdi başarılı bir girişimci. Sosyal medyada herkes onu övüyor. Sen ise hâlâ o 'cool' grubun gölgesindesin - ve onlar bile dağıldı. Vicdan azabı daha da ağırlaştı.",
    choices: {
      choice_0: {
        text: "Özür mesajı at",
        feedback: "Uzun bir özür yazdın. Cevap gelmedi ama en azından içini döktün. Geçmişi değiştiremezsin, sadece kabul edebilirsin.",
      },
      choice_1: {
        text: "Hiçbir şey yapma",
        feedback: "Sessiz kaldın. Başarısını izledin. İçinden 'keşke' diye geçirdin ama harekete geçmedin.",
      },
    },
  },
  dilemma_outcast_friendship: {
    text: "O gün koruduğun 'garip' çocuk artık en yakın arkadaşın. Ama 'cool' grup ona hâlâ zor zamanlar yaşatıyor. Bu sefer daha ağır: eşyalarını saklıyorlar.",
    choices: {
      choice_0: {
        text: "Müdahale et, kavga çıkar",
        feedback: "Kavga çıktı, ikisine de ceza verildi. Ama arkadaşın sana sarıldı: 'Kimse benim için böyle bir şey yapmamıştı.'",
      },
      choice_1: {
        text: "Öğretmene söyle",
        feedback: "Öğretmen müdahale etti. Zorbalık durdu ama 'ispiyoncu' damgası yine geldi. Her doğru yolun bir bedeli var.",
      },
      choice_2: {
        text: "Arkadaşına 'kendini savun' de",
        feedback: "Arkadaşın yalnız kaldığını hissetti. 'Senin de onlar gibi olduğunu düşünmüştüm' dedi. Güveniniz sarsıldı.",
      },
    },
  },
  dilemma_parti_sinav: {
    text: "Yarın büyük sınav var. Ama bu gece yılın partisi - herkes orada olacak ve hoşlandığın kişi de. İkisini aynı anda yapmak imkansız.",
    choices: {
      choice_0: {
        text: "Partiye git (YOLO!)",
        feedback: "Muhteşem bir geceydi! Dans ettin, güldün, hoşlandığın kişiyle konuştun. Ama sınav... felaket geçti.",
      },
      choice_1: {
        text: "Evde çalış (Fedakarlık)",
        feedback: "Sınav çok iyi geçti! Ama herkes partiyi konuşurken sen dışarıda kaldın. Hoşlandığın kişi başkasıyla dans etmiş.",
      },
      choice_2: {
        text: "Biraz git, erken dön",
        feedback: "İkisini de yarım yaptın. Partide 'erken giden' oldun, sınavda da 'ortalama'. Dengeyi buldun mu, yoksa ikisini de mi kaçırdın?",
      },
    },
  },
  dilemma_para_bulma: {
    text: "Okul kantininde yerde 500 TL buldun. Etrafta kimse yok. Biraz önce üst sınıftan birinin cüzdanını düşürdüğünü gördün - o kişi zengin ve kibirli.",
    choices: {
      choice_0: {
        text: "Parayı sahibine ver",
        feedback: "Parayı verdin. 'Teşekkürler' bile demedi, sadece aldı ve gitti. Hiçbir ödül yok. Ama doğru olanı yaptın... değil mi?",
      },
      choice_1: {
        text: "Parayı al (Hak etti)",
        feedback: "500 TL cebinde! Zengin kibirli tip fark etmedi bile. Ama 'hak etti' demek seni haklı çıkarır mı?",
      },
      choice_2: {
        text: "İhbar kutusuna at",
        feedback: "Okul idaresine verdin. Kim alır bilinmez ama en azından sen almadın.",
      },
    },
  },
  dilemma_freelance_etik: {
    text: "Freelance iş aldın. Müşteri, rakip firmanın verilerini 'araştırmanı' istiyor. Yasal değil ama 2000 TL teklif ediyor.",
    choices: {
      choice_0: {
        text: "Kabul et, para önemli",
        feedback: "İşi yaptın, parayı aldın. Ama şimdi bu müşteri seni 'o işler için' tanıyor. Gelecekte daha karanlık teklifler gelebilir.",
      },
      choice_1: {
        text: "Reddet, etik önemli",
        feedback: "Müşteri kızdı ve başkasını buldu. 2000 TL gitti ama uyuyabiliyorsun geceleri.",
      },
    },
  },
  dilemma_dark_job_offer: {
    text: "O eski müşteri geri döndü. Bu sefer 10.000 TL teklif ediyor - ama iş çok daha riskli. Yakalanırsan ciddi sorun.",
    choices: {
      choice_0: {
        text: "Kabul et, bu son olsun",
        feedback: "İşi yaptın ama paranoya başladı. Her gelen mesajda 'yakalandım mı' diye düşünüyorsun. Para var ama huzur yok.",
      },
      choice_1: {
        text: "Bu sefer kesinlikle hayır",
        feedback: "'Bir daha arama' dedin. Müşteri tehdit etti ama bluf yapıyor. Geçmişi geride bırakmak cesaret ister.",
      },
    },
  },
  dilemma_iki_kisi: {
    text: "İki kişi sana ilgi gösteriyor. Biri güvenilir ve sıkıcı, diğeri heyecanlı ama güvenilmez. İkisi de senden cevap bekliyor.",
    choices: {
      choice_0: {
        text: "Güvenilir olanı seç",
        feedback: "Sakin, istikrarlı bir ilişki. Ailen onaylıyor. Ama bazen 'ya diğerini seçseydim' diye düşünüyorsun.",
      },
      choice_1: {
        text: "Heyecanlı olanı seç",
        feedback: "Her gün macera! Ama bir o kadar da kavga, kıskançlık, belirsizlik. Kalbin hem dolu hem yorgun.",
      },
      choice_2: {
        text: "İkisini de reddet",
        feedback: "Yalnız kaldın ama bağımsızsın. Belki doğru kişi henüz gelmedi.",
      },
    },
  },
  dilemma_toxic_relationship: {
    text: "İlişkin yoğun ama yıpratıcı. Sürekli kavgalar, barışmalar, drama. Arkadaşların 'bu sağlıklı değil' diyor ama ayrılmak çok zor.",
    choices: {
      choice_0: {
        text: "Ayrıl, kendini koru",
        feedback: "Ayrılık acı verdi. Haftalarca ağladın. Ama zamanla, içindeki fırtına dindi. Kendine ait bir huzur buldun.",
      },
      choice_1: {
        text: "Bir şans daha ver",
        feedback: "Barıştınız. Bir hafta güzel geçti, sonra yine aynı döngü. Ne zaman çıkacaksın bu çarktan?",
      },
    },
  },
  dilemma_farkli_olmak: {
    text: "Okulda herkes belirli bir 'kalıba' uyuyor. Sen tam uymuyorsun. Değişmeli misin, yoksa kendin mi kalmalısın?",
    choices: {
      choice_0: {
        text: "Kendin ol, ne derlerse desinler",
        feedback: "Bazıları garip buldu, bazıları hayranlık duydu. Yalnız kaldın ama özgürsün. Kendi yolun senin.",
      },
      choice_1: {
        text: "Uyum sağla, hayat kolaylaşsın",
        feedback: "Herkes gibi oldun. Kabul gördün, arkadaşlar arttı. Ama aynaya baktığında, tanıdığın kişiyi görüyor musun?",
      },
      choice_2: {
        text: "Dengeyi bul: özünü koru, keskin köşeleri yumuşat",
        feedback: "Tam kendin değilsin ama tamamen de başkası değil. Pratik bir çözüm. Ama bu 'gerçek sen' mi?",
      },
    },
  },
  dilemma_kardes_firsati: {
    text: "Hayalindeki yaz kampına kabul edildin! Ama aynı hafta küçük kardeşinin ameliyatı var. Ailen 'Sen git' diyor ama kardeşin seni istiyor yanında.",
    choices: {
      choice_0: {
        text: "Kampa git (Kendi hayatın)",
        feedback: "Kamp muhteşemdi, çok şey öğrendin. Ama kardeşinin ameliyat sonrası 'Neredeydin?' sorusu hâlâ kulaklarında çınlıyor.",
      },
      choice_1: {
        text: "Kal, kardeşin yanında (Fedakarlık)",
        feedback: "Kardeşin elini tutarken gözleri doldu. 'Teşekkür ederim abi/abla' dedi. Fırsatı kaçırdın ama ailenin minneti büyük.",
      },
    },
  },
  dilemma_arkadas_vs_basari: {
    text: "Proje grubundasın. Arkadaşın hiç çalışmadı ama sen çok çalıştın. Öğretmen 'bireysel katkıları değerlendirecek' dedi. Arkadaşın senden onu kurtarmanı istiyor.",
    choices: {
      choice_0: {
        text: "Gerçeği söyle (Adalet)",
        feedback: "Öğretmene gerçeği söyledin. Yüksek not aldın, arkadaşın düşük. Artık 'arkadaş' mısınız emin değilsin.",
      },
      choice_1: {
        text: "Onu da koru (Sadakat)",
        feedback: "'Beraber yaptık' dedin. İkiniz de ortalama not aldınız. Arkadaşın minnettar ama sen hak ettiğini alamadın.",
      },
      choice_2: {
        text: "Öğretmenle özel konuş",
        feedback: "Öğretmenle gizlice konuştun. O anladı, sana daha yüksek not verdi. Arkadaşın bilmiyor ama... bu da bir tür yalan değil mi?",
      },
    },
  },
  dilemma_kayip_hayvan: {
    text: "Yolda yaralı bir köpek buldun. Veteriner pahalı ve ailen hayvan istemiyor. Ama köpeği bırakırsan muhtemelen ölecek.",
    choices: {
      choice_0: {
        text: "Veterinere götür, ne pahasına olursa olsun",
        feedback: "Biriktirdiğin tüm para gitti. Ailen kızdı. Ama köpek iyileşti ve şimdi bir barınakta yeni aile bekliyor. Değdi mi?",
      },
      choice_1: {
        text: "Bırak, doğanın işi",
        feedback: "Döndün ve baktığında köpek hâlâ oradaydı. Gözleri sana bakıyordu. O gece uyuyamadın.",
      },
      choice_2: {
        text: "Sosyal medyada paylaş, yardım iste",
        feedback: "Post viral oldu! Birisi gelip köpeği aldı ve tedavi ettirdi. Sen doğrudan yardım etmedin ama köprü oldun.",
      },
    },
  },
  dilemma_sinif_hirsizi: {
    text: "Sınıfta hırsızlık oluyor. Sen hırsızın kim olduğunu biliyorsun - fakir bir aileden gelen, zorluk çeken bir çocuk. Çaldığı şey: yemek parası.",
    choices: {
      choice_0: {
        text: "Sessiz kal (Empati)",
        feedback: "Hırsızlık devam etti, herkes birbirinden şüpheleniyor. Sınıfın ortamı bozuldu. Ama o çocuk aç kalmadı.",
      },
      choice_1: {
        text: "Öğretmene söyle (Kural)",
        feedback: "Çocuk disipline gitti. Durumu öğrenince aile yardımı aldı. Ama sen 'ispiyoncu' oldun ve çocuk sana nefret dolu bakıyor.",
      },
      choice_2: {
        text: "Çocukla konuş, yardım teklif et",
        feedback: "Gizlice konuştun ve kendi harçlığından verdin. Hırsızlık durdu. İkiniz arasında garip ama derin bir bağ oluştu.",
      },
    },
  },
  dilemma_kirik_vazo: {
    text: "Top oynarken annenin en sevdiği vazoyu kırdın. Kimse görmedi. Kardeşin odada uyuyor...",
    choices: {
      vazo_itiraf: {
        text: "İtiraf et",
        feedback: "Annen kızdı ama 'en azından dürüst oldun' dedi. Ceza hafif geçti.",
      },
      vazo_kardes: {
        text: "Kardeşini suçla",
        feedback: "Kardeşin ağladı, ceza aldı. Ama sen biliyorsun gerçeği. Her bakışında suçluluk...",
      },
      vazo_gizle: {
        text: "Parçaları sakla, kimse bilmesin",
        feedback: "Parçaları çöpe attın. Annen 'vazo nerde?' diye sordu ama bulamadı. Sır seninle...",
      },
    },
  },
  dilemma_vazo_ger_ek: {
    text: "Yıllar sonra, kardeşin hâlâ 'o vazo olayını' hatırlıyor. 'Ben kırmamıştım' diyor hep. Gerçeği söyleyecek misin?",
    choices: {
      choice_0: {
        text: "Gerçeği söyle, özür dile",
        feedback: "Kardeşin şaşırdı, sonra sarıldı. 'Biliyordum' dedi. Yük kalktı.",
      },
      choice_1: {
        text: "Sus, çok geç artık",
        feedback: "Sustun. Kardeşin yine kırgın bakışlarla gitti. Bu sır seninle gidecek.",
      },
    },
  },
  dilemma_oyuncak_calma: {
    text: "Arkadaşının evinde çok istediğin bir oyuncak var. Kimse bakmazken alabilirsin...",
    choices: {
      oyuncak_birak: {
        text: "Yerine koy, doğru olan bu",
        feedback: "Zor oldu ama koydun. Belki bir gün kendin alırsın.",
      },
      oyuncak_al: {
        text: "Cebine koy, kimse görmedi",
        feedback: "Aldın. Eve geldin. Ama artık o oyuncakla oynamak eğlenceli değil...",
      },
    },
  },
  dilemma_calinti_bulundu: {
    text: "Arkadaşın oyuncağını arıyor. Annesi herkese soruyor. Şimdi ne yapacaksın?",
    choices: {
      choice_0: {
        text: "Geri ver ve özür dile",
        feedback: "Herkes şok oldu. Ceza aldın. Ama içindeki yük kalktı.",
      },
      choice_1: {
        text: "Oyuncağı at, kimse bilmesin",
        feedback: "Çöpe attın. Kimse bulamadı. Ama o oyuncak her gece rüyana giriyor.",
      },
    },
  },
  dilemma_arkadaslik_sirri: {
    text: "Arkadaşın sana bir sır verdi: evden kaçmayı planlıyor. Anne babasına söylemeli misin?",
    choices: {
      sir_anlat: {
        text: "Ailesine söyle, güvenliği önemli",
        feedback: "Arkadaşın sana çok kızdı. 'İhanetçi!' dedi. Ama güvende. Belki bir gün anlayacak.",
      },
      sir_sakla: {
        text: "Sırrı sakla, güvenini koruma",
        feedback: "Sustun. Arkadaşın kaçtı. 3 gün sonra bulundu. Polis sorgusu... Her şey karıştı.",
      },
      sir_ikna: {
        text: "Arkadaşını vazgeçirmeye çalış",
        feedback: "Saatlerce konuştun. Sonunda vazgeçti. Kimse bilmiyor ama sen bir hayat kurtardın.",
      },
    },
  },
  dilemma_sosyal_medya_ifsa: {
    text: "Sınıftan birinin utanç verici fotoğrafı paylaşıldı. Herkes gülüyor, beğeniyor. Sen?",
    choices: {
      ifsa_savun: {
        text: "O çocuğu savun, paylaşımı şikayet et",
        feedback: "'White knight' dediler, dalga geçtiler. Ama o çocuk teşekkür etti. Belki tek sen dost oldun.",
      },
      ifsa_gul: {
        text: "Sen de beğen ve gül",
        feedback: "Grupta kaldın. Popülersin. Ama o çocuğun gözlerini hatırlıyorsun...",
      },
      ifsa_sessiz: {
        text: "Sessiz kal, karışma",
        feedback: "Beğenmedin ama savunmadın da. Ortada kaldın. Rahat mısın?",
      },
    },
  },
  dilemma_kopya_satin_alma: {
    text: "Yarınki sınavın cevap anahtarı satılıyor. 100 TL. Herkes alıyor. Sen?",
    choices: {
      kopya_al: {
        text: "Al, herkes alıyor zaten",
        feedback: "Sınav çok kolay geçti. Ama o not senin değil. Ve şimdi satıcı senden 'iyilik' bekliyor...",
      },
      kopya_alma: {
        text: "Hayır, kendi gücümle",
        feedback: "Notun düşük geldi. Herkes yüksek aldı. Ama senin notun gerçek.",
      },
    },
  },
  dilemma_santaj: {
    text: "O 'cevap anahtarı' satan kişi şimdi senden iyilik istiyor. 'Ödevimi yap, yoksa öğretmene söylerim.'",
    choices: {
      choice_0: {
        text: "Ödevini yap, risk alma",
        feedback: "Yaptın. Ama bu bitmeyecek. Şimdi kölesin.",
      },
      choice_1: {
        text: "Hayır de, ne olursa olsun",
        feedback: "'Söyle bakalım' dedin. O da söyledi. Disipline gittin. Ama özgürsün.",
      },
      choice_2: {
        text: "Ona karşı kanıt topla",
        feedback: "Mesajları sakladın. O seni ihbar ettiğinde, sen de onu ihbar ettin. İkiniz de ceza aldınız ama artık eşitsiniz.",
      },
    },
  },
  dilemma_yasli_komsu: {
    text: "Yaşlı komşunuz yardım istiyor. Ama arkadaşların bekliyor.",
    choices: {
      komsu_yardim: {
        text: "Yardım et, arkadaşlar beklesin",
        feedback: "Yarım saat geçti. Arkadaşlar kızdı ama yaşlı komşunuz size çay ısmarladı ve teşekkür etti.",
      },
      komsu_gec: {
        text: "Özür dile ve geç, acelin var",
        feedback: "Arkadaşlarla eğlendin ama eve gelince yaşlı komşunun ambulansla götürüldüğünü duydun.",
      },
    },
  },
  dilemma_cevre_kirliligi: {
    text: "Piknikte herkes çöplerini yere atıyor. Sen ne yapacaksın?",
    choices: {
      cevre_topla: {
        text: "Kendi çöpünü topla, hatta başkalarınınkini de",
        feedback: "'Temizlikçi mi oldun?' dediler. Ama doğayı korudun. Bir gün anlayacaklar.",
      },
      cevre_ayni: {
        text: "Sen de at, herkes atıyor",
        feedback: "Attın. Kolaydı. Ama geri döndüğünde o güzel yerin çöplük olduğunu gördün.",
      },
    },
  },
  dilemma_dedikodu: {
    text: "Birinin sırrını biliyorsun. Paylaşırsan herkes seni ilgi çekici bulacak.",
    choices: {
      dedikodu_paylas: {
        text: "Sırrı paylaş, ilgi çekici ol",
        feedback: "Anlattın. Herkes dinledi. Popülersin. Ama o kişi öğrenirse?",
      },
      dedikodu_sus: {
        text: "Sus, sır sırdır",
        feedback: "'Neden anlatmıyorsun?' diye sordular. 'Sır' dedin. Güvenilir biri oldun.",
      },
    },
  },
  dilemma_dedikodu_sonuc: {
    text: "O kişi sırrın yayıldığını öğrendi. Senin anlattığını biliyor. Yüz yüze geldiniz.",
    choices: {
      choice_0: {
        text: "Özür dile, kabul et",
        feedback: "'Söylememeliyim' dedin. Affetmedi ama en azından yüzleştin.",
      },
      choice_1: {
        text: "Inkar et: 'Ben söylemedim'",
        feedback: "İnkar ettin ama ikna olmadı. Şimdi hem dedikoducu hem yalancısın.",
      },
    },
  },
  dilemma_hayvan_bulma: {
    text: "Sokakta yaralı bir kedi var. Ne yapacaksın?",
    choices: {
      kedi_veteriner: {
        text: "Kumbaranı boşalt, veterinere götür",
        feedback: "Param bitti, aile kızdı. Ama kedi iyileşti ve birisi sahiplendi. Bir hayat kurtardın.",
      },
      kedi_birak: {
        text: "Üzüldüm ama yapacak bir şey yok",
        feedback: "Devam ettin. Ama o gece rüyanda o kediyi gördün.",
      },
      kedi_sosyal: {
        text: "Sosyal medyada yardım iste",
        feedback: "Post paylaştın. Birisi gelip kediyi aldı ve tedavi ettirdi. Köprü oldun.",
      },
    },
  },
  dilemma_yemek_paylasimi: {
    text: "Okulda öğle yemeği vakti. Yanındaki çocuğun yemeği yok, parasız. Sen fazla getirdin...",
    choices: {
      yemek_ver: {
        text: "Yarısını ver",
        feedback: "Sandviçin yarısını verdin. Çocuk utangaç bir teşekkür etti. Karnın az aç ama kalbin tok.",
      },
      yemek_tut: {
        text: "Annem bana hazırladı, paylaşamam",
        feedback: "Yedin. Lezzetliydi. Ama yanındaki çocuk boş oturdu.",
      },
    },
  },
  dilemma_romantic_jealousy_test: {
    text: "Sevgilin son günlerde sınıftan biriyle çok fazla vakit geçiriyor. \"Sadece arkadaşız\" diyor ama içinde bir şüphe var... Bu durumu nasıl ele alacaksın?",
    choices: {
      jealousy_confront: {
        text: "😤 Yüzleş ve hesap sor",
        feedback: "Tartıştınız. 'Güvenmiyorsan bu ilişki yürümez' dedi. Belki haklı...",
      },
      jealousy_trust: {
        text: "🤝 Güven: \"Sana inanıyorum\"",
        feedback: "Güvendiğini gösterdin. Gülümsedi ve 'Sen en iyisisin' dedi.",
      },
      jealousy_spy: {
        text: "🕵️ Gizlice araştır",
        feedback: "Takip ettin, mesajlarına baktın... Paranoya seni ele geçiriyor.",
      },
    },
  },
  dilemma_caught_spying: {
    text: "Sevgilin senin onu takip ettiğini, mesajlarına baktığını öğrendi. Yüzü kıpkırmızı: \"Sen nasıl bunu yaparsın?! Bu güvensizlik mi?!\"",
    choices: {
      spy_apologize: {
        text: "😔 Özür dile: \"Hata yaptım\"",
        feedback: "Samimi özrün kabul gördü. Ama güven zedelendi. İyileşmesi zaman alacak.",
      },
      spy_justify: {
        text: "😤 Kendini savun: \"Şüphelenecek şeyler vardı!\"",
        feedback: "'Sen hâlâ anlamamışsın' dedi ve gitti. İlişki tehlikede.",
      },
    },
  },
  dilemma_temptation: {
    text: "Sevgilin ile birlikteyken sınıftan çok çekici biri seninle flört etmeye başladı. \"Sevgilin duyarsa ne olur?\" diye düşünüyorsun ama... Bu ilgi hoşuna gidiyor.",
    choices: {
      temptation_loyal: {
        text: "❤️ Sadık kal: \"Sevgilim var\"",
        feedback: "Net bir şekilde sınır çizdin. Bazıları buna saygı duyar, bazıları ısrar eder.",
      },
      temptation_flirt_back: {
        text: "😏 Flörte karşılık ver",
        feedback: "Zararsız bir flört... değil mi? Ama içinde bir suçluluk var.",
      },
      temptation_question: {
        text: "🤔 İlişkini sorgula",
        feedback: "Belki bu bir işaret. Gerçekten mutlu musun? Düşünmelisin.",
      },
    },
  },
  dilemma_partner_finds_out: {
    text: "Sevgilin senin başkasıyla flört ettiğini duymuş. Gözleri dolu, sesi titriyor: \"Bu doğru mu? Beni aldattın mı?\"",
    choices: {
      found_out_lie: {
        text: "🙈 Yalan söyle: \"Hayır, iftira!\"",
        feedback: "'Gözlerinin içine bakıyorum... İnanmak istiyorum' dedi. Ama şüphe kaldı.",
      },
      found_out_confess: {
        text: "😔 İtiraf et: \"Evet, hata yaptım\"",
        feedback: "Dürüst oldun ama kalpleri kırdın. İlişki belki bitti, belki kırık devam edecek.",
      },
      found_out_breakup: {
        text: "💔 Bitir: \"Belki de bu ilişki bitmeli\"",
        feedback: "Karmaşık durumu bitirerek temizledin. Acı verici ama belki doğru karar.",
      },
    },
  },
  dilemma_loyalty_test: {
    text: "Hoşlandığın kişi sana yaklaştı: \"Ben senden hoşlanıyorum, Sevgilin'den ayrılsan olmaz mı?\" diyor. Kalbin ikiye bölündü. Ne yapacaksın?",
    choices: {
      loyalty_stay: {
        text: "❤️ Sevgiline sadık kal",
        feedback: "'Hayır, ben mutluyum' dedin. İçinde bir şüphe kaldı mı?",
      },
      loyalty_breakup: {
        text: "💔 Sevgilinden ayrıl",
        feedback: "Ayrılık acı verdi. Yeni bir başlangıç ama geçmiş acıtıyor.",
      },
      loyalty_indecisive: {
        text: "😔 \"Karar veremiyorum\" de",
        feedback: "Kararsızlık her iki tarafı da kırdı. Arada kaldın.",
      },
    },
  },
  dilemma_best_friend_vs_partner: {
    text: "En iyi arkadaşın ve Sevgilin birbirinden hoşlanmıyor. En iyi arkadaşın: \"O ya da ben! Seç.\" Sevgilin: \"O arkadaşınla görüşme.\" İkisi de ultimatom veriyor.",
    choices: {
      bff_vs_partner_bff: {
        text: "💎 En iyi arkadaşını seç",
        feedback: "Yıllardır süren dostluğu seçtin. Ama aşk... acıttı.",
      },
      bff_vs_partner_partner: {
        text: "❤️ Sevgilini seç",
        feedback: "Aşkı seçtin. En iyi arkadaşın çok kırıldı, belki bir daha konuşmazsınız.",
      },
      bff_vs_partner_neither: {
        text: "🚶 İkisini de reddet",
        feedback: "'Ultimatom veren kimseyle olmam' dedin. Cesur ama yalnız kaldın.",
      },
    },
  },
  pers_sosyal_davet: {
    text: "Sınıftan biri doğum günü partisine çağırdı. Gitmek mi, kalmak mı? İkisi de makul görünüyor.",
    choices: {
      go_party_introvert: {
        text: "Git ve sosyalleş (Zor seçim)",
        feedback: "Gittin. Her dakika işkence gibiydi. Köşede durdun, zoraki gülümsedin. Ama... birkaç kişiyle konuştun. Belki o kadar da kötü değildi?",
      },
      stay_home_introvert: {
        text: "Evde kal (Rahat seçim)",
        feedback: "Bahaneler uydurdun ve evde kaldın. Rahat bir gece geçirdin. Ama partinin fotoğraflarını görünce içinde bir şey sızladı.",
      },
      go_party_extrovert: {
        text: "Tabii ki git!",
        feedback: "Parti muhteşemdi! Herkes seninle konuşmak istiyordu. Yeni numaralar aldın, dans ettin, gece yarısına kadar kaldın!",
      },
      skip_party_extrovert: {
        text: "Bu sefer pas (Zor seçim)",
        feedback: "Evde kaldın. Sürekli telefonu kontrol ettin. Herkes eğlenirken sen... bekledin. Ama yarınki sınava hazırsın.",
      },
      go_party_neutral: {
        text: "Gideyim biraz",
        feedback: "Gidip bir saat takıldın, sonra sessizce ayrıldın. Ne çok sosyalleştin ne de kaçırdın. Dengeli bir gece.",
      },
    },
  },
  pers_sinif_sunumu: {
    text: "Öğretmen 'Yarın herkes sunum yapacak' dedi. Biraz gerginsin ama hazırlanırsan iyi geçer.",
    choices: {
      presentation_brave: {
        text: "Hazırlan ve parlak bir sunum yap",
        feedback: "Ellerin titredi, sesin kısıldı başta. Ama devam ettin. Bitirdiğinde alkış koptu. 'Vay, konuşabiliyor!' diye fısıldaştılar.",
      },
      presentation_avoid: {
        text: "Hasta numarası yap / Kaç",
        feedback: "O gün okula gitmedin. Karnın ağrıyordu (gerçekten mi?). Sunum ertelendi ama... bir gün yine yapman gerekecek.",
      },
      presentation_extrovert: {
        text: "Sahneye çık ve gösterisini yap!",
        feedback: "Muhteşem bir performans! Espri yaptın, sınıf güldü, öğretmen etkilendi. Bu senin doğal ortamın.",
      },
    },
  },
  pers_tehlikeli_teklif: {
    text: "Arkadaşların inşaat halindeki binaya girmeyi planlıyor. Riskli ama heyecanlı görünüyor.",
    choices: {
      risk_take_cautious: {
        text: "Git, korkunu yen (Zor seçim)",
        feedback: "Her adımda 'geri dön' diye düşündün. Ama çatıya çıktığında... şehir ayaklarının altındaydı. Korkutucu ama muhteşem.",
      },
      risk_refuse_cautious: {
        text: "Hayır de, mantıklı ol (Rahat seçim)",
        feedback: "'Korkak' dediler ve gittiler. Evde kaldın. Güvendesin. Ama... her zaman güvende olmak yaşamak mı?",
      },
      risk_take_brave: {
        text: "Hadi gidelim!",
        feedback: "Efsane bir macera! Çatıda selfie çektiniz, hikayeler anlattınız. Hayat böyle yaşanır!",
      },
      risk_refuse_brave: {
        text: "Bu sefer pas (Zor seçim)",
        feedback: "İçinden 'git' diye bağırıyordu ama dururdun kendini. Bazen en cesur şey hayır demektir.",
      },
    },
  },
  pers_risk_consequence: {
    text: "Hatırlıyor musun o inşaata girdiğinizi? Birisi polise şikayet etmiş. İsimler soruluyor.",
    choices: {
      choice_0: {
        text: "İtiraf et",
        feedback: "Doğruyu söyledin. Ceza aldın ama vicdan rahat.",
      },
      choice_1: {
        text: "İnkar et",
        feedback: "Kimse kanıtlayamadı. Ama şimdi bir sır taşıyorsun.",
      },
    },
  },
  pers_dilenci_cocuk: {
    text: "Sokakta küçük bir çocuk mendil satıyor. Durmalı mısın, geçmeli misin?",
    choices: {
      help_empath: {
        text: "Dur ve yardım et",
        feedback: "Mendil aldın, biraz sohbet ettin. Çocuk gülümsedi. Belki dünyayı değiştirmedin ama bir anı güzelleştirdin.",
      },
      ignore_empath: {
        text: "Geç, yapabileceğin bir şey yok (Zor seçim)",
        feedback: "Geçtin ama o gözler aklından çıkmadı. Gece yatakta düşündün. Neden durmsadın?",
      },
      ignore_selfish: {
        text: "Geç, senin sorunun değil",
        feedback: "Devam ettin. Herkesin kendi derdi var. Sen de kendi hayatını yaşıyorsun.",
      },
      help_selfish: {
        text: "Dur ve bir şey al (Zor seçim)",
        feedback: "Neden durduğunu bilmiyorsun. Mendil aldın. Çocuk 'teşekkürler' dedi. Garip bir sıcaklık hissettin.",
      },
    },
  },
  pers_kopya_verme: {
    text: "Sınavda yanındaki kopya çekmek istiyor. Ne yapacaksın?",
    choices: {
      give_copy_empath: {
        text: "Kağıdını aç, görsün",
        feedback: "Kağıdını hafifçe açtın. Gördü ve rahatladı. Öğretmen fark etmedi... bu sefer. Ama ya yakalansaydın?",
      },
      refuse_copy_empath: {
        text: "Hayır, risk alamam (Zor seçim)",
        feedback: "Başını çevirdin. Yanındaki hayal kırıklığı içinde kaldı. Doğru olanı yaptın ama... içinde bir şey acıdı.",
      },
      refuse_copy_selfish: {
        text: "Kendi kağıdını kapa",
        feedback: "Kağıdını kapattın ve konsantre oldun. Senin emeğin, senin notun. Adil olan bu.",
      },
      give_copy_selfish: {
        text: "Tamam ama borcunu öde (Zor seçim)",
        feedback: "Fısıldadın: 'Teneffüste 20 lira.' Kabul etti. İş tamam. Yardım ettin... bir nevi.",
      },
    },
  },
  pers_uzun_kuyruk: {
    text: "Kantinde uzun bir kuyruk var. Beklemeli misin?",
    choices: {
      wait_impatient: {
        text: "Dişini sık ve bekle (Zor seçim)",
        feedback: "Her dakika bir saat gibi geçti. Ama bekledin. Sonunda sıra sana geldi. Belki sabır öğrenilebilir bir şeydir.",
      },
      skip_impatient: {
        text: "Sıradan atla / Vazgeç",
        feedback: "Ya araya girdin ya da vazgeçtin. Zaman kazandın ama birinin hakkını yedin veya aç kaldın.",
      },
      wait_patient: {
        text: "Bekle, acele yok",
        feedback: "Beklerken müzik dinledin, etrafı izledin. Zaman geçti, sıra geldi. Hayat bu kadar basit.",
      },
    },
  },
  pers_ani_firsat: {
    text: "Arkadaşın 'Şu an gel, konser bedava!' diye aradı. Yarın sınavın var. Ne yapacaksın?",
    choices: {
      go_concert_impulsive: {
        text: "Hemen git! YOLO!",
        feedback: "Konser muhteşemdi! Sabaha kadar eğlendin. Ama sınav... felaket geçti. Değdi mi? O an değdi.",
      },
      study_impulsive: {
        text: "Hayır, sınav önemli (Zor seçim)",
        feedback: "Telefonu kapattın ve ders çalıştın. Konser fotoğraflarını görünce içinde bir şey koptu. Ama not yüksek geldi.",
      },
      study_patient: {
        text: "Sınav önce",
        feedback: "Rahat bir kararla ders çalıştın. Konser kaçtı ama gelecekte daha çok fırsat olacak.",
      },
    },
  },
  pers_kural_ihlali: {
    text: "Öğretmen sınıfı terk etti. Herkes gürültü yapıyor. Ne yapacaksın?",
    choices: {
      enforce_rules_conformist: {
        text: "Sessiz ol de, kural kural",
        feedback: "Herkes sana ters ters baktı. 'Müdür müsün sen?' dediler. Ama içinden kurallar senin için önemli.",
      },
      join_chaos_conformist: {
        text: "Bu sefer katıl (Zor seçim)",
        feedback: "Katıldın ve... eğlendin. Kuralları çiğnemek bu kadar iyi hissettirmemeli değil mi? Kafan karışık.",
      },
      join_chaos_rebel: {
        text: "Partiye katıl!",
        feedback: "En çok gürültüyü sen yaptın. Özgürlük güzel! Kurallar... kim takar?",
      },
      stay_quiet_rebel: {
        text: "Kendi halinde kal (Zor seçim)",
        feedback: "Katılmadın. Neden bilmiyorsun. Belki her isyana katılmak zorunda değilsin.",
      },
    },
  },
  pers_breakdown_warning: {
    text: "Son günlerde çok yıprandın. Uyuyamıyorsun, iştahın yok. Her şeye sinirleniyorsun. Bir şeyler değişmeli.",
    choices: {
      choice_0: {
        text: "Yardım iste (aile/arkadaş)",
        feedback: "Birine açıldın. Ağladın belki. Ama yük hafiflerdi. Yalnız değilsin.",
      },
      choice_1: {
        text: "Kendi başına çöz",
        feedback: "Dişini sıktın ve devam ettin. Güçlü görünüyorsun ama... ne kadar daha dayanabilirsin?",
      },
    },
  },
  pers_breakdown_crisis: {
    text: "PATLAMA. Bugün her şey çok fazla oldu. Gözyaşları, öfke, çaresizlik... Vücudun 'dur' diyor.",
    choices: {
      choice_0: {
        text: "Tamamen çök ve dinlen",
        feedback: "Bir gün hiçbir şey yapmadın. Sadece yattın. Dünya dönmeye devam etti. Belki her şey o kadar acil değildi.",
      },
      choice_1: {
        text: "Zorla devam et",
        feedback: "Devam ettin. Robotik hareketlerle. Kimse fark etmedi. Ama içindeki bir şey kırıldı.",
      },
    },
  },
  pers_hospital_stress: {
    text: "Sabah kalkmaya çalıştın ama vücudun izin vermedi. Ailenne haber verdin. Hastanedesin. Doktor 'aşırı stres ve tükenmişlik' diyor.",
    choices: {
      choice_0: {
        text: "Tedaviyi kabul et",
        feedback: "Birkaç gün hastanede kaldın. İlaçlar, terapi, dinlenme. Yavaş yavaş toparlanıyorsun. Hayat hız değilmiş.",
      },
    },
  },
  pers_personality_reflection: {
    text: "17. yaş günün. Aynaya bakıyorsun. Artık dengeli biri olduğunu biliyorsun. Peki bu sen olmak istemiştin mi?",
    choices: {
      choice_0: {
        text: "Evet, bu benim",
        feedback: "Kendini kabul etmek güçtür. Kim olduğunu biliyorsun. Bu özgürlük.",
      },
      choice_1: {
        text: "Hayır, değişmek istiyorum",
        feedback: "Değişim zordur, özellikle 17 yaşında. Ama ilk adım farkındalıktır. Şimdi ne yapacaksın?",
      },
    },
  },
  pers_yeni_sinif: {
    text: "Yeni bir sınıfa/okula geçtin. Kimseyi tanımıyorsun. Nasıl yaklaşacaksın?",
    choices: {
      sinif_cesur: {
        text: "İlk sen merhaba de",
        feedback: "Kalbin güm güm attı ama 'Merhaba, ben yeni öğrenci' dedin. Birkaç kişi seninle konuştu!",
      },
      sinif_bekle: {
        text: "Birinin sana gelmesini bekle",
        feedback: "Köşede bekledin. Kimse gelmedi. Yalnız bir başlangıç.",
      },
      sinif_sosyal: {
        text: "Herkesle tanış!",
        feedback: "Gün sonunda herkesin adını öğrendin. Popüler yeni öğrenci sensin!",
      },
    },
  },
  pers_oyun_kaybetme: {
    text: "Oyun oynuyorsun ve kaybettin. Ne yapacaksın?",
    choices: {
      oyun_sakin: {
        text: "Sakin ol, tebrik et",
        feedback: "'Tebrikler' dedin, elini sıktın. İçinden patlıyordun ama centilmen oldun.",
      },
      oyun_sinirlen: {
        text: "Sinirlen ve oyunu devir",
        feedback: "Masayı devirdin! Herkes sana baktı. Kötü kaybedensin.",
      },
      oyun_yeniden: {
        text: "Bir daha oynayalım!",
        feedback: "Hemen yeniden başladın. Azimlisin!",
      },
    },
  },
  pers_kardesle_paylasim: {
    text: "Son dondurma kalmış. Kardeşin de istiyor. Ne yapacaksın?",
    choices: {
      dondurma_paylas: {
        text: "Paylaş",
        feedback: "Yarısını verdin. Kardeşin mutlu oldu. Sen de... biraz?",
      },
      dondurma_al: {
        text: "Hepsini ye, önce gören alır",
        feedback: "Hepsini yedin. Kardeşin ağladı. Annen seni azarladı ama dondurma güzeldi.",
      },
      dondurma_ver: {
        text: "Ona ver",
        feedback: "'Al sen ye' dedin. Kardeşin sana sarıldı. Fedakarlık.",
      },
    },
  },
  pers_korkunc_film: {
    text: "Arkadaşların korku filmi izliyor. Katılacak mısın?",
    choices: {
      film_izle: {
        text: "İzle, korkma",
        feedback: "Tüm film boyunca gözlerini kapattın ama dayanın! Arkadaşların 'cesursun' dedi.",
      },
      film_red: {
        text: "Korku filmi izlemem",
        feedback: "Eve gittin. Rahat uyudun. Arkadaşların sonra filmi anlattı.",
      },
      film_heyecan: {
        text: "Tabii, korkulu filmler harika!",
        feedback: "Filmi çok sevdin! En çok bağıran sendin ama eğlencesine!",
      },
    },
  },
  pers_sira_beklemek: {
    text: "Birisi sırana atladı. Ne yapacaksın?",
    choices: {
      sira_itiraz: {
        text: "İtiraz et, sıra var",
        feedback: "'Sıra var!' dedin. Kişi utanarak geri gitti. Adalet!",
      },
      sira_kabul: {
        text: "Ses çıkarma",
        feedback: "Bir şey demedin. Bekledin. Biraz sinirlisin ama kavga olmadı.",
      },
      sira_atla: {
        text: "Sen de birine atla",
        feedback: "Misliyle ödedin! Başka birine sen de atladın. Kaos!",
      },
    },
  },
  pers_yaratici_proje: {
    text: "Serbest proje ödevi var. Farklı ve cesur bir fikrin var ama riskli. Klasik yapsan güvenli not alırsın.",
    choices: {
      proje_cesur: {
        text: "Risklı ama yaratıcı olanı yap",
        feedback: "Projen çok farklıydı! Öğretmen şaşırdı. Alkış aldın!",
      },
      proje_guvenli: {
        text: "Klasik ve güvenli olanı yap",
        feedback: "Standart bir proje yaptın. Güvenli not aldın. Ama kimse hatırlamayacak.",
      },
    },
  },
  pers_yalanla_kurtulma: {
    text: "Bir hata yaptın. Yalan söylersen kurtulursun. Ne yapacaksın?",
    choices: {
      yalan_do_ru: {
        text: "Doğruyu söyle",
        feedback: "Doğruyu söyledin. Ceza aldın ama herkes sana güveniyor artık.",
      },
      yalan_yalan: {
        text: "Yalan söyle ve kurtur",
        feedback: "Yalan işe yaradı. Ama vicdanın rahat değil...",
      },
    },
  },
  pers_uyku_saati: {
    text: "Uyku saatin geldi. Yatacak mısın?",
    choices: {
      uyku_yat: {
        text: "Tamam anne, iyi geceler",
        feedback: "Uslu çocuk! Ertesi gün dinlenmiş kalktın.",
      },
      uyku_kac: {
        text: "Gizlice oyun oyna",
        feedback: "Battaniyenin altında telefon oynadın. Annene yakalandın!",
      },
      uyku_pazarlik: {
        text: "10 dakika daha?",
        feedback: "'5 dakika' dedi annen. Kabul!",
      },
    },
  },
  pers_grup_zorbaligi: {
    text: "Popüler çocuklar bir öğrenciyle dalga geçiyor. Ne yapacaksın?",
    choices: {
      zorbalik_mudahale: {
        text: "Araya gir, onu savun",
        feedback: "'Bırakın onu!' dedin. Sana da laf attılar. Ama o çocuk teşekkür etti. Değdi.",
      },
      zorbalik_izle: {
        text: "Bir şey yapma, riskli",
        feedback: "İzledin. O çocuğun gözleri seni buldu. Yardım etmedin...",
      },
      zorbalik_ogretmen: {
        text: "Öğretmene söyle",
        feedback: "Öğretmen geldi, zorbalık durdu. Ama 'ispiyoncu' damgası mı yedin?",
      },
    },
  },
  pers_bir_gun_yalniz: {
    text: "Bugün kimseyle konuşmadın. Nasıl hissediyorsun?",
    choices: {
      yalniz_rahat: {
        text: "Yalnızlık benim için iyi",
        feedback: "Kendinle baş başa kaldın. Düşündün, dinlendin. Huzur.",
      },
      yalniz_zor: {
        text: "Çok zor, insanları özledim",
        feedback: "Sosyal medyayı açıp herkesin eğlendiğini gördün. FOMO acı veriyor.",
      },
      yalniz_mesaj: {
        text: "Birine mesaj at",
        feedback: "'Naber?' yazdın. Sohbet başladı. Belki yalnızlık o kadar da iyi değildir.",
      },
    },
  },
  pers_balanced_mediator: {
    text: "İki arkadaşın kavga ediyor. İkisini de anlıyorsun çünkü her iki tarafı da görebiliyorsun.",
    choices: {
      balanced_mediate: {
        text: "Arabulucu ol",
        feedback: "Her iki tarafı da dinledin ve barıştırdın. Dengeli bakış açın çok işe yaradı!",
      },
      balanced_stay_out: {
        text: "Karışma",
        feedback: "Karışmadın. Bazen en iyi hamle hiçbir şey yapmamaktır.",
      },
    },
  },
  pers_balanced_allrounder: {
    text: "Okulda proje yarışması var. Hangi alanda yarışacağını seçmelisin ama sen her konuda idare ediyorsun...",
    choices: {
      allrounder_combine: {
        text: "Disiplinler arası proje yap",
        feedback: "Farklı alanları birleştiren projen jüriyi etkiledi! Çok yönlülüğün senin gücün.",
      },
      allrounder_pick_one: {
        text: "Tek bir alan seç",
        feedback: "Bir alana odaklandın. Güvenli bir seçim.",
      },
    },
  },
  pers_balanced_leadership: {
    text: "Grup projesinde kimse liderlik yapmak istemiyor. Sen ne yaparsın?",
    choices: {
      balanced_lead: {
        text: "Sessizce organize et",
        feedback: "Dikkat çekmeden grubu organize ettin. Sessiz liderliğin çok etkili oldu.",
      },
      balanced_delegate: {
        text: "Görev dağıt ve çekil",
        feedback: "Herkes payını aldı. Pratik bir çözüm.",
      },
    },
  },
  pers_balanced_moral_gray: {
    text: "Arkadaşın kopya çekiyor. Sana da kopya veriyor. Ne doğru ne yanlış tam olarak net değil...",
    choices: {
      gray_refuse_quietly: {
        text: "Sessizce reddet",
        feedback: "Kopya almadın ama arkadaşını da ele vermedin. Kendi yolunu çizdin.",
      },
      gray_accept: {
        text: "Kopyayı al",
        feedback: "Kopyayı aldın. Not iyi geldi ama vicdanın biraz sızladı.",
      },
    },
  },
  pers_balanced_hobby_dilemma: {
    text: "Birden fazla hobinle ilgileniyorsun ama hepsine zaman yetmiyor. Uzmanlaşmalı mısın yoksa çeşitlilik mi korumalısın?",
    choices: {
      hobby_diversify: {
        text: "Çeşitliliği koru",
        feedback: "Her şeyden biraz bilmek de bir yetenek! Çok yönlülüğünü korudun.",
      },
      hobby_specialize: {
        text: "Bir tanesine odaklan",
        feedback: "Bir alana odaklandın. Derinleşmek seni uzman yapıyor.",
      },
    },
  },
  pers_balanced_family_talk: {
    text: "Ailen sana \"Sen ne düşünüyorsun?\" diye soruyor. Genelde herkesin fikrini dinlersin...",
    choices: {
      family_honest_opinion: {
        text: "Düşünceni açıkça söyle",
        feedback: "Ailenle açıkça konuştun. Seni dinlediler ve fikrini ciddiye aldılar.",
      },
      family_agree: {
        text: "\"Siz bilirsiniz\" de",
        feedback: "Ailenin kararına uyum sağladın.",
      },
    },
  },
  pers_balanced_new_student: {
    text: "Sınıfa yeni bir öğrenci geldi. Kimse yanına gitmedi. Sen hem merak ediyorsun hem de rahatın yerinde...",
    choices: {
      new_student_approach: {
        text: "Yanına git",
        feedback: "Yeni öğrenciyle tanıştın. Çok sevindi! Yeni bir arkadaşlık başlayabilir.",
      },
      new_student_wait: {
        text: "Bekle, o sana gelsin",
        feedback: "Bekledin. Belki yarın konuşursunuz.",
      },
    },
  },
  pers_balanced_competition: {
    text: "Bir yarışmada ikincilik kazandın. Birinciye çok yakındın. Nasıl hissediyorsun?",
    choices: {
      competition_motivated: {
        text: "Daha çok çalış",
        feedback: "İkincilik seni motive etti. Gelecek sefer birinci olacaksın!",
      },
      competition_content: {
        text: "İkincilikle mutlu ol",
        feedback: "İkincilik de güzel bir başarı. Kendini yorma, zaten iyisin.",
      },
    },
  },
  pers_balanced_career_talk: {
    text: "Herkes ne olmak istediğini biliyor gibi görünüyor. Sen ise birçok şeyle ilgileniyorsun...",
    choices: {
      career_explore: {
        text: "Tüm seçenekleri araştır",
        feedback: "Her alanı araştırdın. Çok yönlülüğün bir avantaj — doğru zamanı bekle.",
      },
      career_follow_crowd: {
        text: "Popüler olanı seç",
        feedback: "Herkesin gittiği yolu seçtin. Güvenli ama sıkıcı olabilir.",
      },
    },
  },
  pers_balanced_conflict_resolve: {
    text: "Bir tartışmada hem haklı hem haksız olduğunu hissediyorsun. İki bakış açısını da görüyorsun.",
    choices: {
      conflict_compromise: {
        text: "Uzlaşma yolu bul",
        feedback: "Her iki tarafı da dinledin ve orta yolu buldun. Diplomatik yeteneklerin parlıyor.",
      },
      conflict_withdraw: {
        text: "Tartışmadan çekil",
        feedback: "Çekildin. Bazen en akıllıca hamle savaşmamaktır.",
      },
    },
  },
  npc_new_classmate: {
    text: "Sınıfa yeni bir kız öğrenci geldi. Öğretmen onu senin yanına oturmasını söyledi. İlk izlenimin nasıl olacak?",
    choices: {
      choice_0: {
        text: "🤝 Kendimi tanıt ve sohbet et",
        feedback: "Yeni arkadaşınla güzel bir sohbet yaptın. İlk adım atılmış oldu!",
      },
      choice_1: {
        text: "📖 Sessizce ders çalışmaya devam et",
        feedback: "Derslere odaklandın ama yeni arkadaşlık fırsatını kaçırdın.",
      },
      choice_2: {
        text: "🙄 Onu görmezden gel",
        feedback: "Kendi işine baktın. Belki sonra konuşursunuz.",
      },
    },
  },
  npc_homework_together: {
    text: "Birisi senden ödev konusunda yardım istedi. \"Şu matematik konusunu hiç anlamadım, bana anlatır mısın?\"",
    choices: {
      choice_0: {
        text: "📚 Sabırla anlat",
        feedback: "Konuyu anlatırken sen de daha iyi öğrendin. Arkadaşın minnettar!",
      },
      choice_1: {
        text: "📝 Ödevini direkt yap",
        feedback: "Ödevini yaptın ama bu onu tembelliğe alıştıracak...",
      },
      choice_2: {
        text: "🙅 \"Vaktim yok\" de",
        feedback: "Kendi çalışmana odaklandın ama arkadaşın bozuldu.",
      },
    },
  },
  npc_friend_fight: {
    text: "Birisi ile tartıştınız. Sebebi bile tam hatırlamıyorsun ama ikisini de çok kırdınız. Şimdi ne yapacaksın?",
    choices: {
      choice_0: {
        text: "🤝 Özür dile ve barış",
        feedback: "Özür dilemek zor ama arkadaşlığınız daha da güçlendi.",
      },
      choice_1: {
        text: "⏰ Biraz zaman ver",
        feedback: "Bazen mesafe yakınlaştırır. Birkaç gün sonra her şey normale döndü.",
      },
      choice_2: {
        text: "😤 Haklı olduğunda ısrar et",
        feedback: "Haklı olabilirsin ama inatçılık arkadaşlığa zarar verdi.",
      },
    },
  },
  npc_secret_sharing: {
    text: "Birisi sana bir sır verdi: \"Bunu sadece sana söylüyorum, kimseye söyleme...\" Başkalarına mı anlatacaksın?",
    choices: {
      choice_0: {
        text: "🤐 Sırrını sakla",
        feedback: "Güvenilir bir arkadaş olduğunu kanıtladın.",
      },
      choice_1: {
        text: "👥 Sadece bir kişiye söyle",
        feedback: "Bir kişi... iki kişi... ve sır yayıldı. Arkadaşın öğrenirse çok kırılacak.",
      },
      choice_2: {
        text: "📢 Herkese anlat",
        feedback: "Dedikodu kralı/kraliçesi oldun ama güven kalmadı.",
      },
    },
  },
  npc_secret_revealed: {
    text: "Birisi senin sırrını yaydığını öğrendi. Yüzüne bile bakmıyor. \"Sana nasıl güvendim?\" diye bağırdı.",
    choices: {
      choice_0: {
        text: "😢 Özür dile ve açıkla",
        feedback: "Özür diledin ama güven bir kere kırıldı. Zaman gerekecek.",
      },
      choice_1: {
        text: "🤷 \"Abartma\" de",
        feedback: "Umursamaz tavrın arkadaşlığı bitirdi.",
      },
    },
  },
  npc_group_formation: {
    text: "Birkaç arkadaşın bir araya gelip takılmak istiyor. \"Sürekli beraber takılsak mı? Bir grup kuralım!\" diyorlar.",
    choices: {
      choice_0: {
        text: "🎉 Harika fikir! Gruba katıl",
        feedback: "Artık bir grubun var! Teneffüslerde hep berabersiniz.",
      },
      choice_1: {
        text: "🤔 Düşüneyim biraz",
        feedback: "Acele etmedin ama belki fırsatı kaçırırsın.",
      },
      choice_2: {
        text: "🚶 Ben yalnız takılmayı tercih ederim",
        feedback: "Bağımsız kalmayı seçtin. Bazıları bunu garip buldu.",
      },
    },
  },
  npc_first_crush: {
    text: "Sınıfta birini fark etmeye başladın. Ayşe... O güldüğünde kalbinin hızlandığını hissediyorsun. Acaba o da farkında mı?",
    choices: {
      choice_0: {
        text: "💕 Yaklaşmaya çalış",
        feedback: "Yanına yaklaştın ve \"Merhaba\" dedin. Kalbin çarpar ama gülümsedi!",
      },
      choice_1: {
        text: "👀 Uzaktan izle",
        feedback: "Sadece bakakaldın... Cesaret edemedim ama belki başka zaman.",
      },
      choice_2: {
        text: "📝 Mektup yaz",
        feedback: "Duygularını kağıda döktün. Vermeye cesaret edecek misin?",
      },
    },
  },
  npc_love_letter_decision: {
    text: "Yazdığın mektup çantanda bekliyor. Vermeye cesaret edecek misin yoksa yırtacak mısın?",
    choices: {
      choice_0: {
        text: "💌 Mektubu ver",
        feedback: "Eli titreyerek mektubu uzattın. Gülümsedi ve \"Teşekkürler, okuyacağım\" dedi!",
      },
      choice_1: {
        text: "🗑️ Mektubu yırt",
        feedback: "Mektubu yırttın. Bazı şeyler söylenmeden kalır...",
      },
    },
  },
  npc_confession: {
    text: "Hoşlandığın kişi teneffüste seni köşeye çekti. \"Seninle konuşmam lazım...\" diyor.",
    choices: {
      choice_0: {
        text: "❤️ \"Ben de senden hoşlanıyorum\"",
        feedback: "İkiniz de gülümsüyorsunuz. Artık bir çiftsiniz!",
      },
      choice_1: {
        text: "😰 \"Bilmiyorum, düşünmem lazım\"",
        feedback: "Panikledin ve kaçtın. Ama bu konuşma bitmedi...",
      },
      choice_2: {
        text: "💔 \"Sadece arkadaş olarak görüyorum\"",
        feedback: "Dürüst oldun ama karşı taraf çok üzüldü.",
      },
    },
  },
  npc_jealousy: {
    text: "Sevgilin başka biriyle çok fazla vakit geçiriyor. Seni kıskandırıyor mu?",
    choices: {
      choice_0: {
        text: "😤 Kavga çıkar",
        feedback: "Kıskançlık kontrolden çıktı. Büyük bir tartışma yaşandı.",
      },
      choice_1: {
        text: "🗣️ Sakin bir şekilde konuş",
        feedback: "Duygularını ifade ettin ve karşı taraf anlayış gösterdi.",
      },
      choice_2: {
        text: "🙈 Görmezden gel",
        feedback: "Hiç tepki vermedin ama içinde bu kalacak mı?",
      },
    },
  },
  npc_breakup: {
    text: "Sevgilin seninle konuşmak istiyor. Yüzünden bir şeylerin yanlış gittiği belli...",
    choices: {
      choice_0: {
        text: "😢 Kabul et ve ağla",
        feedback: "Ayrılık acı verdi ama bazen yollar ayrılır.",
      },
      choice_1: {
        text: "😠 \"Ben de zaten bitirmek istiyordum\"",
        feedback: "Gurur yapmak işe yaramaz ama en azından daha az incinmiş görünüyorsun.",
      },
      choice_2: {
        text: "🙏 \"Bir şans daha ver\"",
        feedback: "Yalvardın ama karar verilmişti. Belki ileride yeniden...",
      },
    },
  },
  npc_bully_encounter: {
    text: "Okulda bir grup seni köşeye sıkıştırdı. \"Cep harçlığını ver yoksa...\" diye tehdit ediyorlar.",
    choices: {
      choice_0: {
        text: "💪 Karşı koy",
        feedback: "Dayak yedin ama pes etmedin. Bir daha rahat rahat yaklaşamayacaklar.",
      },
      choice_1: {
        text: "💰 Parayı ver",
        feedback: "Parayı verdin ve gittiler. Ama bu bitmedi...",
      },
      choice_2: {
        text: "🏃 Kaç",
        feedback: "Hızlıca kaçtın! Ama okulda yüzün kalmadı.",
      },
      choice_3: {
        text: "👨‍🏫 Öğretmene söyle",
        feedback: "Öğretmen zorbaları cezalandırdı ama \"gammazlama\" damgası yedin.",
      },
    },
  },
  npc_bully_return: {
    text: "Zorbalar yine karşına çıktı. \"Daha fazla para istiyoruz!\" diyorlar. Bu sefer daha agresifler.",
    choices: {
      choice_0: {
        text: "💪 Bu sefer direniş!",
        feedback: "Sert bir kavga çıktı. Yaralandın ama onlar da. Bir daha gelmeyecekler.",
      },
      choice_1: {
        text: "👨‍👩‍👧 Ailene anlat",
        feedback: "Ailen okula geldi ve ciddi bir toplantı yapıldı. Zorbalar uzaklaştırıldı.",
      },
    },
  },
  npc_rivalry_start: {
    text: "Sınıfta biriyle sürekli bir rekabet var. Her sınavda, her konuda karşı karşıya geliyorsunuz.",
    choices: {
      choice_0: {
        text: "🏆 Rekabeti kabul et",
        feedback: "Rekabet seni daha çok çalışmaya itti. Belki bu iyi bir şey.",
      },
      choice_1: {
        text: "🤝 Barış teklif et",
        feedback: "Rekabet yerine arkadaşlık. Birlikte daha güçlüsünüz!",
      },
      choice_2: {
        text: "🔥 Onu ezmeye çalış",
        feedback: "Düşmanlık büyüdü. Bu savaş bitmedi.",
      },
    },
  },
  npc_gossip_about_you: {
    text: "Arkandan kötü söz söylendiğini duydun. Birisi hakkında dedikodu yayıyor.",
    choices: {
      choice_0: {
        text: "😡 Yüzleş",
        feedback: "Karşısına geçip \"Neden böyle yapıyorsun?\" dedin. Suratı kızardı.",
      },
      choice_1: {
        text: "🙄 Aldırma",
        feedback: "Dedikodu kendiliğinden söner. Tepki vermemek en iyisi.",
      },
      choice_2: {
        text: "🗣️ Sen de onun hakkında dedikodu yay",
        feedback: "Misliyle karşılık verdin. Ama şimdi ikisini de güvenilmez buluyorlar.",
      },
    },
  },
  npc_group_exclusion: {
    text: "Teneffüste grubun seni çağırmadan bir yere gitti. Dışlandığını hissediyorsun. Acaba bir şey mi oldu?",
    choices: {
      choice_0: {
        text: "😢 Üzül ve evde ağla",
        feedback: "Evde yalnız ağladın. Belki yarın her şey normale döner.",
      },
      choice_1: {
        text: "🗣️ Arkadaşlarına sor",
        feedback: "\"Yanlış anladın, acele ettik\" dediler. Belki haklılar...",
      },
      choice_2: {
        text: "🚶 Yeni arkadaşlar edin",
        feedback: "Başka insanlarla takılmaya başladın. Belki daha iyi bir grup bulursun.",
      },
    },
  },
  npc_group_leader_challenge: {
    text: "Grubun lideri olan arkadaşın son zamanlarda herkesin üzerinde baskı kuruyor. Bazıları senden \"bir şey yap\" bekliyor.",
    choices: {
      choice_0: {
        text: "👑 Liderliği üstlen",
        feedback: "Grubun yeni lideri oldun! Ama eski lider artık senin düşmanın.",
      },
      choice_1: {
        text: "🤝 Liderle konuş",
        feedback: "İkisi arasında yapıcı bir konuşma geçti. İşler düzeldi.",
      },
      choice_2: {
        text: "🚶 Gruptan ayrıl",
        feedback: "Drama istemiyorsun. Sessizce uzaklaştın.",
      },
    },
  },
  npc_new_friend_opportunity: {
    text: "Bahçede müzik dinlerken birini gördün ve ortak ilgi alanınız olduğunu fark ettin. Konuşacak mısın?",
    choices: {
      choice_0: {
        text: "🗣️ Merhaba de ve konuş",
        feedback: "Harika bir sohbet geçti! Yeni bir potansiyel arkadaş.",
      },
      choice_1: {
        text: "👀 Sadece gülümse ve geç",
        feedback: "Gülümsedin, o da gülümsedi. Belki başka zaman konuşursunuz.",
      },
      choice_2: {
        text: "🚶 Hiç fark etme, yürü",
        feedback: "Kendi işine baktın. Kim bilir kim kayboldu.",
      },
    },
  },
  npc_friend_needs_help: {
    text: "Yakın bir arkadaşın zor bir dönemden geçiyor. Ailesinde sorunlar varmış. Sana açıldı.",
    choices: {
      choice_0: {
        text: "🤗 Dinle ve destek ol",
        feedback: "Sadece dinledin ve yanında olduğunu hissettirdin. Bazen en iyi yardım budur.",
      },
      choice_1: {
        text: "💡 Çözüm öner",
        feedback: "Pratik çözümler sundun. Belki işe yarar, belki yaramaz...",
      },
      choice_2: {
        text: "🙄 \"Herkesin sorunları var\" de",
        feedback: "Empati kuramadın. Arkadaşın daha da kapandı.",
      },
    },
  },
  npc_checkin_friend_positive_01: {
    text: "Arkadasin sana \"Aksam parkta yuruyus var, gelir misin?\" diye yazdi.",
    choices: {
      choice_0: {
        text: "Giderim, sohbet ederiz",
        feedback: "Sohbet iyi geldi, baginiz guclendi.",
      },
      choice_1: {
        text: "Kisa cevap yaz, ertele",
        feedback: "Plan ertelendi ama iletisimi koparmadin.",
      },
      choice_2: {
        text: "Mesaji gormezden gel",
        feedback: "Arkadasin kirildi.",
      },
    },
  },
  npc_checkin_friend_study_02: {
    text: "Arkadasin \"Yarin sinav var, beraber tekrar yapalim mi?\" diye sordu.",
    choices: {
      choice_0: {
        text: "Beraber calisalim",
        feedback: "Ortak tekrar ikinize de iyi geldi.",
      },
      choice_1: {
        text: "Notlarimi yollarim",
        feedback: "Destek oldun ama bir araya gelmediniz.",
      },
      choice_2: {
        text: "Vaktim yok de",
        feedback: "Onceligini korudun ama mesafe artti.",
      },
    },
  },
  npc_checkin_friend_problem_03: {
    text: "Arkadasin \"Bugun moralim cok bozuk\" diye yazdi.",
    choices: {
      choice_0: {
        text: "Dinlemeyi teklif et",
        feedback: "Dinledin, guven artti.",
      },
      choice_1: {
        text: "Kisa bir moral mesaji gonder",
        feedback: "Yine de destek oldun.",
      },
      choice_2: {
        text: "Konuyu degistir",
        feedback: "Duygularini ciddiye almadigini dusundu.",
      },
    },
  },
  npc_checkin_friend_game_04: {
    text: "Arkadasin sana online oyuna davet atti.",
    choices: {
      choice_0: {
        text: "Birlikte oynayalim",
        feedback: "Eglenceli bir aksam oldu.",
      },
      choice_1: {
        text: "Yarim saat girerim",
        feedback: "Dengeyi korudun.",
      },
      choice_2: {
        text: "Bugun olmaz de",
        feedback: "Programini bozmadin ama biraz soguk kaldi.",
      },
    },
  },
  npc_checkin_friend_conflict_05: {
    text: "Arkadasin dedikodular yuzunden sana kirgin oldugunu yazdi.",
    choices: {
      choice_0: {
        text: "Acilik getirip konusalim",
        feedback: "Acik konusma havayi yumusatti.",
      },
      choice_1: {
        text: "Sakinlesince konusalim",
        feedback: "Acele etmedin, gerginlik biraz azaldi.",
      },
      choice_2: {
        text: "Savunmaya gec",
        feedback: "Tartisma buyudu.",
      },
    },
  },
  npc_checkin_bestfriend_late_01: {
    text: "Arkadasin \"Uzun zamandir oturup dertlesemedik\" diye yazdi.",
    choices: {
      choice_0: {
        text: "Bu aksam goruselim",
        feedback: "Yakinlik hissi geri geldi.",
      },
      choice_1: {
        text: "Hafta sonu planlayalim",
        feedback: "Niyetin iyi bulundu.",
      },
      choice_2: {
        text: "Musait degilim de",
        feedback: "En iyi arkadasin kendini geri planda hissetti.",
      },
    },
  },
  npc_checkin_bestfriend_crisis_02: {
    text: "Arkadasin \"Sana ihtiyacim var\" diye acil mesaj atti.",
    choices: {
      choice_0: {
        text: "Hemen ara",
        feedback: "Yaninda oldugunu hissettirdin.",
      },
      choice_1: {
        text: "Mesajla destek ol",
        feedback: "Uzak da olsan destek oldun.",
      },
      choice_2: {
        text: "Sonra donerim de",
        feedback: "Gec kaldigin icin guven sarsildi.",
      },
    },
  },
  npc_checkin_bestfriend_memory_03: {
    text: "Arkadasin eski bir ani fotografi atip \"Tekrar boyle bir gun yapalim\" dedi.",
    choices: {
      choice_0: {
        text: "Hemen tarih belirle",
        feedback: "Plan netlestikce heyecan artti.",
      },
      choice_1: {
        text: "Fikir guzel, bakariz",
        feedback: "Niyet var ama belirsizlik de var.",
      },
      choice_2: {
        text: "Simdilik istemiyorum de",
        feedback: "Aranizda soguk bir hava olustu.",
      },
    },
  },
  npc_checkin_crush_invite_01: {
    text: "Arkadasin \"Hafta sonu bir kahve?\" diye cekingen bir mesaj atti.",
    choices: {
      choice_0: {
        text: "Evet, bulusalim",
        feedback: "Samimi bir bulusma oldu.",
      },
      choice_1: {
        text: "Arkadaslarla olursa olur",
        feedback: "Guvenli bir cevap verdin.",
      },
      choice_2: {
        text: "Kibarca reddet",
        feedback: "Aradaki heyecan azaldi.",
      },
    },
  },
  npc_checkin_crush_signal_02: {
    text: "Arkadasin attigin hikayeye kalp birakti ve sohbet baslatmak istedi.",
    choices: {
      choice_0: {
        text: "Sohbeti surdur",
        feedback: "Iletisim dogal akti.",
      },
      choice_1: {
        text: "Kisa cevap ver",
        feedback: "Temkinli kaldin.",
      },
      choice_2: {
        text: "Gormezden gel",
        feedback: "Sinyal cevapsiz kaldi.",
      },
    },
  },
  npc_checkin_crush_mixed_03: {
    text: "Arkadasin senin hakkinda yanlis bir sey duydugunu yazdi.",
    choices: {
      choice_0: {
        text: "Sakin acikla",
        feedback: "Durumu netlestirdin.",
      },
      choice_1: {
        text: "Saka ile gecistir",
        feedback: "Konu dagildi ama tam kapanmadi.",
      },
      choice_2: {
        text: "Sinirli cevap ver",
        feedback: "Yanlis anlasilma buyudu.",
      },
    },
  },
  npc_checkin_partner_support_01: {
    text: "Arkadasin bugunun zor gectigini soyleyip seninle konusmak istedi.",
    choices: {
      choice_0: {
        text: "Tum dikkatimi veririm",
        feedback: "Destegin iliskiye iyi geldi.",
      },
      choice_1: {
        text: "Kisa konusalim",
        feedback: "Iletisim surdu ama yetersiz kaldi.",
      },
      choice_2: {
        text: "Bugun degil de",
        feedback: "Kendini yalniz hissetti.",
      },
    },
  },
  npc_checkin_partner_plan_02: {
    text: "Arkadasin birlikte gelecek planlari hakkinda konusmak istiyor.",
    choices: {
      choice_0: {
        text: "Net bir plan yapalim",
        feedback: "Birlikte yol cizmek guven verdi.",
      },
      choice_1: {
        text: "Kisa bir plan yap",
        feedback: "Kismen netlestiniz.",
      },
      choice_2: {
        text: "Konuyu ertele",
        feedback: "Belirsizlik iliskiyi gerdi.",
      },
    },
  },
  npc_checkin_partner_argument_03: {
    text: "Arkadasin son tartismayi kapatmak icin mesaj atti.",
    choices: {
      choice_0: {
        text: "Ortak nokta bul",
        feedback: "Tonusudu ve toparlandiniz.",
      },
      choice_1: {
        text: "Sakin kal ama mesafeli ol",
        feedback: "Gerilim azaldi ama tam bitmedi.",
      },
      choice_2: {
        text: "Eski konuyu tekrar ac",
        feedback: "Tartisma yeniden alevlendi.",
      },
    },
  },
  npc_checkin_rival_challenge_01: {
    text: "Arkadasin \"Bir sonraki denemede seni gececegim\" diye mesaj atti.",
    choices: {
      choice_0: {
        text: "Meydan okumayi kabul et",
        feedback: "Rekabet seni motive etti.",
      },
      choice_1: {
        text: "Saygili cevap ver",
        feedback: "Gerilimi kontrollu tuttun.",
      },
      choice_2: {
        text: "Asagilayici cevap ver",
        feedback: "Rekabet dusmanliga kaydi.",
      },
    },
  },
  npc_checkin_rival_result_02: {
    text: "Arkadasin son sonuclari paylasip seni etiketledi.",
    choices: {
      choice_0: {
        text: "Kendi sonucunu sakin paylas",
        feedback: "Sakin tavrin etkili oldu.",
      },
      choice_1: {
        text: "Tatli bir rekabet mesaji at",
        feedback: "Rekabet dozunda kaldi.",
      },
      choice_2: {
        text: "Sert cikis yap",
        feedback: "Sosyal gerilim artti.",
      },
    },
  },
  npc_checkin_rival_teamup_03: {
    text: "Arkadasin ortak bir proje icin gecici isbirligi teklif etti.",
    choices: {
      choice_0: {
        text: "Isbirligini kabul et",
        feedback: "Rekabetten ogrenmeye gectiniz.",
      },
      choice_1: {
        text: "Sinirli destek ver",
        feedback: "Iliski notr kaldI.",
      },
      choice_2: {
        text: "Reddet",
        feedback: "Kopruler biraz daha yandi.",
      },
    },
  },
  npc_checkin_enemy_taunt_01: {
    text: "Arkadasin alayci bir mesajla seni provoke etmeye calisti.",
    choices: {
      choice_0: {
        text: "Sakin kal, cevap verme",
        feedback: "Provokasyona dusmedin.",
      },
      choice_1: {
        text: "Kisa ve net cevap ver",
        feedback: "Sinir cizdin.",
      },
      choice_2: {
        text: "Ayni sertlikte cevap ver",
        feedback: "Gerilim buyudu.",
      },
    },
  },
  npc_checkin_enemy_rumor_02: {
    text: "Arkadasin senin hakkinda yeni bir soylenti yayildigini yazdi.",
    choices: {
      choice_0: {
        text: "Kanit toplayip sakin cevap ver",
        feedback: "Durumu akilla yonettin.",
      },
      choice_1: {
        text: "Destek iste",
        feedback: "Yalniz kalmadin.",
      },
      choice_2: {
        text: "Aninda patla",
        feedback: "Duygusal tepki durumu zorlastirdi.",
      },
    },
  },
  npc_checkin_enemy_boundary_03: {
    text: "Arkadasin beklenmedik sekilde \"Bu isi kapatalim mi?\" diye yazdi.",
    choices: {
      choice_0: {
        text: "Sinirli bir baris denemesi yap",
        feedback: "Gerilim biraz azaldi.",
      },
      choice_1: {
        text: "Mesafeli notr kal",
        feedback: "Sorun donduruldu, cozulmedi.",
      },
      choice_2: {
        text: "Kesin reddet",
        feedback: "Dusmanlik aynen surdu.",
      },
    },
  },
  milestone_new_friendship: {
    text: "Yeni arkadaşın artık gerçek bir arkadaşın oldu! Teneffüslerde birlikte takılıyor, sırlarınızı paylaşıyorsunuz. \"Seninle arkadaş olmak çok güzel!\" diyor.",
    choices: {
      choice_0: {
        text: "🤝 \"Sen de benim için özelsin!\"",
        feedback: "Arkadaşlığınız güçlendi. Birbirinize güvenebilirsiniz.",
      },
      choice_1: {
        text: "😊 Gülümse ve sarıl",
        feedback: "Sıcak bir kucaklaşma. Arkadaşlık bağları kuvvetlendi.",
      },
    },
  },
  milestone_best_friend_celebration: {
    text: "En iyi arkadaşın ile artık en iyi arkadaşsınız! Yıllar içinde kurduğunuz bağ çok özel. \"Sen benim kardeşimsin, hayatımdaki en önemli insanlardan birisin!\" diyor gözleri parlayarak.",
    choices: {
      choice_0: {
        text: "🤗 Sıkıca sarıl: \"Sen de benim için öylesin!\"",
        feedback: "Birbirinize sıkıca sarıldınız. Bu dostluk ömür boyu sürecek.",
      },
      choice_1: {
        text: "💎 Özel bir hediye ver",
        feedback: "Arkadaşlığınızın simgesi olacak bir hediye verdin. Gözleri doldu.",
      },
      choice_2: {
        text: "📝 \"Ömür boyu arkadaş kalacağız\" sözü ver",
        feedback: "Birbirinize söz verdiniz. Bu söz her şeyden değerli.",
      },
    },
  },
  milestone_bff_promise: {
    text: "En iyi arkadaşın sana küçük bir kutu uzattı. İçinde iki parça halinde bölünmüş bir kolye var. \"Yarısı sende, yarısı bende. Nerede olursak olalım, hep bağlı kalacağız.\"",
    choices: {
      choice_0: {
        text: "💝 Kolyenin yarısını tak",
        feedback: "Kolyeyi boynuna taktın. Bu sembol, aranızdaki bağın simgesi.",
      },
      choice_1: {
        text: "🙏 Teşekkür et ama kabul etme",
        feedback: "Nazikçe reddetttin. Arkadaşın biraz hayal kırıklığına uğradı.",
      },
    },
  },
  milestone_butterflies: {
    text: "O kişi'a bakarken kalbinin hızlandığını, yüzünün kızardığını fark ettin. O kişi her güldüğünde içinde kelebekler uçuşuyor. İlk defa birinden bu kadar hoşlanıyorsun...",
    choices: {
      choice_0: {
        text: "💕 Bu hisleri kabullen",
        feedback: "Hoşlandığını kabul ettin. Şimdi ne yapacağına karar vermelisin.",
      },
      choice_1: {
        text: "📓 Günlüğüne yaz",
        feedback: "Tüm hislerini günlüğüne döktün. Rahatladın ama hâlâ kafan karışık.",
      },
      choice_2: {
        text: "🙈 Bu hisleri bastır",
        feedback: "Hislerini görmezden gelmeye çalıştın. Ama görmezden gelmek çözüm mü?",
      },
    },
  },
  milestone_first_crush_feelings: {
    text: "Hoşlandığın kişi bugün sana farklı baktı. Yoksa o da mı...? Kalbin yerinden çıkacak gibi atıyor. Arkadaşların fark etti: \"Neden yüzün bu kadar kırmızı?\"",
    choices: {
      choice_0: {
        text: "😳 \"Hiçbir şey, sıcak geldi\" de",
        feedback: "Geçiştirdin ama herkes anladı. Artık bir sırrın var.",
      },
      choice_1: {
        text: "😊 Arkadaşlarına itiraf et",
        feedback: "Arkadaşların heyecanlandı! Artık yardımcıların var.",
      },
    },
  },
  milestone_new_relationship: {
    text: "Sevgilin ile artık resmi olarak birlikteisiniz! İlk ilişkin... El ele tutuştuğunuzda tüm dünya duruyor sanki. \"Seninle olmak çok güzel\" diyor Sevgilin.",
    choices: {
      choice_0: {
        text: "❤️ \"Ben de çok mutluyum\"",
        feedback: "İlk ilişkiniz başladı. Hayat çok güzel!",
      },
      choice_1: {
        text: "😊 Utanarak gülümse",
        feedback: "Utangaç ama mutlu bir başlangıç.",
      },
    },
  },
  milestone_first_date: {
    text: "Sevgilin ile ilk buluşmanız! Nereye gideceksiniz?",
    choices: {
      choice_0: {
        text: "🎬 Sinemaya git",
        feedback: "Harika bir film izlediniz. El ele tutuştunuz!",
      },
      choice_1: {
        text: "🌳 Parkta yürüyüş yap",
        feedback: "Doğada güzel bir vakit geçirdiniz. Sohbet çok akıcıydı.",
      },
      choice_2: {
        text: "☕ Kafede buluş",
        feedback: "Saatlerce sohbet ettiniz. Birbirinizi daha iyi tanıdınız.",
      },
    },
  },
  milestone_rivalry_begins: {
    text: "O kişi ile aranızda bir rekabet başladı. Her fırsatta birbirinizi alt etmeye çalışıyorsunuz. \"Seni yeneceğim\" diyor gözlerindeki ateşle.",
    choices: {
      choice_0: {
        text: "🔥 \"Ben daha iyiyim!\"",
        feedback: "Rekabeti kabul ettin. Bu seni daha çok çalışmaya itecek.",
      },
      choice_1: {
        text: "🤝 Barış teklif et",
        feedback: "\"Düşman olmak zorunda değiliz\" dedin. Belki işe yarar.",
      },
      choice_2: {
        text: "🙄 Aldırma",
        feedback: "Umursamaz davrandın. Ama o vazgeçmeyecek gibi görünüyor.",
      },
    },
  },
  milestone_enemy_made: {
    text: "O kişi artık senden nefret ediyor. Gözlerindeki soğukluk çok belirgin. \"Senden intikam alacağım\" der gibi bakıyor.",
    choices: {
      choice_0: {
        text: "😰 Tedirgin ol",
        feedback: "Bir düşman kazandın. Arkana dikkat etmelisin.",
      },
      choice_1: {
        text: "💪 \"Korkumuyorum!\"",
        feedback: "Cesur duruyorsun ama bu savaş kolay olmayacak.",
      },
    },
  },
  milestone_friendship_lost: {
    text: "Bir zamanlar arkadaşın olan kişi artık senden uzaklaşmış. Selam bile vermeden geçiyor yanından. Ne oldu böyle?",
    choices: {
      choice_0: {
        text: "😢 Üzül",
        feedback: "Arkadaşlıklar bazen biter. Ama bu acıtıyor.",
      },
      choice_1: {
        text: "🤷 Hayat böyle",
        feedback: "Pragmatik davrandın. Ama içinde bir şey eksik.",
      },
    },
  },
  milestone_heartbreak: {
    text: "İlişkin sona erdi. Kalbin kırık, gözlerin dolu. \"Neden?\" diye soruyorsun kendine. Ama cevap yok.",
    choices: {
      choice_0: {
        text: "😭 Ağla",
        feedback: "Gözyaşları akıyor. Ama zamanla geçecek... belki.",
      },
      choice_1: {
        text: "😤 Öfkelen",
        feedback: "Öfke içini yakıyor. Belki de hak etmişti.",
      },
      choice_2: {
        text: "🎧 Müzik dinle ve içine kapat",
        feedback: "Acıyı müzikle bastırdın. Ama hâlâ orada.",
      },
    },
  },
  lt_yks_trial_night: {
    text: "YKS deneme sonucu beklediginden dusuk geldi. Masada sessizce tekrar programi yapiyorsun.",
    choices: {
      choice_0: {
        text: "Eksik konulari planla",
        feedback: "Planli bir tekrar listesi cikardin.",
      },
      choice_1: {
        text: "Bugunu dinlenerek kapat",
        feedback: "Kisa bir ara zihnini toplamana yardim etti.",
      },
    },
  },
  lt_lgs_mentor_offer: {
    text: "Mahalleden bir abi/abla LGS-YKS surecinde mentor olabilecegini soyluyor.",
    choices: {
      choice_0: {
        text: "Mentorlugu kabul et",
        feedback: "Dogru yonlendirme motivasyonunu arttirdi.",
      },
      choice_1: {
        text: "Kendi basima devam et",
        feedback: "Daha bagimsiz ama daha zor bir yol sectin.",
      },
    },
  },
  lt_first_internship_call: {
    text: "Bir startup staj gorusmesi icin seni aradi. Heyecanla not defterine sarildin.",
    choices: {
      choice_0: {
        text: "Gorusmeye hazirlan",
        feedback: "Gorusmede guclu bir ilk izlenim biraktin.",
      },
      choice_1: {
        text: "Ertelensin diye mesaj at",
        feedback: "Kendine zaman kazandin ama firsat riske girdi.",
      },
    },
  },
  lt_identity_crisis_week: {
    text: "Herkes senden bir sey bekliyor. Sen ise ne istedigini bulmaya calisiyorsun.",
    choices: {
      choice_0: {
        text: "Gunluk yazarak dusuncelerini netlestir",
        feedback: "Yazarak kendini daha iyi anladin.",
      },
      choice_1: {
        text: "Sosyal medyada oyalan",
        feedback: "Kisa sureli kacis iyi geldi ama sorular duruyor.",
      },
    },
  },
  lt_first_love_confession: {
    text: "Uzun suredir hoslandigin kisiye mesaj yazip silmekten yoruldun.",
    choices: {
      choice_0: {
        text: "Duygularini acikca yaz",
        feedback: "Cesur bir adim attin; sonucu ne olursa olsun buyudun.",
      },
      choice_1: {
        text: "Arkadas olarak kal",
        feedback: "Durumu sakin sekilde yonettin.",
      },
    },
  },
  lt_first_heartbreak: {
    text: "Bir mesajla iliskinizin bittigini ogrendin. Odadaki sessizlik agirlasti.",
    choices: {
      choice_0: {
        text: "Destek almak icin yakinina anlat",
        feedback: "Paylasmak yukunu hafifletti.",
      },
      choice_1: {
        text: "Icinde tut ve derslere gomul",
        feedback: "Disaridan guclusun ama iceride yoruldun.",
      },
    },
  },
  lt_family_independence_argument: {
    text: "Ailen eve donus saatini kisitliyor, sen ise daha fazla ozgurluk istiyorsun.",
    choices: {
      choice_0: {
        text: "Sakin bir sekilde kurallari yeniden konus",
        feedback: "Gerilim azaldi ve ortak bir yol bulundu.",
      },
      choice_1: {
        text: "Kapini carpip cik",
        feedback: "Anlik rahatlama oldu ama iliski zorlandi.",
      },
    },
  },
  lt_budgeting_first_income: {
    text: "Part-time isten ilk maasin yatti. Harcamak mi biriktirmek mi?",
    choices: {
      choice_0: {
        text: "Butce yapip bir kismini biriktir",
        feedback: "Kontrollu harcama aliskanligi kazandin.",
      },
      choice_1: {
        text: "Tamamini kendine harca",
        feedback: "Anlik keyif guzel ama ay sonu zor olabilir.",
      },
    },
  },
  lt_career_fair_day: {
    text: "Okulda kariyer gunu var. Standlar, insanlar ve bir suru secenek seni bekliyor.",
    choices: {
      choice_0: {
        text: "Uc farkli bolumle detayli gorus",
        feedback: "Secenekler kafanda daha net oturdu.",
      },
      choice_1: {
        text: "Sadece yakin arkadaslarla dolas",
        feedback: "Rahat bir gun oldu ama bilgi kazanimin sinirli kaldi.",
      },
    },
  },
  lt_exam_week_burnout_signal: {
    text: "Sinav haftasi ortasinda uykusuzluk ve dalginlik belirginlesmeye basladi.",
    choices: {
      choice_0: {
        text: "Bugunu hafiflet, uyku ve mola koy",
        feedback: "Ritmi duzenleyince verim geri geldi.",
      },
      choice_1: {
        text: "Aynen devam et",
        feedback: "Kisa vadede ilerledin ama bedeli agir oldu.",
      },
    },
  },
  lt_memory_echo_regret: {
    text: "Bazi anilar sessizce geri gelir ve sana yon verir.",
    choices: {
      choice_0: {
        text: "Bu kez daha bilincli davran",
        feedback: "Gecmisin dersini bugune tasidin.",
      },
      choice_1: {
        text: "Dusunmeyi ertele ve kac",
        feedback: "Anlik kacis iyi geldi ama konu kapanmadi.",
      },
    },
  },
  lt_exam_pomodoro_reset: {
    text: "Deneme haftasinda odagini toparlamak icin Pomodoro plani deniyorsun.",
    choices: {
      choice_0: {
        text: "Programa sadik kal",
        feedback: "Ritmin oturdu.",
      },
      choice_1: {
        text: "Plani birak",
        feedback: "Odagin yine dagildi.",
      },
    },
  },
  lt_exam_group_revision: {
    text: "Arkadaslarla toplu tekrar oturumu teklif edildi.",
    choices: {
      choice_0: {
        text: "Katil ve soru coz",
        feedback: "Takimla daha iyi ilerledin.",
      },
      choice_1: {
        text: "Evde tek calis",
        feedback: "Yalniz calisma modunda kaldin.",
      },
    },
  },
  lt_exam_mock_recovery: {
    text: "Son deneme beklenenden kotu geldi, toparlanma zamani.",
    choices: {
      choice_0: {
        text: "Hata analizi yap",
        feedback: "Zayif alanlarini net gordun.",
      },
      choice_1: {
        text: "Sonucu gorme",
        feedback: "Kisa rahatlama uzun strese dondu.",
      },
    },
  },
  lt_exam_library_lockin: {
    text: "Kutuphane maratonu icin sessiz bir masa buldun.",
    choices: {
      choice_0: {
        text: "Iki saat kesintisiz calis",
        feedback: "Verimli bir seans oldu.",
      },
      choice_1: {
        text: "Erken cik",
        feedback: "Kendini fazla zorlamadin.",
      },
    },
  },
  lt_exam_sleep_tradeoff: {
    text: "Gece tekrar mi, erken uyku mu ikileminde kaldin.",
    choices: {
      choice_0: {
        text: "Erken uyu",
        feedback: "Dinlenmis bir zihinle kalktin.",
      },
      choice_1: {
        text: "Sabahla",
        feedback: "Kisa kazanc, agir yorgunluk.",
      },
    },
  },
  lt_exam_parent_expectation: {
    text: "Ailenin beklentisi yuksek, baskiyi hissediyorsun.",
    choices: {
      choice_0: {
        text: "Acikca konus",
        feedback: "Baski bir miktar azaldi.",
      },
      choice_1: {
        text: "Icinde tut",
        feedback: "Disaridan sakin, iceriden gerginsin.",
      },
    },
  },
  lt_exam_focus_walk: {
    text: "Sinav oncesi odaklanmak icin kisa yuruyus dusunuyorsun.",
    choices: {
      choice_0: {
        text: "Yuruyus yap",
        feedback: "Nefesin ve odagin duzeldi.",
      },
      choice_1: {
        text: "Masada kal",
        feedback: "Calistim ama zihnim dolu.",
      },
    },
  },
  lt_exam_last_week_plan: {
    text: "Sinava son bir hafta; planin da plani gerekiyor.",
    choices: {
      choice_0: {
        text: "Gunluk hedef listesi yap",
        feedback: "Kritik haftayi yonettin.",
      },
      choice_1: {
        text: "Rastgele ilerle",
        feedback: "Kafandaki daginiklik artti.",
      },
    },
  },
  lt_career_shadow_day: {
    text: "Bir gunlugune bir profesyoneli gozlemleme firsati buldun.",
    choices: {
      choice_0: {
        text: "Tum gun takip et",
        feedback: "Meslek gercegini yakindan gordun.",
      },
      choice_1: {
        text: "Yarida ayril",
        feedback: "Firsatin bir kismini kacirdin.",
      },
    },
  },
  lt_career_portfolio_push: {
    text: "Staj basvurusu icin kisa bir portfolyo hazirlaman gerekiyor.",
    choices: {
      choice_0: {
        text: "Projeleri duzenle",
        feedback: "Basvuru dosyan guclendi.",
      },
      choice_1: {
        text: "Eski haliyle gonder",
        feedback: "Riskli bir tercih yaptin.",
      },
    },
  },
  lt_career_teacher_reference: {
    text: "Ogretmenden referans mektubu istemeyi dusunuyorsun.",
    choices: {
      choice_0: {
        text: "Kibarca talep et",
        feedback: "Guvencen artan bir adim oldu.",
      },
      choice_1: {
        text: "Vazgec",
        feedback: "Guvensizlik seni geri tuttu.",
      },
    },
  },
  lt_career_networking_event: {
    text: "Kampuste mini kariyer bulusmasi duzenlendi.",
    choices: {
      choice_0: {
        text: "Uc kisiyle tanis",
        feedback: "Yeni baglantilar kurdun.",
      },
      choice_1: {
        text: "Sadece dinle",
        feedback: "Pasif ama guvenli bir katilim oldu.",
      },
    },
  },
  lt_career_internship_rejection: {
    text: "Staj basvurundan red cevabi geldi.",
    choices: {
      choice_0: {
        text: "Geri bildirim iste",
        feedback: "Red cevabini derse cevirdin.",
      },
      choice_1: {
        text: "Tamamen birak",
        feedback: "Moralin epey dustu.",
      },
    },
  },
  lt_career_internship_offer: {
    text: "Kucuk bir ekipten yaz staji teklifi geldi.",
    choices: {
      choice_0: {
        text: "Teklifi kabul et",
        feedback: "Ilk profesyonel adimini attin.",
      },
      choice_1: {
        text: "Sinava odaklanmak icin ertele",
        feedback: "Onceliklerini netlestirdin.",
      },
    },
  },
  lt_career_public_speaking: {
    text: "Sinifta meslek hedefinle ilgili kisa bir sunum yapacaksin.",
    choices: {
      choice_0: {
        text: "Hazirlikla cik",
        feedback: "Kendini daha net ifade ettin.",
      },
      choice_1: {
        text: "Dogaclama git",
        feedback: "Kurtardin ama zorlandin.",
      },
    },
  },
  lt_identity_value_map: {
    text: "Kendi degerlerini yazip hayatinla karsilastirmaya karar verdin.",
    choices: {
      choice_0: {
        text: "Detayli liste yap",
        feedback: "Icine dair daha net bir harita cikti.",
      },
      choice_1: {
        text: "Ertesi haftaya birak",
        feedback: "Soru isaretleri duruyor.",
      },
    },
  },
  lt_identity_new_hobby: {
    text: "Yeni bir hobinin seni degistirebilecegini hissediyorsun.",
    choices: {
      choice_0: {
        text: "Deneme dersine git",
        feedback: "Yeni bir tarafini kesfettin.",
      },
      choice_1: {
        text: "Konfor alaninda kal",
        feedback: "Guvenli ama durağan kaldin.",
      },
    },
  },
  lt_identity_role_conflict: {
    text: "Evdeki beklentilerle okuldaki hedeflerin cakisiyor.",
    choices: {
      choice_0: {
        text: "Oncelik sirasi belirle",
        feedback: "Cakismayi yonetilebilir hale getirdin.",
      },
      choice_1: {
        text: "Her seyi ayni anda yap",
        feedback: "Yuk birikti.",
      },
    },
  },
  lt_identity_journal_reflection: {
    text: "Aynaya bakarken yazdiklarinla yuzlesiyorsun.",
    choices: {
      choice_0: {
        text: "Acik yaz",
        feedback: "Yazmak icini rahatlatti.",
      },
      choice_1: {
        text: "Bos sayfa birak",
        feedback: "Konu yine ertelendi.",
      },
    },
  },
  lt_identity_social_mask: {
    text: "Kalabalikta farkli, yalnizken farkli hissetmeye basladin.",
    choices: {
      choice_0: {
        text: "Guvendigin biriyle konus",
        feedback: "Maske biraz indi.",
      },
      choice_1: {
        text: "Her seyi icinde tut",
        feedback: "Yuk hafiflemedi.",
      },
    },
  },
  lt_identity_gap_year_talk: {
    text: "Mezuniyet sonrasi yolun icin farkli bir yil plani fikri dogdu.",
    choices: {
      choice_0: {
        text: "Arastirip planla",
        feedback: "Belirsizlik azaldi.",
      },
      choice_1: {
        text: "Fikri kapat",
        feedback: "Karar ertelendi.",
      },
    },
  },
  lt_love_mixed_signals: {
    text: "Hoslandigin kisiden karisik sinyaller aliyorsun.",
    choices: {
      choice_0: {
        text: "Net bir soru sor",
        feedback: "Belirsizlik yerine netlik geldi.",
      },
      choice_1: {
        text: "Sinyalleri yorumlamaya devam et",
        feedback: "Belirsizlik uzadi.",
      },
    },
  },
  lt_love_friend_advice: {
    text: "En yakin arkadasin iliski konusunda fikrini acikca soyluyor.",
    choices: {
      choice_0: {
        text: "Tavsiyeyi dinle",
        feedback: "Dis gorus iyi geldi.",
      },
      choice_1: {
        text: "Kendi bildigini yap",
        feedback: "Bagimsiz ama riskli bir secim.",
      },
    },
  },
  lt_love_boundary_talk: {
    text: "Iliskide sinirlar ve beklentiler konusulmasi gerekiyor.",
    choices: {
      choice_0: {
        text: "Acilikla konus",
        feedback: "Daha saglikli bir denge kuruldu.",
      },
      choice_1: {
        text: "Konuyu gecistir",
        feedback: "Sorun ertelendi.",
      },
    },
  },
  lt_love_long_text_night: {
    text: "Gece uzun bir mesaj yazip yazmamak arasinda kaldin.",
    choices: {
      choice_0: {
        text: "Mesaji sadelestir",
        feedback: "Kendini net anlattin.",
      },
      choice_1: {
        text: "Duyguyla gonder",
        feedback: "Sonucu beklemek yordu.",
      },
    },
  },
  lt_love_reconciliation_attempt: {
    text: "Ayrilik sonrasi yeniden konusma teklifi geldi.",
    choices: {
      choice_0: {
        text: "Sakin bir bulusma ayarla",
        feedback: "Durumu olgunlukla yonettin.",
      },
      choice_1: {
        text: "Hemen reddet",
        feedback: "Kendi sinirlarini korudun.",
      },
    },
  },
  lt_love_new_crush: {
    text: "Uzun zaman sonra tekrar birine karsi heyecan hissettin.",
    choices: {
      choice_0: {
        text: "Nazik bir selamla basla",
        feedback: "Yeni bir sayfa aralandi.",
      },
      choice_1: {
        text: "Sadece uzaktan izle",
        feedback: "Temkinli kalmayi sectin.",
      },
    },
  },
  lt_family_budget_meeting: {
    text: "Aile butce toplantisinda soz hakki verildi.",
    choices: {
      choice_0: {
        text: "Planli bir oneride bulun",
        feedback: "Fikrin ciddiye alindi.",
      },
      choice_1: {
        text: "Sessiz kal",
        feedback: "Katki firsatini kacirdin.",
      },
    },
  },
  lt_family_house_rules_reset: {
    text: "Ev kurallari mezuniyet yaklasirken yeniden yaziliyor.",
    choices: {
      choice_0: {
        text: "Ortak kural seti olustur",
        feedback: "Evde guven duygusu artti.",
      },
      choice_1: {
        text: "Tepki goster",
        feedback: "Gerilim tirmandi.",
      },
    },
  },
  lt_family_uni_city_conflict: {
    text: "Universite sehri secimi ailede fikir ayriligi yaratti.",
    choices: {
      choice_0: {
        text: "Veriyle anlat",
        feedback: "Kararin daha anlasilir oldu.",
      },
      choice_1: {
        text: "Inat et",
        feedback: "Iletisim zorlasti.",
      },
    },
  },
  lt_family_responsibility_share: {
    text: "Evde sorumluluk paylasimi yeniden dagitiliyor.",
    choices: {
      choice_0: {
        text: "Duzenli gorev al",
        feedback: "Ev icinde guven kazandin.",
      },
      choice_1: {
        text: "Surekli ertele",
        feedback: "Guven kaybi basladi.",
      },
    },
  },
  lt_family_weekend_argument: {
    text: "Hafta sonu cikis planin yine tartismaya dondu.",
    choices: {
      choice_0: {
        text: "Saat planiyla anlas",
        feedback: "Orta yol bulundu.",
      },
      choice_1: {
        text: "Ani cikis yap",
        feedback: "Tansiyon yukseldi.",
      },
    },
  },
  lt_family_trust_rebuild: {
    text: "Gecmis bir tartismadan sonra guveni yeniden kurma sansi dogdu.",
    choices: {
      choice_0: {
        text: "Tutarlilik goster",
        feedback: "Iliski toparlanmaya basladi.",
      },
      choice_1: {
        text: "Bosver",
        feedback: "Mesafe korundu.",
      },
    },
  },
  lt_finance_first_salary_plan: {
    text: "Ilk duzenli kazancin icin 50/30/20 planini duydun.",
    choices: {
      choice_0: {
        text: "Plani uygula",
        feedback: "Parani daha kontrollu yonettin.",
      },
      choice_1: {
        text: "Plansiz harca",
        feedback: "Ay sonu zorlasti.",
      },
    },
  },
  lt_finance_emergency_fund: {
    text: "Acil durum birikimi fikri aklina yatti.",
    choices: {
      choice_0: {
        text: "Kenar para ayir",
        feedback: "Beklenmedik gunler icin guvencen artti.",
      },
      choice_1: {
        text: "Hepsini bugun kullan",
        feedback: "Anlik keyif secimi yaptin.",
      },
    },
  },
  lt_finance_impulse_buy: {
    text: "Indirimde gereksiz ama cazip bir urun gordun.",
    choices: {
      choice_0: {
        text: "24 saat bekle",
        feedback: "Durup dusunmek ise yaradi.",
      },
      choice_1: {
        text: "Hemen satin al",
        feedback: "Sonra pismanlik geldi.",
      },
    },
  },
  lt_finance_side_hustle_start: {
    text: "Kucuk bir yan gelir fikri: online tasarim/soru cozum destegi.",
    choices: {
      choice_0: {
        text: "Deneme sureci baslat",
        feedback: "Ilk musterini buldun.",
      },
      choice_1: {
        text: "Risk alma",
        feedback: "Stabil kaldin.",
      },
    },
  },
  lt_finance_debt_from_friend: {
    text: "Bir arkadasin kucuk bir borc istedi.",
    choices: {
      choice_0: {
        text: "Yazili planla ver",
        feedback: "Sinirlarini koruyarak destek oldun.",
      },
      choice_1: {
        text: "Hayir de",
        feedback: "Mali dengeni korudun.",
      },
    },
  },
  lt_finance_savings_goal: {
    text: "Kendine 3 aylik bir birikim hedefi koydun.",
    choices: {
      choice_0: {
        text: "Hedef takip cizelgesi yap",
        feedback: "Hedef gorunur hale geldi.",
      },
      choice_1: {
        text: "Aklinda tut yeter",
        feedback: "Takip zorlasti.",
      },
    },
  },
  lt_finance_scholarship_search: {
    text: "Burs arama portallarinda uzun bir aksam gecirdin.",
    choices: {
      choice_0: {
        text: "Basvurulari tamamla",
        feedback: "Maddi nefes aldiracak bir adim attin.",
      },
      choice_1: {
        text: "Yarim birak",
        feedback: "Firsat penceresi daraldi.",
      },
    },
  },
  lt_finance_micro_investment: {
    text: "Kucuk tutarli yatirim uygulamasi dikkatini cekti.",
    choices: {
      choice_0: {
        text: "Temel bilgiyi ogrenip basla",
        feedback: "Riski anlayarak adim attin.",
      },
      choice_1: {
        text: "Arastirmadan gir",
        feedback: "Dalgalanma moralini bozdu.",
      },
    },
  },
  lt_exam_strategic_reset: {
    text: "Sinav haftasi yorucu ama sen sogukkanlisin. Planlama yaparak ilerlemen gerektigini biliyorsun.",
    choices: {
      exam_strategic_schedule: {
        text: "Detayli bir calisma programi yap",
        feedback: "Her konuya saat verdim, molalari planladim. Sistem calisiyor.",
      },
      exam_strategic_focus: {
        text: "Sadece zayif konulara odaklan",
        feedback: "Her seyi bilmek zorunda degilim. Acik konulari kapatmak yeterli.",
      },
    },
  },
  lt_career_startup_idea: {
    text: "Kariyer fuarindan sonra kafanda bir is fikri var. Staj yerine kendi projeni mi yapsan?",
    choices: {
      startup_build: {
        text: "Projeye basla!",
        feedback: "Gece gunduz calisan bir girisimci oldun. Fikrin kucuk ama potansiyel buyuk.",
      },
      startup_plan: {
        text: "Once is plani hazirla",
        feedback: "Pazar arastirmasi, maliyet analizi... Hayal gercege donusuyor, adim adim.",
      },
    },
  },
  lt_first_love_deepening: {
    text: "Itiraf ettin ve karsilik gordun! Ama iliski yeni, kisisel sinirlar ve beklentiler hala belirsiz...",
    choices: {
      love_deep_communicate: {
        text: "Acik acik konusun",
        feedback: "Zor bir konusma ama iliskiniz daha saglikli bir zemine oturdu.",
      },
      love_deep_slow: {
        text: "Yavas yavas, acele etme",
        feedback: "Her seyin zamani var. Yavas ilerliyorsunuz ama saglam.",
      },
    },
  },
  lt_family_calm_negotiation: {
    text: "Aile toplantisi var. Bu kez kavga yerine sakin bir sekilde fikirlerini sunmaya karar verdin.",
    choices: {
      family_calm_propose: {
        text: "Somut oneriler sun",
        feedback: "Sakin bir dille konustun. Ailen dinledi, bazi kurallari gevsetmeye razi oldu.",
      },
      family_calm_listen: {
        text: "Once onlari dinle, sonra konus",
        feedback: "Dinlemek zor ama etkili oldu. Karsilikli anlayis buyuyor.",
      },
    },
  },
  fam_strict_notebook_check: {
    text: "Baban not defterini masaya koydu: 'Tum sayfalari gormek istiyorum.'",
    choices: {
      strict_obey_notebook: {
        text: "Defteri ver, acikla",
        feedback: "Sert bir sorguydu ama notlari saklamadin. Evde guven puanin artti.",
      },
      strict_hide_notebook: {
        text: "Defteri gizle",
        feedback: "Bir tur atlattin ama yakalanma korkusu gun boyu pesini birakmadi.",
      },
    },
  },
  fam_strict_no_permission: {
    text: "Arkadaslarin disari cagiriyor ama evde cevap net: 'Izin yok.'",
    choices: {
      strict_follow_rule: {
        text: "Kurala uy",
        feedback: "Evde kaldin. Disaridan gelen mesajlar canini sikti ama evde tansiyon cikmadi.",
      },
      strict_sneak_out: {
        text: "Gizlice cik",
        feedback: "Eglendin ama eve donus bir sorgu odasina dondu. Risk buyuktu, bedeli de.",
      },
      strict_negotiate: {
        text: "Pazarlik et",
        feedback: "Tam istedigin olmadi ama bir orta yol buldun. Ilk kez seni sonuna kadar dinlediler.",
      },
    },
  },
  fam_strict_university_choice: {
    text: "Universite tercihi masada. Baban kendi istedigi bolumu net sekilde dayatiyor.",
    choices: {
      strict_obey_career: {
        text: "Itaat et",
        feedback: "Aile mutlu, kapilar acik. Ama seciminin ne kadari sana ait emin degilsin.",
      },
      strict_rebel_career: {
        text: "Kendi yolunu sec",
        feedback: "Masada sesler yukseldi. Yalnizlastin ama karar bu kez sana ait.",
      },
      strict_compromise_career: {
        text: "Uzlasma ara",
        feedback: "Tam istedigin degil ama tek tarafli da degil. Ikna masasi bu kez ise yaradi.",
      },
    },
  },
  fam_chaotic_no_dinner: {
    text: "Eve geldin, mutfakta yemek yok. Herkes kendi telasinda kaybolmus.",
    choices: {
      chaotic_ask_neighbor: {
        text: "Komsudan iste",
        feedback: "Kapiyi caldin, yardim istedin. Bir tabak yemek ve sicak bir sohbet iyi geldi.",
      },
      chaotic_cook_self: {
        text: "Kendin hazirla",
        feedback: "Mutfagi dagittin ama altindan kalktin. Kimse soylemeden sorumluluk aldin.",
      },
    },
  },
  fam_chaotic_parent_conflict: {
    text: "Evde tartisma buyuyor. Sesler yukseldikce ortada kaldigini hissediyorsun.",
    choices: {
      chaotic_mediate: {
        text: "Arabulucu ol",
        feedback: "Kimse tamamen sakinlesmedi ama ipleri kopmadan tuttun. Yuk buyuktu.",
      },
      chaotic_hide_room: {
        text: "Odana cekil",
        feedback: "Kapini kapattin. Sesler uzaktan geldi, vicdan ise yakindan.",
      },
      chaotic_go_out: {
        text: "Disari cik, nefes al",
        feedback: "Sokakta yururken basin acildi. Eve dondugunde sorun bitmemisti ama sen biraz toparlandin.",
      },
    },
  },
  fam_supportive_surprise_day: {
    text: "Ailen senin icin surpriz bir gun planlamis. Her sey fazla mi, yoksa tam mi?",
    choices: {
      supportive_celebrate: {
        text: "Sevin ve katil",
        feedback: "Gulerek katildin. Evdeki bagi daha da guclendi.",
      },
      supportive_shy_accept: {
        text: "Utanarak kabul et",
        feedback: "Cekingen kaldin ama kirmadin. Sessiz bir uyum yakalandi.",
      },
    },
  },
  fam_supportive_failure_response: {
    text: "Basarisiz oldun. Ailen 'olur boyle' diyor ama icin hala rahat degil.",
    choices: {
      supportive_push_yourself: {
        text: "Kendini zorla",
        feedback: "Bir sonraki deneme icin plan yaptin. Gelisim var, ama bedeni de zorluyorsun.",
      },
      supportive_self_compassion: {
        text: "Kendini toparla",
        feedback: "Bir adim geri cektin, nefes aldin. Buyume yavas ama daha saglikli bir ritim yakaladin.",
      },
    },
  },
  fam_strict_rebel_pushback: {
    text: "Arkadaslarin disari ciktilar, sen evde mahkum gibi hissediyorsun. Bu kurallara daha fazla dayanamiyorsun.",
    choices: {
      strict_rebel_confront: {
        text: "Babana kafa tut",
        feedback: "Sesler yukseldi, kapilar catladi. Ama ilk kez kendi sesini duydun.",
      },
      strict_rebel_plan: {
        text: "Sessizce kendi planini yap",
        feedback: "Disarida inatkar, icinde stratejist. Zamanini bekleyeceksin.",
      },
    },
  },
  fam_chaotic_self_reliance: {
    text: "Evde yine kavga var. Ama artik aralara girmeyi denemiyorsun. Kendi duzenini kurmaya karar verdin.",
    choices: {
      chaotic_self_routine: {
        text: "Kendi rutinini kur",
        feedback: "Kendi yemegini, kendi programini, kendi kurallarini yaptin. Bagimsizlik ogreniliyor.",
      },
      chaotic_self_escape: {
        text: "Evden mumkun oldugunca uzaklas",
        feedback: "Kutuphane, park, arkadaslarin evi... Her yer evden iyi. Uzaklasmak rahatlatici ama kopu buyuyor.",
      },
    },
  },
  fam_supportive_growth_challenge: {
    text: "Ailen seni destekliyor ama sen daha fazlasini istiyorsun. Yeni bir hedef koymak istiyorsun.",
    choices: {
      supportive_big_goal: {
        text: "Buyuk bir hedef koy",
        feedback: "Ailenin destegi arkanda, gozun ileride. Zorlu ama ilham verici bir yol.",
      },
      supportive_share_dream: {
        text: "Hayalini ailenle paylas",
        feedback: "Gozleri doldu. Seninle gurur duyuyorlar. Birlikte plan yaptiniz.",
      },
    },
  },
  fam_supportive_calm_foundation: {
    text: "Ailen 'Seni destekliyoruz' diyor ama ne istediginden emin degilsin. Temkinli adimlar atiyorsun.",
    choices: {
      supportive_explore_slow: {
        text: "Farkli seyler dene, acele etme",
        feedback: "Her seyi denemek zorunda degilsin. Adim adim kendin oluyorsun.",
      },
      supportive_stay_safe: {
        text: "Bildik yolda devam et",
        feedback: "Guvenli bolgeden cikmadin ama isin saglamini sectin. Ailenin destegi saglam.",
      },
    },
  },
  fam_evo_strict_softening: {
    text: "Baban bu kez emir vermedi. Sessizce yanina oturup mac acip 'izleyelim mi?' dedi.",
    choices: {
      strict_softening_warm: {
        text: "Anin tadini cikar",
        feedback: "Ilk kez kurallar disinda bir yakinlik hissettin. Evdeki hava belirgin bicimde yumusadi.",
      },
      strict_softening_guarded: {
        text: "Temkinli kal ama kal",
        feedback: "Mesafeyi korudun ama masayi terk etmedin. Yeni bir denge doguyor.",
      },
    },
  },
  fam_evo_family_silence: {
    text: "Evde artik kimse kimseyle konusmuyor. Sessizlik bogucu bir duvar gibi.",
    choices: {
      family_crisis_talk: {
        text: "Sessizligi boz",
        feedback: "Zor bir konusma oldu ama duvarin bir parcasi kirildi.",
      },
      family_crisis_withdraw: {
        text: "Kendi yoluna cekil",
        feedback: "Uzaklastin ve ayakta kalmak icin kendi duzenini kurdun. Baglar ise daha da inceldi.",
      },
    },
  },
  econ_poor_scholarship_offer: {
    text: "Mahalledeki bir egitim vakfi, ihtiyac sahibi ogrenciler icin burs basvurusu acti.",
    choices: {
      econ_scholarship_careful_apply: {
        text: "Basvuruyu ozenle hazirla",
        feedback: "Basvurun kabul edildi. Burs ile okul masraflarin bir sureligine rahatlayacak.",
      },
      econ_scholarship_last_minute: {
        text: "Son gece acele bir dosya gonder",
        feedback: "Evraklar eksik kaldi ama yine de kisitli bir destek alabildin.",
      },
    },
  },
  econ_poor_mentor_support: {
    text: "Mahalleden bir abi/abla seni ucretsiz mentor programina davet etti.",
    choices: {
      econ_mentor_accept_plan: {
        text: "Programi kabul et, duzenli calis",
        feedback: "Mentorun hem kaynak buldu hem de plan cikardi. Yalniz olmadigini hissettin.",
      },
      econ_mentor_weekend_job: {
        text: "Mentorla hafta sonu mini is dene",
        feedback: "Yorucu gecti ama kazanc sagladin ve is deneyimi edindin.",
      },
    },
  },
  econ_poor_community_aid: {
    text: "Yerel bir dernek okul ve temel ihtiyaclar icin topluluk destek paketi acikladi.",
    choices: {
      econ_community_accept_aid: {
        text: "Destegi kabul et ve odagini derslere ver",
        feedback: "Topluluk destegi nefes aldirdi. Evdeki gerginlik bir miktar azaldi.",
      },
      econ_community_giveback: {
        text: "Destegi al ve gonullu calismayla geri ode",
        feedback: "Yardim alirken sahada da calistin. Hem destek buldun hem guven kazandin.",
      },
    },
  },
  mem_guilt_apology: {
    text: "Geçmişte yaptığın bir şey seni hâlâ rahatsız ediyor.",
    choices: {
      guilt_apologize: {
        text: "Özür dile",
        feedback: "Özür diledin. Karşındaki gülümsedi. İçindeki yük hafifledi.",
      },
      guilt_avoid: {
        text: "Görmezden gel",
        feedback: "Gözlerini kaçırdın. Ama içindeki his gitmedi...",
      },
    },
  },
  mem_pride_confidence: {
    text: "Geçmişte başardığın bir şey sana güven veriyor.",
    choices: {
      pride_try_new: {
        text: "Yeni bir şey dene",
        feedback: "Geçmiş başarın sana cesaret verdi. Yeni bir şey öğrendin!",
      },
      pride_stay_safe: {
        text: "Bildiğin şeylere devam et",
        feedback: "Bildiklerinde ustalaşmaya devam ettin.",
      },
    },
  },
  mem_regret_second_chance: {
    text: "Hayat sana ikinci bir şans veriyor.",
    choices: {
      regret_fix: {
        text: "Bu sefer doğrusunu yap",
        feedback: "Geçmişten ders aldın. Bu sefer doğru olanı seçtin.",
      },
      regret_repeat: {
        text: "Aynı şeyi yap",
        feedback: "Aynı hatayı tekrarladın. İçinde bir burukluk var...",
      },
    },
  },
  mem_satisfaction_sharing: {
    text: "Paylaşmak güzel bir his...",
    choices: {
      share_yes: {
        text: "Oyuncağını paylaş",
        feedback: "Paylaştın! Arkadaşın çok sevindi. Birlikte oynadınız.",
      },
      share_no: {
        text: "Bu sefer paylaşma",
        feedback: "Oyuncağını kendine sakladın. Arkadaşın biraz üzüldü.",
      },
    },
  },
  mem_fear_overcome: {
    text: "Korkuların seni durdurmak istiyor ama sen daha güçlüsün.",
    choices: {
      fear_face: {
        text: "Korkuyla yüzleş",
        feedback: "Korkunu yendin! Artık daha cesursun.",
      },
      fear_retreat: {
        text: "Geri çekil",
        feedback: "Bu sefer geri çekildin. Belki başka zaman...",
      },
    },
  },
  mem_school_pride_challenge: {
    text: "Öğretmen zor bir soru sordu. Cevabı bildiğini düşünüyorsun.",
    choices: {
      school_answer: {
        text: "Parmağını kaldır",
        feedback: "Doğru cevap! Sınıf sana hayretle baktı. Öğretmen gururlandı.",
      },
      school_silent: {
        text: "Sessiz kal",
        feedback: "Cevabı biliyordun ama söyleyemedin...",
      },
    },
  },
  mem_friendship_guilt_repair: {
    text: "Bir arkadaşın yalnız oturuyor. Yanına gidebilirsin.",
    choices: {
      friend_repair: {
        text: "Yanına git ve konuş",
        feedback: "Konuştunuz. Barıştınız. İkisinin de yüzü güldü.",
      },
      friend_ignore: {
        text: "Görmezden gel",
        feedback: "Görmezden geldin. Ama aklından çıkmadı.",
      },
    },
  },
  mem_exam_regret_study: {
    text: "Yarın sınav var. Hazırlanmalısın.",
    choices: {
      study_hard: {
        text: "Bu sefer ciddi çalış",
        feedback: "Geçmişten ders aldın. Saatlerce çalıştın. Bu sefer hazırsın!",
      },
      study_skip: {
        text: "Yine erteleme",
        feedback: "Yine ertelidin. Tarih tekerrür ediyor...",
      },
    },
  },
  mem_bully_pride_stand: {
    text: "Birisi sınıfta zorbalığa uğruyor. Müdahale edebilirsin.",
    choices: {
      bully_intervene: {
        text: "Araya gir",
        feedback: "Cesaretinle zorbalığı durdurdun. Sınıftan alkış aldın!",
      },
      bully_tell_teacher: {
        text: "Öğretmene söyle",
        feedback: "Öğretmene söyledin. Doğru olanı yaptın ama sessizce.",
      },
      bully_ignore: {
        text: "Karışma",
        feedback: "Karışmadın. Ama vicdanın rahat değil...",
      },
    },
  },
  mem_talent_show_fear: {
    text: "Okulda yetenek gösterisi var. Sahneye çıkabilirsin.",
    choices: {
      talent_perform: {
        text: "Bu sefer sahneye çık",
        feedback: "Sahneye çıktın! Eller titriyordu ama başardın!",
      },
      talent_skip: {
        text: "Yine vazgeç",
        feedback: "Yine sahneye çıkamadın. Pişmanlık büyüyor...",
      },
    },
  },
  mem_honesty_reward: {
    text: "Yerde para buldun. Ne yapacaksın?",
    choices: {
      money_return: {
        text: "Sahibini bul",
        feedback: "Parayı sahibine verdin. Çok teşekkür etti! Dürüstlüğün ödüllendirildi.",
      },
      money_keep: {
        text: "Cebine at",
        feedback: "Parayı aldın. Ama vicdanın biraz sızladı...",
      },
    },
  },
  mem_group_project_guilt: {
    text: "Grup ödevi yapmanız gerekiyor.",
    choices: {
      group_lead: {
        text: "Grubu yönet ve çalış",
        feedback: "Bu sefer herkesin payını aldın. Grup başarılı oldu!",
      },
      group_slack: {
        text: "Yine başkalarına bırak",
        feedback: "Yine kaytardın. Arkadaşların sinirli...",
      },
    },
  },
  mem_teacher_conflict_regret: {
    text: "Öğretmen senden sınıfta yardım etmeni istiyor.",
    choices: {
      teacher_help: {
        text: "Yardım et",
        feedback: "Yardım ettin. Öğretmenin gözleri gülümsedi. Aranız düzeldi.",
      },
      teacher_refuse: {
        text: "Reddet",
        feedback: "Reddettsin. Öğretmen üzgün görünüyordu.",
      },
    },
  },
  mem_identity_pride_path: {
    text: "İnsanlar senin hakkında konuşuyor. Seni nasıl tanımladıklarını merak ediyorsun.",
    choices: {
      identity_embrace: {
        text: "Bu kimliği kucakla",
        feedback: "Başarılı olmak seni tanımlıyor. Bu sorumluluğu kabul ettin.",
      },
      identity_question: {
        text: "Kendini sorgula",
        feedback: "Kim olduğunu sorguladın. Rahatsız edici ama büyüten bir süreç.",
      },
    },
  },
  mem_regret_chain_moral: {
    text: "Geçmişte yaptığın hatalar seni rahatsız ediyor. Değişim zamanı mı?",
    choices: {
      change_commit: {
        text: "Değişmeye karar ver",
        feedback: "Bu bir dönüm noktası. Geçmişi geride bırakıp yeni bir sayfa açtın.",
      },
      change_deny: {
        text: "\"Ben böyleyim\" de",
        feedback: "Değişimi reddettsin. Ama huzursuzluk devam ediyor...",
      },
    },
  },
  mem_betrayal_trust: {
    text: "Birine güvenmek kolay değil. Ama bazen gerekli.",
    choices: {
      trust_give: {
        text: "Güven ver",
        feedback: "Güveni yeniden inşa ettin. Hem karşındakine hem kendine.",
      },
      trust_withhold: {
        text: "Mesafeni koru",
        feedback: "Mesafeni korudun. Güvenilmez olmanın bedelini ödüyorsun.",
      },
    },
  },
  mem_leadership_echo: {
    text: "Sınıf başkanı seçimleri var. Aday olabilirsin.",
    choices: {
      leader_run: {
        text: "Aday ol",
        feedback: "Aday oldun! Seçim kampanyası heyecan vericiydi.",
      },
      leader_support: {
        text: "Başka birini destekle",
        feedback: "Arkadaşını destekledin. Takım oyuncususun.",
      },
    },
  },
  mem_creative_expression: {
    text: "Okulda resim yarışması var. Katılabilirsin.",
    choices: {
      art_compete: {
        text: "Yarışmaya katıl",
        feedback: "Yarışmaya katıldın. Resmin herkesin beğenisini topladı!",
      },
      art_skip: {
        text: "Katılma",
        feedback: "Bu sefer katılmadın. Belki bir dahaki sefere.",
      },
    },
  },
  mem_peer_pressure_guilt: {
    text: "Arkadaşların senden risk alman isteniyor.",
    choices: {
      peer_resist: {
        text: "Hayır de",
        feedback: "\"Hayır\" dedin. Arkadaşların şaşırdı ama sana saygı duydular.",
      },
      peer_give_in: {
        text: "Kabul et",
        feedback: "Kabul ettin. Eğlendin ama vicdanın rahat değil.",
      },
    },
  },
  mem_family_conflict_growth: {
    text: "Ailenle bir anlaşmazlığın var.",
    choices: {
      family_talk: {
        text: "Sakin konuş ve anla",
        feedback: "Ailenle sakin konuştun. Birbirinizi daha iyi anladınız.",
      },
      family_argue: {
        text: "Bağır ve kapıyı çarp",
        feedback: "Kapıyı çarptın. Ailenle arandaki mesafe büyüdü.",
      },
    },
  },
  mem_career_pride_vision: {
    text: "Gelecek hakkında düşünme zamanı geldi.",
    choices: {
      career_dream_big: {
        text: "Büyük hayal kur",
        feedback: "Hayallerin büyük! Bu motivasyon seni ileri taşıyacak.",
      },
      career_play_safe: {
        text: "Güvenli bir yol seç",
        feedback: "Güvenli yolu seçtin. Ailen memnun.",
      },
    },
  },
  mem_regret_last_chance: {
    text: "Lise bitiyor. Geçmişe bakıyorsun.",
    choices: {
      last_redemption: {
        text: "Son bir hamle yap",
        feedback: "Geçmişin yükünü omuzlarından attın. Yeni bir insan olarak mezun oluyorsun.",
      },
      last_accept: {
        text: "Geçmişi kabul et",
        feedback: "Geçmişi değiştiremezsin. Ama ondan öğrenebilirsin.",
      },
    },
  },
  mem_friendship_satisfaction_deep: {
    text: "En yakın arkadaşınla geçirdiğin zamanları düşünüyorsun.",
    choices: {
      friend_deep_talk: {
        text: "Derin bir sohbet et",
        feedback: "Saatlerce konuştunuz. Gerçek dostluk budur.",
      },
      friend_casual: {
        text: "Takıl",
        feedback: "Güzel vakit geçirdiniz. Her zamanki gibi.",
      },
    },
  },
  mem_guilt_confession: {
    text: "Bazı şeyler seni içten içe kemiriyor.",
    choices: {
      confess_open: {
        text: "Birine itiraf et",
        feedback: "İtiraf ettin. Ağır bir yük omuzlarından kalktı.",
      },
      confess_hide: {
        text: "İçinde tut",
        feedback: "Her şeyi içinde tuttun. Yük ağırlaşıyor...",
      },
    },
  },
  mem_exam_stress_pride: {
    text: "Üniversite sınavı yaklaşıyor. Kendine güvenmelisin.",
    choices: {
      exam_focus: {
        text: "O gücü hatırla ve çalış",
        feedback: "Geçmiş başarıların sana güç verdi. Saatlerce çalıştın!",
      },
      exam_panic: {
        text: "Panikle",
        feedback: "Panik yaptın. Geçmiş başarıların bile seni sakinleştiremedi.",
      },
    },
  },
  tr_bayram_sabahi_cocuk: {
    text: "Bayram sabahı! Yeni kıyafetler giydin, ev çikolata kokuyor. Büyükler elini öpeceksin, harçlık alacaksın!",
    choices: {
      bayram_cocuk_opme: {
        text: "Herkesi öp, harçlık topla!",
        feedback: "Cepler şıkır şıkır! Her büyük seni sevdi, harçlık verdi.",
      },
      bayram_cocuk_utangac: {
        text: "Annenin arkasına saklan",
        feedback: "Utanarak da olsa birkaç kişinin elini öptün. Az harçlık ama rahatın yerinde.",
      },
      bayram_cocuk_baklava: {
        text: "Doğruca baklavaya koş!",
        feedback: "Ağzın baklava dolu! Annen 'Önce el öp!' diye bağırdı ama çok geç...",
      },
    },
  },
  tr_aile_yemegi_cocuk: {
    text: "Akşam yemeği! Aile sofraya oturdu. Annen güzel yemekler yapmış ama sebze de var...",
    choices: {
      yemek_cocuk_anlat: {
        text: "Okulda ne olduğunu anlat",
        feedback: "Herkes seni dinledi! Baban gülümsedi, annen sorular sordu.",
      },
      yemek_cocuk_sebze: {
        text: "Sebzeleri tabağın kenarına it",
        feedback: "Annen fark etti. 'Sebzesini yemezsen tatlı yok!' Zor tercih...",
      },
      yemek_cocuk_yardim: {
        text: "Sofra toplamaya yardım et",
        feedback: "Tabakları taşıdın! Annen 'Aferin sana!' dedi. İçin gururla doldu.",
      },
    },
  },
  tr_ramazan_iftar_cocuk: {
    text: "Ramazan ayı! Bugün iftar var. Ev güzel kokuyor, misafirler gelecek. Annen çok çalışıyor...",
    choices: {
      iftar_cocuk_yardim: {
        text: "Anneye mutfakta yardım et",
        feedback: "Bardakları dizedin, peçeteleri yerleştirdin. Annen mutlu oldu!",
      },
      iftar_cocuk_oruc: {
        text: "Yarım gün oruç tutmayı dene",
        feedback: "Öğlene kadar dayandın! Herkes seninle gurur duydu.",
      },
      iftar_cocuk_bekle: {
        text: "İftar saatini sabırsızca bekle",
        feedback: "Saati saymaktan sıkıldın ama sonunda sofra kuruldu!",
      },
    },
  },
  tr_kurban_bayrami_cocuk: {
    text: "Kurban Bayramı! Kuzenler geldi, bahçede oyun var. Ama kesim de oluyor...",
    choices: {
      kurban_cocuk_oyun: {
        text: "Kuzenlerle koştur!",
        feedback: "Kuzenlerle saklambaç, körebe, futbol! En güzel bayram!",
      },
      kurban_cocuk_et: {
        text: "Et dağıtımına git",
        feedback: "Fakir ailelere et götürdün. 'Allah razı olsun' dediler. Güzel hissettirdi.",
      },
      kurban_cocuk_kork: {
        text: "Kesimden kork, eve kaç",
        feedback: "Sesler çok korkutucuydu. Kulaklarını kapadın ve odana kaçtın.",
      },
    },
  },
  tr_karne_gunu_ilkokul: {
    text: "Karne günü! Öğretmenin tek tek karneleri dağıtıyor. Kalbin çarpıyor...",
    choices: {
      karne_ilk_gurur: {
        text: "Karneyi gururla al",
        feedback: "Karneni aldın! Eve koşarak gidip göstereceksin!",
      },
      karne_ilk_korkut: {
        text: "Gözlerini kapat ve aç",
        feedback: "Bir gözünü aralayarak baktın... İyi mi kötü mü? Kalbim duracak!",
      },
      karne_ilk_arkadas: {
        text: "Arkadaşınla karşılaştır",
        feedback: "Sen de ben de birbirimize gösterdik. Kimi sevinçli, kimi biraz üzgün.",
      },
    },
  },
  tr_sabah_uyanis_cocuk: {
    text: "Sabah oldu! Annen 'Kalk okula geç kalacaksın!' diye sesleniyor. Yatağın çook sıcak...",
    choices: {
      sabah_cocuk_kalk: {
        text: "Hemen kalk!",
        feedback: "Zor oldu ama kalktın! Kahvaltıya yetiştin, gün enerjik başladı!",
      },
      sabah_cocuk_ertele: {
        text: "5 dakika daha...",
        feedback: "'5 dakika' 20 dakika oldu. Koşarak okula gideceksin!",
      },
      sabah_cocuk_hasta: {
        text: "Hastayım de (yalan)",
        feedback: "Annen baktı... inanmadı. 'Kalk hemen!' Planın tutmadı.",
      },
    },
  },
  tr_milli_mac_cocuk: {
    text: "Türkiye maç yapıyor! Baban TV'nin karşısına geçti, mahallede bayraklar asılmış!",
    choices: {
      mac_cocuk_baba: {
        text: "Babanla birlikte izle",
        feedback: "GOL! Babanla birlikte bağırdınız! Çok güzel bir an.",
      },
      mac_cocuk_mahalle: {
        text: "Mahallede çocuklarla kutla",
        feedback: "Kornalar çalıyor, herkes bağırıyor! Bayram gibi!",
      },
      mac_cocuk_oyna: {
        text: "İlgilenmiyorum, oyun oynayayım",
        feedback: "Herkes maç izlerken sen kendi halinde takıldın. Sakin bir akşam.",
      },
    },
  },
  tr_mahalle_futbolu_cocuk: {
    text: "Mahallede büyük çocuklar futbol oynuyor! 'Küçük de alsak mı?' diye konuşuyorlar...",
    choices: {
      futbol_cocuk_katil: {
        text: "Ben de oynayacağım!",
        feedback: "Büyük çocuklarla oynadın! Düştün, kalktın ama asla pes etmedin!",
      },
      futbol_cocuk_izle: {
        text: "Kenardan izle",
        feedback: "İzlemek de güzeldi. Belki bir dahakine katılırsın.",
      },
      futbol_cocuk_topla: {
        text: "Topu geri getiren çocuk ol",
        feedback: "Top her dışarı çıkışında sen koştun aldın. 'Sağ ol küçük!' dediler.",
      },
    },
  },
  tr_komsu_ziyaret_cocuk: {
    text: "Komşular geldi! Yanlarında senin yaşında bir çocuk da var. Annen 'Git oyna!' diyor.",
    choices: {
      komsu_cocuk_oyna: {
        text: "Odanda birlikte oyna",
        feedback: "Yeni arkadaşınla oyuncaklarını paylaştın. Eğlenceli geçti!",
      },
      komsu_cocuk_gosterme: {
        text: "Oyuncaklarını sakla, göstermek istemiyorsun",
        feedback: "Annen kızgın, 'Paylaşmayı öğreneceksin!' dedi. Ama onlar senin oyuncakların!",
      },
      komsu_cocuk_cay: {
        text: "Misafirlere çay götür",
        feedback: "'Maşallah ne terbiyeli çocuk!' Herkes seni övdü!",
      },
    },
  },
  tr_yagmurlu_gun_cocuk: {
    text: "Dışarıda yağmur yağıyor! Pencereden damlalara bakıyorsun. Bugün ne yapacaksın?",
    choices: {
      yagmur_cocuk_resim: {
        text: "Resim yap",
        feedback: "Renkli kalemlerle güzel bir resim yaptın. Annen buzdolabına astı!",
      },
      yagmur_cocuk_oyun: {
        text: "Oyuncaklarla oyna",
        feedback: "Arabaları dizdin, bebeklere çay partisi yaptın. Hayal gücün harika!",
      },
      yagmur_cocuk_disari: {
        text: "Yağmurda oyna!",
        feedback: "Su birikintilerinde zıpladın! Sırılsıklam ama çok mutlusun!",
      },
    },
  },
  tr_park_salincak: {
    text: "Parkta salıncağa binmek istiyorsun ama başka bir çocuk sallanıyor. Sıra beklemen lazım...",
    choices: {
      salincak_bekle: {
        text: "Sabırla bekle",
        feedback: "Bekledin... bekledin... Sonunda sıra geldi! Sabretmenin karşılığı!",
      },
      salincak_agla: {
        text: "Ağla ve bağır",
        feedback: "Ağladın ama çocuk inmedi. Annen utandı, seni başka oyuncağa götürdü.",
      },
      salincak_kaydir: {
        text: "Kaydırağa git",
        feedback: "Kaydırak da eğlenceli! Hem sıra yok!",
      },
    },
  },
  tr_sokak_kedisi: {
    text: "Yolda küçük bir kedi görüyorsun. Miyavlıyor, aç görünüyor. Ne yapacaksın?",
    choices: {
      kedi_besle: {
        text: "Ekmeğinden ver",
        feedback: "Kedi ekmeği yedi, sana sürtündü. Yeni bir dostun var!",
      },
      kedi_kork: {
        text: "Korkarak uzaklaş",
        feedback: "Kedi sana baktı, sen kaçtın. Belki bir dahakine cesaret edersin.",
      },
      kedi_eve: {
        text: "Eve götürmek iste",
        feedback: "'Hayır eve kedi almayacağız!' Annen kararlıydı. Ama kediyi düşünmeye devam ettin.",
      },
    },
  },
  tr_ilk_dogum_gunu: {
    text: "Bugün doğum günün! Pasta, balon, arkadaşlar! Ama bir çocuk sana hediye getirmemiş...",
    choices: {
      dogum_paylas: {
        text: "Olsun, pastadan ver",
        feedback: "Herkes eğlendi! Hediye olmasa da arkadaşlık önemli.",
      },
      dogum_uzul: {
        text: "Neden getirmedi?",
        feedback: "Biraz üzüldün ama pasta güzeldi. Belki unutmuştur?",
      },
    },
  },
  tr_kirilan_oyuncak: {
    text: "En sevdiğin oyuncak kırıldı! Üzgünsün, ağlamak istiyorsun...",
    choices: {
      oyuncak_tamir: {
        text: "Babandan tamir etmesini iste",
        feedback: "Baban bantla yapıştırdı. Eskisi gibi değil ama yine de seviyorsun!",
      },
      oyuncak_agla: {
        text: "Ağla ve kız",
        feedback: "Bir süre ağladın. Sonra diğer oyuncaklarla oynamaya başladın.",
      },
      oyuncak_yeni: {
        text: "Yenisini iste",
        feedback: "'Her kırılanda yenisini almayız!' dedi annen. Hayal kırıklığı...",
      },
    },
  },
  tr_yabanci_seker: {
    text: "Parkta tanımadığın bir amca sana şeker uzatıyor. 'Al bakalım, güzel çocuk' diyor...",
    choices: {
      yabanci_reddet: {
        text: "Hayır, tanımıyorum seni!",
        feedback: "Doğru karar! Annen sana 'Yabancılardan bir şey alma' demişti. Hatırladın!",
      },
      yabanci_anne: {
        text: "Anneye koş!",
        feedback: "Koşarak annenin yanına gittin. Güvende hissettin.",
      },
      yabanci_al: {
        text: "Şekeri al",
        feedback: "Şekeri aldın. Annen gördü ve çok kızdı! 'Yabancılardan almak yok!'",
      },
    },
  },
  tr_doktor_asi: {
    text: "Bugün aşı günü! Doktora gideceksiniz. İğne... KORKUNÇ!",
    choices: {
      asi_cesur: {
        text: "Cesur ol, ağlama",
        feedback: "İğne battı, gözlerin doldu ama ağlamadın! Doktor 'Aferin, cesur çocuk!' dedi.",
      },
      asi_agla: {
        text: "Ağla (normal tepki)",
        feedback: "Ağladın, annen seni tuttu. Sonra dondurma aldı. O kadar da kötü değildi.",
      },
      asi_kac: {
        text: "Kaçmaya çalış!",
        feedback: "Yakalandın. Zorla oldu ama aşın yapıldı. Herkes yoruldu!",
      },
    },
  },
  tr_hayali_arkadas: {
    text: "Odanda kimse yok ama sen birine konuşuyorsun. 'Görünmez arkadaşım var!' diyorsun.",
    choices: {
      hayali_oyna: {
        text: "Hayali arkadaşınla oynamaya devam et",
        feedback: "Hayal gücün müthiş! Hikayeler uydurdun, maceralar yaşadın.",
      },
      hayali_anlat: {
        text: "Anneye arkadaşını tanıt",
        feedback: "Annen gülümsedi ve arkadaşına çay ikram etti. 'Hoş geldin!' dedi.",
      },
    },
  },
  tr_ilk_yalan: {
    text: "Vazoyu kırdın ama kimse görmedi. Annen 'Kim kırdı bunu?' diye soruyor...",
    choices: {
      yalan_itiraf: {
        text: "Ben kırdım...",
        feedback: "Annen kızdı ama sonra sarıldı. 'Doğruyu söylediğin için teşekkür ederim.'",
      },
      yalan_kedi: {
        text: "Kedi yaptı!",
        feedback: "Annen baktı... inanmadı ama bir şey demedi. İçinde suçluluk kaldı.",
      },
      yalan_sus: {
        text: "Bilmiyorum...",
        feedback: "Suçluluk duygusu seni rahat bırakmadı. Gece yatakta düşündün...",
      },
    },
  },
  tr_anne_yardim_cocuk: {
    text: "Annen yerleri siliyor. Çok yorgun görünüyor. 'Yardım eder misin?' diye soruyor.",
    choices: {
      anne_yardim_et: {
        text: "Hemen yardım et!",
        feedback: "Bezi aldın, yerleri sildin! Annen seni öptü. 'Benim yardımcım!'",
      },
      anne_yardim_oyna: {
        text: "Oyun oynuyorum, sonra",
        feedback: "Oynamaya devam ettin. Annen tek başına halletti ama biraz kırgın.",
      },
    },
  },
  tr_markette_kaybolma: {
    text: "Markette annenin elini bıraktın. Etrafına bakıyorsun... ANNEN YOK! Kaybolmuşsun!",
    choices: {
      kaybol_agla: {
        text: "Olduğun yerde dur ve ağla",
        feedback: "Ağladın, bir görevli geldi. Annenle anonsla buluştunuz. Sarıldınız!",
      },
      kaybol_cesur: {
        text: "Anneni aramaya git",
        feedback: "Koridorları gezdin ve sonunda anneni buldun! Korkmuştun ama başardın!",
      },
      kaybol_gorevli: {
        text: "Görevliye git ve söyle",
        feedback: "Görevli anneni buldu. Doğru hamleydi! Annen 'Ne kadar akıllısın' dedi.",
      },
    },
  },
  tr_resim_yarisma: {
    text: "Öğretmen en güzel resmi seçecek! Herkes çiziyor. Sen ne çizeceksin?",
    choices: {
      resim_farkli: {
        text: "Farklı bir şey çiz (uzay gemisi!)",
        feedback: "Herkes ev çizerken sen uzay gemisi çizdin! Öğretmen çok beğendi!",
      },
      resim_herkes: {
        text: "Herkes gibi ev ve güneş çiz",
        feedback: "Güzel bir ev çizdin. Kötü değildi ama birçok çocuk aynı şeyi çizmişti.",
      },
    },
  },
  tr_yatmadan_hikaye: {
    text: "Yatma vakti geldi. Annen 'Bir hikaye okuyayım mı?' diyor.",
    choices: {
      hikaye_dinle: {
        text: "Hikaye dinle!",
        feedback: "Annenin sıcak sesi, yumuşak yatak... Gözlerin kapandı. Güzel rüyalar.",
      },
      hikaye_kendisi: {
        text: "Kendi hikayeni uydurmaya çalış",
        feedback: "Ejderhalar, prensesler, uçan arabalar! Hayal gücün sınırsız!",
      },
    },
  },
  tr_ilk_odev: {
    text: "Öğretmen ev ödevi verdi! Hayatındaki ilk ödev: 10 tane toplama işlemi. Defterini açtın...",
    choices: {
      odev_hemen: {
        text: "Hemen otur yap",
        feedback: "Zor oldu ama bitirdin! Annen kontrol etti, hepsi doğru!",
      },
      odev_sonra: {
        text: "Önce biraz oynayayım...",
        feedback: "Oynarken akşam oldu! Son anda yazdın ama yarısı yanlış...",
      },
      odev_anne: {
        text: "Annene yardım iste",
        feedback: "Annen yanına oturdu, birlikte yaptınız. Güzel vakit geçirdiniz.",
      },
    },
  },
  tr_yeni_arkadas: {
    text: "Sınıfa yeni bir öğrenci geldi. Tek başına oturuyor, kimseyi tanımıyor...",
    choices: {
      yeni_ark_konus: {
        text: "Yanına git, tanış",
        feedback: "Adı Ali! Çok iyi anlaştınız. Teneffüste birlikte oynadınız!",
      },
      yeni_ark_izle: {
        text: "Uzaktan izle",
        feedback: "Başkaları yanına gitti. Sen fırsatı kaçırdın ama belki yarın...",
      },
    },
  },
  tr_sinav_sonucu: {
    text: "Öğretmen sınav kağıtlarını dağıtıyor! Kalbin çarpıyor... Kağıdını çevirdin...",
    choices: {
      sinav_iyi: {
        text: "Notu gör ve sevin",
        feedback: "Not güzel! Eve koşarak gidip göstereceksin!",
      },
      sinav_kotu: {
        text: "Kötü notu sakla",
        feedback: "Kağıdı çantanın dibine tıktın. Ama er ya da geç ortaya çıkacak...",
      },
      sinav_calis: {
        text: "Bir dahakine daha çok çalış diye söz ver",
        feedback: "Kararını verdin: Bir dahakine mutlaka daha iyi olacak!",
      },
    },
  },
  tr_teneffus_yalniz: {
    text: "Teneffüste herkes gruplar halinde oynuyor. Sen tek başınasın...",
    choices: {
      teneffus_katil: {
        text: "Bir gruba katıl",
        feedback: "'Ben de oynayabilir miyim?' Kabul ettiler! Eğlenceli teneffüs!",
      },
      teneffus_kitap: {
        text: "Kitap oku kenarda",
        feedback: "Sakin bir teneffüs. Kitabın çok ilginçti, dünyayı unuttun.",
      },
      teneffus_ogretmen: {
        text: "Öğretmenle konuş",
        feedback: "Öğretmenin yanında oturup sohbet ettin. Büyüklerle konuşmak da güzel.",
      },
    },
  },
  tr_kopya_iste: {
    text: "Sınav sırasında yanındaki arkadaşın fısıldıyor: 'Cevapları göster, lütfen!'",
    choices: {
      kopya_goster: {
        text: "Göster, arkadaşın sonuçta",
        feedback: "Gösterdin ama öğretmen neredeyse yakalayacaktı! Kalbin çarpıyordu.",
      },
      kopya_reddet: {
        text: "Hayır, kendim çalıştım",
        feedback: "Arkadaşın biraz kırıldı ama doğru olanı yaptın.",
      },
    },
  },
  tr_harclik_kaybet: {
    text: "Bugün okula harçlıkla gittin. Kantinden bir şey alacaktın ama... Cebindeki para yok! Düşürmüşsün!",
    choices: {
      harclik_agla: {
        text: "Ağla, çok üzgünsün",
        feedback: "Biraz ağladın. Bir arkadaşın kantin sandviçinden verdi. Güzel insanlar var.",
      },
      harclik_ara: {
        text: "Koridorlarda ara",
        feedback: "Her yere baktın... bulamadın. Ama aramayı denemek önemliydi.",
      },
      harclik_kabul: {
        text: "Olsun, yarın da var",
        feedback: "Kayıpla başa çıkmayı öğrendin. Su içtin, idare ettin.",
      },
    },
  },
  tr_kardes_kavga: {
    text: "Kardeşin senin eşyanı aldı! Kızgınsın, kavga edeceksiniz...",
    choices: {
      kardes_konu: {
        text: "Sakin ol, konuş",
        feedback: "Zor oldu ama bağırmadan konuştun. Kardeşin geri verdi. Büyüdün!",
      },
      kardes_bagir: {
        text: "Bağır ve kız",
        feedback: "İkiniz de ağladınız. Annen geldi, ikisini de azarladı.",
      },
      kardes_anne: {
        text: "Anneye şikayet et",
        feedback: "Annen 'Kendi aranızda çözün!' dedi. Büyüklere güvenmek her zaman işe yaramıyor.",
      },
    },
  },
  tr_gece_kalma: {
    text: "Arkadaşın seni evine gece kalmaya davet etti! Ama daha önce hiç evden ayrı kalmadın...",
    choices: {
      gece_git: {
        text: "Cesur ol, git!",
        feedback: "Başta garip hissettin ama çok eğlendin! Film izlediniz, gece sohbet ettiniz!",
      },
      gece_reddet: {
        text: "Hayır, evde kalayım",
        feedback: "Rahatın yerinde ama arkadaşın biraz üzüldü.",
      },
    },
  },
  tr_grup_dislanma: {
    text: "Sınıfta bir grup oluştu. Herkes o gruba girmek istiyor. Seni de istediler ama bir şartla: 'Şu çocukla konuşma!'",
    choices: {
      grup_reddet: {
        text: "Bu doğru değil, ben herkesin arkadaşıyım",
        feedback: "Gruba girmedin ama vicdanın rahat. Dışlanan çocuk sana minnettar.",
      },
      grup_kabul: {
        text: "Tamam, artık onunla konuşmam",
        feedback: "Gruba girdin ama o çocuğun yalnız kaldığını gördükçe içinden bir ses sızlıyor.",
      },
    },
  },
  tr_sosyal_medya_ilk: {
    text: "Arkadaşların hep telefonlarıyla bir şey yapıyor. 'Sen yok musun sosyal medyada?' diye soruyorlar.",
    choices: {
      sosyal_ac: {
        text: "Hesap aç!",
        feedback: "Hesap açtın! Arkadaşlarını ekledin, fotoğraf paylaştın. Yeni bir dünya!",
      },
      sosyal_bekle: {
        text: "Ailem izin vermez, bekleyeceğim",
        feedback: "Arkadaşların dalga geçti ama ailen senin tarafında. Doğru zamanda olacak.",
      },
      sosyal_gizli: {
        text: "Gizlice hesap aç",
        feedback: "Gizlice açtın ama her an yakalanma korkusu... Stresli!",
      },
    },
  },
  tr_takim_kaptan: {
    text: "Beden eğitimi dersinde takım kaptanı seçiliyor. Seni de aday gösterdiler!",
    choices: {
      kaptan_kabul: {
        text: "Kaptan olurum!",
        feedback: "Takımı seçtin, taktik yaptın! Kazandınız mı kaybettiniz mi önemli değil - liderlik deneyimi kazandın!",
      },
      kaptan_reddet: {
        text: "Yok, ben olmayayım",
        feedback: "Baskı altına girmemek rahatlatıcı ama fırsatı kaçırdığını hissettin.",
      },
    },
  },
  tr_aile_tatili: {
    text: "Aile tatile gidecek! Ama arkadaşlarından bir hafta ayrı kalacaksın...",
    choices: {
      tatil_heyecan: {
        text: "Tatile bayıldım!",
        feedback: "Deniz, kum, güneş! Ailecek çok güzel vakit geçirdiniz!",
      },
      tatil_istemem: {
        text: "Arkadaşlarımdan ayrılmak istemiyorum",
        feedback: "Somurtarak gittin ama sonra denizi görünce unutuverdin!",
      },
    },
  },
  tr_biriktir_harca: {
    text: "Biriktirdiğin harçlıklarla istediğin oyuncağı/oyunu alabilirsin. Ama tam tutarı biriktirmek 2 hafta daha sürecek...",
    choices: {
      biriktir_bekle: {
        text: "2 hafta daha bekle, tam al",
        feedback: "İki hafta daha sabrettin. Sonunda aldın! Kendi paranla almak daha güzel!",
      },
      biriktir_ucuz: {
        text: "Daha ucuz bir şey al şimdi",
        feedback: "Anlık mutluluk ama asıl istediğin hala rafta duruyor...",
      },
      biriktir_iste: {
        text: "Ailenden farkı iste",
        feedback: "Baban 'Kendi biriktir' dedi. Annen biraz verdi. Tam oldu ama borçlusun.",
      },
    },
  },
  tr_ortaokul_gecis: {
    text: "Ortaokula geçiyorsun! Farklı sınıflar, farklı öğretmenler, sınav sistemi değişti. Her şey yeni...",
    choices: {
      ortaokul_heyecan: {
        text: "Heyecanlıyım, yeni insanlar tanıyacağım!",
        feedback: "Yeni arkadaşlar, yeni dersler! Her şey heyecan verici!",
      },
      ortaokul_korku: {
        text: "Çok korkuyorum, eski okulumu istiyorum",
        feedback: "İlk günler zor geçti ama yavaş yavaş alışmaya başladın.",
      },
      ortaokul_hazirlan: {
        text: "Önceden hazırlanayım, defter kitaplarımı düzenleyeyim",
        feedback: "Her şey düzenli! İlk gün hazırlıklı gittin, öğretmenler etkilendi.",
      },
    },
  },
  tr_ogretmen_adaletsiz: {
    text: "Öğretmenin seni suçsuz yere azarladı! Sınıfın önünde utandın. Haksızlık!",
    choices: {
      ogretmen_itiraz: {
        text: "Saygılıca itiraz et",
        feedback: "Sakin bir şekilde durumu anlattın. Öğretmen düşündü ve özür diledi!",
      },
      ogretmen_sus: {
        text: "Ses çıkarma, çökersin",
        feedback: "İçinden 'haksız' dedin ama sustun. Akşam eve gelince annene anlattın.",
      },
      ogretmen_agla: {
        text: "Ağlamaya başla",
        feedback: "Gözyaşlarını tutamadın. Bazı arkadaşların yanına geldi. Destekleri iyi geldi.",
      },
    },
  },
  tr_zorbalik_tanik: {
    text: "Teneffüste büyük bir çocuk küçük birini itiyor, çantasını alıyor. Kimse bir şey yapmıyor...",
    choices: {
      zorba_mudahale: {
        text: "Dur! Bırak onu!",
        feedback: "Zorba şaşırdı, çantayı bıraktı. Küçük çocuk sana minnettar. Cesur bir hareket!",
      },
      zorba_ogretmen: {
        text: "Öğretmene haber ver",
        feedback: "Öğretmen geldi, zorbayı azarladı. Doğru olanı yaptın, güvenli bir şekilde.",
      },
      zorba_ignore: {
        text: "Karışma, başını belaya sokarsın",
        feedback: "Geçip gittin ama o çocuğun yüzü gözünün önünden gitmedi.",
      },
    },
  },
  tr_hobi_secimi: {
    text: "Okul kurs kayıtları başladı! Müzik, resim, spor, kodlama... Hangisini seçeceksin?",
    choices: {
      hobi_muzik: {
        text: "Müzik kursu - gitar!",
        feedback: "İlk akortlar zor ama ses çıkarmaya başladın! Bu işin başı.",
      },
      hobi_kodlama: {
        text: "Kodlama kursu",
        feedback: "İlk programın: 'Merhaba Dünya!' Ekranda kendi yazdığın şeyi görmek harika!",
      },
      hobi_spor: {
        text: "Basketbol/Futbol takımı",
        feedback: "Antrenmanlar zor ama takım ruhu muhteşem! Yeni arkadaşlar edindin.",
      },
      hobi_resim: {
        text: "Resim kursu",
        feedback: "Fırçaları eline aldın, renklerin dünyasına daldın. Sakinleştirici!",
      },
    },
  },
  tr_telefon_isteme: {
    text: "Arkadaşlarının çoğunda telefon var. Sen de istiyorsun! Ama ailen 'Daha erken' diyor.",
    choices: {
      telefon_kabul: {
        text: "Tamam, beklerim",
        feedback: "Zor ama ailene güveniyorsun. Zamanı gelince alacaksın.",
      },
      telefon_tartis: {
        text: "Ama herkesin var! Adaletsiz!",
        feedback: "Tartıştınız. 'Herkes yaparsa sen de mi yaparsın?' Klasik anne cevabı...",
      },
      telefon_pazarlik: {
        text: "Karne iyi olursa anlaşma yap",
        feedback: "Baban düşündü... 'Takdir belgesi getir, düşünelim.' Motivasyon buldum!",
      },
    },
  },
  tr_ders_baskisi: {
    text: "Sınav haftası! Annen 'Çalış!' diyor, baban 'Notların düşüyor!' diyor. Stres çok...",
    choices: {
      ders_plan: {
        text: "Ders programı yap ve çalış",
        feedback: "Her gün 2 saat çalıştın. Zor ama sistematik çalışma işe yaradı!",
      },
      ders_son_gece: {
        text: "Son gece çalış",
        feedback: "Son gece gözlerini ovuşturarak çalıştın. Yarısını unuttun zaten...",
      },
      ders_arkadas: {
        text: "Arkadaşla birlikte çalış",
        feedback: "Birbirinize sordunuz, anlatınız. Hem eğlenceli hem öğretici!",
      },
    },
  },
  tr_sir_saklama: {
    text: "En yakın arkadaşın sana bir sır söyledi. Ama başka bir arkadaşın da bu sırrı soruyor!",
    choices: {
      sir_sakla: {
        text: "Asla söylemem, söz verdim",
        feedback: "Sır saklayabildın. Arkadaşın sana daha çok güveniyor artık.",
      },
      sir_soyle: {
        text: "Söyle, o da arkadaşın sonuçta",
        feedback: "Söyledin ve tabii ki yayıldı. Sırrını verdiğin arkadaşın çok kırıldı.",
      },
    },
  },
  tr_okul_takim_secme: {
    text: "Okul basketbol/futbol takımına seçmeler var! Takıma girmek istiyorsun ama rakipler çok...",
    choices: {
      secme_dene: {
        text: "Seçmelere katıl!",
        feedback: "Elinden gelenin en iyisini yaptın! Seçildin mi seçilmedin mi fark etmez - deneme cesareti önemliydi!",
      },
      secme_vazgec: {
        text: "Çok iyi değilim, katılmasam mı...",
        feedback: "İzleyici oldun. Belki bir dahakine? Ama 'keşke deneseyim' düşüncesi kaldı.",
      },
    },
  },
  tr_mahalle_cesaret: {
    text: "Mahallede çocuklar cesaret oyunu oynuyor. Sıra sende: karanlık bodruma gir!",
    choices: {
      cesaret_gir: {
        text: "Gir! Korkacak ne var!",
        feedback: "Karanlıkta 30 saniye durdun! Çıkınca herkes alkışladı. Yüreğin ağzındaydı!",
      },
      cesaret_reddet: {
        text: "Hayır, bu saçma",
        feedback: "'Korkak!' dediler ama umursamadın. Akıl herkesin yapmasını gerektirmez.",
      },
    },
  },
  tr_kardese_bakma: {
    text: "Annen dışarı çıkacak. 'Kardeşine göz kulak ol, 1 saat döneceğim' diyor.",
    choices: {
      kardes_bak: {
        text: "Tamam, ben hallederim!",
        feedback: "Kardeşinle oyun oynadın, yemek yedirdin. Annen gelince mutlu oldu!",
      },
      kardes_tv: {
        text: "TV'ye oturt, telefonla oyna",
        feedback: "Kardeşin TV izledi, sen telefonla oynadın. Rahat ama pek kaliteli değildi.",
      },
      kardes_istemem: {
        text: "Ben bakıcı mıyım?",
        feedback: "Annen üzüldü. Komşuya bırakmak zorunda kaldı. Sen rahat ettin ama...",
      },
    },
  },
  tr_grup_projesi: {
    text: "Öğretmen grup projesi verdi! 4 kişi bir grup. Ama grubundakiler çalışmıyor, her şeyi sen yapıyorsun...",
    choices: {
      proje_hepsini_yap: {
        text: "Her şeyi kendin yap",
        feedback: "Proje mükemmel oldu ama sen bitik haldesin. Arkadaşlar bedava not aldı.",
      },
      proje_uyar: {
        text: "Herkese görev dağıt, takip et",
        feedback: "Liderlik yaptın! Herkes biraz çalıştı. Mükemmel değil ama adil.",
      },
      proje_ogretmen: {
        text: "Öğretmene şikayet et",
        feedback: "Öğretmen uyardı ama arkadaşların sana kızdı. 'İspiyoncu!' İçin rahat, dışın zor.",
      },
    },
  },
  tr_arkadas_kus: {
    text: "En yakın arkadaşınla kavga ettiniz. 3 gündür konuşmuyorsunuz. Seni arıyor ama...",
    choices: {
      kus_baris: {
        text: "Barış, özür dile",
        feedback: "Özür diledin, o da diledi. Arkadaşlık daha güçlü geri döndü!",
      },
      kus_bekle: {
        text: "O gelsin, ben haklıyım",
        feedback: "İnat ettin. Birkaç gün sonra barıştınız ama arada mesafe kaldı.",
      },
    },
  },
  tr_beden_degisim: {
    text: "Son zamanlarda bedenin değişiyor. Boyun uzuyor, sesin farklılaşıyor. Garip hissediyorsun...",
    choices: {
      beden_konus: {
        text: "Anneye/babaya sor",
        feedback: "Ailen anlattı, normal olduğunu söyledi. Rahatlayıcıydı.",
      },
      beden_utanc: {
        text: "Utanarak görmezden gel",
        feedback: "Herkesin aynı şeyi yaşadığını bilmeden endişelendin. Sonra öğreneceksin.",
      },
      beden_arastir: {
        text: "Kitaptan/internetten araştır",
        feedback: "Ergenlik hakkında bilgi edindin. Bilgi güçtür! Daha az endişelisin.",
      },
    },
  },
  tr_bilim_fuari: {
    text: "Okulda bilim fuarı! Bir proje hazırlamak zorundasın. Ne yapacaksın?",
    choices: {
      bilim_volkan: {
        text: "Yanardağ maketi yap",
        feedback: "Sirke ve karbonat! Patladı! Herkes etkilendi. Bilim eğlenceli!",
      },
      bilim_bitki: {
        text: "Bitki büyütme deneyi yap",
        feedback: "3 hafta boyunca her gün bitki ölçtün, fotoğrafladın. Sabır ve bilim!",
      },
      bilim_son_gun: {
        text: "Son güne bırak",
        feedback: "Aceleyle bir poster yaptın. Kötü değildi ama olabilecek en iyisi de değildi.",
      },
    },
  },
  tr_konser_izin: {
    text: "Çok sevdiğin şarkıcı şehrine konser verecek! Ama hafta içi ve okul var. Annenden izin istemelisin...",
    choices: {
      konser_iste: {
        text: "Lütfen izin ver! Söz notlarım düşmez!",
        feedback: "Annen düşündü... 'Bu seferlik tamam.' YAŞASIN! Harika bir gece oldu!",
      },
      konser_vazgec: {
        text: "Okul daha önemli, izlemem",
        feedback: "Çok zor bir karar ama sorumluluk bilincin takdir topladı.",
      },
    },
  },
  tr_karne_pazarlik: {
    text: "Karne haftası. Baban dedi: 'Takdir getir, telefon alırız.' Notların sınırda...",
    choices: {
      karne_pazar_calis: {
        text: "Son bir hafta gaz ver!",
        feedback: "Gece gündüz çalıştın! Notların yükseldi. Hedef yakın!",
      },
      karne_pazar_sans: {
        text: "Belki tutar, Allah kerim",
        feedback: "Şansa bıraktın. Karne günü gergin bekleyiş...",
      },
    },
  },
  goal_chain_stage1_discovery: {
    text: "Ogretmenin sende akademik potansiyel gordu ve bir mentor programina davet etti.",
    choices: {
      goal_chain_s1_safe: {
        text: "Planli ve kontrollu ilerle",
        feedback: "Ogretmenin sende akademik potansiyel gordu ve bir mentor programina davet etti. Dengeli bir yaklasimla istikrar kurdun.",
      },
      goal_chain_s1_risky: {
        text: "Agresif hamle yap, hizli sonucu zorla",
        feedback: "Ogretmenin sende akademik potansiyel gordu ve bir mentor programina davet etti. Yuksek risk aldin; hizli kazandin ama bedel odedin.",
      },
    },
  },
  goal_chain_stage2_first_competition: {
    text: "Ilk ciddi deneme sinavina cikiyorsun. Programin gercekten ise yarayip yaramadigi belli olacak.",
    choices: {
      goal_chain_s2_safe: {
        text: "Planli ve kontrollu ilerle",
        feedback: "Ilk ciddi deneme sinavina cikiyorsun. Programin gercekten ise yarayip yaramadigi belli olacak. Dengeli bir yaklasimla istikrar kurdun.",
      },
      goal_chain_s2_risky: {
        text: "Agresif hamle yap, hizli sonucu zorla",
        feedback: "Ilk ciddi deneme sinavina cikiyorsun. Programin gercekten ise yarayip yaramadigi belli olacak. Yuksek risk aldin; hizli kazandin ama bedel odedin.",
      },
    },
  },
  goal_chain_stage3_grand_final: {
    text: "Ulke capindaki burs finaline ciktin. Tum emegin bu son performansta olculecek.",
    choices: {
      goal_chain_s3_safe: {
        text: "Planli ve kontrollu ilerle",
        feedback: "Ulke capindaki burs finaline ciktin. Tum emegin bu son performansta olculecek. Dengeli bir yaklasimla istikrar kurdun.",
      },
      goal_chain_s3_risky: {
        text: "Agresif hamle yap, hizli sonucu zorla",
        feedback: "Ulke capindaki burs finaline ciktin. Tum emegin bu son performansta olculecek. Yuksek risk aldin; hizli kazandin ama bedel odedin.",
      },
    },
  },
  npcq_friend_secret_share: {
    text: "Arkadasin seni kenara cekiyor. Gozleri ciddi. \"Sana bir sey soyleyecegim ama kimseye anlatmayacagina soz ver...\" Ailesiyle ilgili zor bir durumu anlatmaya basliyor.",
    choices: {
      choice_0: {
        text: "🤫 \"Soz veriyorum, guvenebilirsin bana\"",
        feedback: "Arkadasin rahatlamis gorunuyor. Aranizda derin bir guven baglandi.",
      },
      choice_1: {
        text: "😬 \"Bu cok agir... baska birine de soylemeli misin?\"",
        feedback: "Arkadasin biraz hayal kirikligina ugramis gibi. Ama samimiyetini takdir ediyor.",
      },
      choice_2: {
        text: "🙄 \"Herkesin derdi var, cok buyutme\"",
        feedback: "Arkadasinin yuzu dusturdu. Belki de bu kadar guvendigi icin pismandir.",
      },
    },
  },
  npcq_friend_family_visit: {
    text: "Arkadasin seni evine davet etti. Ailesiyle tanisacaksin. Kapidan girer girmez sicak yemek kokusu ve kahkaha sesleri duyuyorsun.",
    choices: {
      choice_0: {
        text: "😊 Ailesiyle sicak bir sohbet kur",
        feedback: "Harika bir aksam gecirdin. Arkadasinin ailesi seni cok sevdi.",
      },
      choice_1: {
        text: "📱 Utaniyorum, telefonuma bakayim",
        feedback: "Sessiz bir aksam oldu. Firsat kacti ama kotu de gecmedi.",
      },
    },
  },
  npcq_friend_casual_hangout: {
    text: "Arkadasin ile okuldan sonra parka gittiniz. \"Bugun sadece takilalim, hic bir sey dusunmeden\" diyor.",
    choices: {
      choice_0: {
        text: "🎉 \"Harika fikir! Ne yapalim?\"",
        feedback: "Gulerek vakit gecirdiniz. Bazen en guzel anlar plansiz olanlar.",
      },
      choice_1: {
        text: "📚 \"Aslinda ders calismaliyim...\"",
        feedback: "Arkadasin biraz uzuldu ama anladigindan emin.",
      },
    },
  },
  npcq_friend_shared_crisis: {
    text: "Okulda buyuk bir kavga cikti. Birisi Arkadasin'a iftira atiyor — ogretmenler de inaniyor. Arkadasin yardim istiyor gibi sana bakiyor.",
    choices: {
      choice_0: {
        text: "🛡️ \"Ben sahit oldum, yanlis anliyorsunuz!\"",
        feedback: "Arkadasin icin risk aldin. Bu bagliligi asla unutmayacak.",
      },
      choice_1: {
        text: "😰 Karisma, uzak dur",
        feedback: "Kenara cekildin. Arkadasin yalniz kaldi ve bunu hissetti.",
      },
      choice_2: {
        text: "🕵️ Gizlice gercegi arastir",
        feedback: "Kanitlari topluyorsun. Bu cesaretini arkadasin fark etti.",
      },
    },
  },
  npcq_friend_adventure: {
    text: "Arkadasin ile birlikte \"yasak bolge\"ye — mahallenin terk edilmis deposuna — girmeye karar verdiniz. Soylentilere gore icerde eski bir hazine var.",
    choices: {
      choice_0: {
        text: "🔦 \"Hadi birlikte girelim!\"",
        feedback: "Icerde hazine yoktu ama birlikte yasadiginiz heyecan paha bicilmezdi.",
      },
      choice_1: {
        text: "🤔 \"Fikir degistirdim, tehlikeli olabilir\"",
        feedback: "Arkadasin biraz hayal kirikligina ugradi. Ama guvenliginizi korudunuz.",
      },
    },
  },
  npcq_friend_loyalty_test: {
    text: "Okuldaki populer grubun lideri sana yaklasti: \"Bizimle takilmak istersen Arkadasin'i birak. O seni asagi cekiyor.\" Populer grup seni kabul edecek ama...",
    choices: {
      choice_0: {
        text: "💎 \"${n} benim arkadasim, tesekkurler ama hayir\"",
        feedback: "Populer grubun teklifini reddettin. Arkadasin bunun ne kadar zor oldugunu biliyor.",
      },
      choice_1: {
        text: "🤷 \"Dusuneyim...\" — iki tarafla da iyi gecin",
        feedback: "Iki ayak uzerinde duruyorsun ama hicbir tarafa tam baglanmadin.",
      },
      choice_2: {
        text: "⭐ Populer gruba katil",
        feedback: "Populer gruba katildin ama arkadasinin bakislari... bir sey kirildi.",
      },
    },
  },
  npcq_friend_final_bond: {
    text: "Arkadasin ile yillarin biriktigi bir aksam. Damlarin ustunde oturmus sehri izliyorsunuz. \"Sen benim en yakin arkadasimsin\" diyor sessizce.",
    choices: {
      choice_0: {
        text: "💎 \"Sen de benim. Ne olursa olsun.\"",
        feedback: "Bu gece asla unutulmayacak. Gercek bir dostluk muyhurlendi.",
      },
    },
  },
  npcq_friend_drift_apart: {
    text: "Arkadasin ile artik eskisi gibi konusamiyorsunuz. Mesajlarina gec cevap veriyor, planlari surekli iptal ediyor. Bir seyler degisti.",
    choices: {
      choice_0: {
        text: "💬 \"Ne oldu aramizda? Konusalim mi?\"",
        feedback: "Acik konustunuz. Belki her sey eskisi gibi olmayacak ama saygi kaldi.",
      },
      choice_1: {
        text: "🚶 Kabullen ve yoluna devam et",
        feedback: "Bazi insanlar hayatinin bir doneminde kalir. Ve bu da tamam.",
      },
    },
  },
  npcq_romance_first_spark: {
    text: "Arkadasin ile gorusleriniz kesisti — bir an icin hic kimse yokmus gibi hissettin. Kalbin hizlandi. Bu his... yeni bir sey.",
    choices: {
      choice_0: {
        text: "😊 Gulu ve bak — belki o da hissediyordur",
        feedback: "Gulumsedin. O da gulumsedi. Kalbindeki carpinti sessizce buyuyor.",
      },
      choice_1: {
        text: "😳 Kafani cevir — bu konuda hazirligi deilsin",
        feedback: "Gozlerini kacirdin. Ama o his bir yere gitmiyor...",
      },
    },
  },
  npcq_romance_confession: {
    text: "Okuldan sonra Arkadasin ile bas basasiniz. Icinden bir ses \"soyle\" diyor. Diger ses \"dur, ya reddederse?\"",
    choices: {
      choice_0: {
        text: "💕 \"Senden hoslaniyorum...\"",
        feedback: "Soyledin. Kalbin patlayacak gibi. Ama bir yukten kurtulmus gibi de hissediyorsun.",
      },
      choice_1: {
        text: "📝 Mektup yaz — yuzyuze soyleyemezsin",
        feedback: "Kelimelerin kagittan daha guclu akmis. Simdi bekleme zamani.",
      },
      choice_2: {
        text: "🙊 Yutkundun ama soyleyemedin",
        feedback: "An gecti. Belki baska bir gun... ama bugun degil.",
      },
    },
  },
  npcq_romance_first_date: {
    text: "Arkadasin ile ilk kez bas basa disari cikiyorsunuz. \"Nereye gidelim?\" diye soruyor.",
    choices: {
      choice_0: {
        text: "☕ Sakin bir kafeye git — konusalim",
        feedback: "Saatlerin nasil gectigini anlamadin. Konusmak hic bu kadar kolay olmamisti.",
      },
      choice_1: {
        text: "🎬 Sinemaya git — klasik secim",
        feedback: "Film guzeldi ama en guzel kismi yan yana oturmakti.",
      },
      choice_2: {
        text: "🌳 Parka yuru — dogada vakit gecir",
        feedback: "Gun batiminda yuruyus. Sessizlik bile rahatti.",
      },
    },
  },
  npcq_romance_jealousy_jealous: {
    text: "Arkadasin baskasiyala komustugun icin sinirli. \"O kim? Neden surekli konusuyorsunuz?\" Sesi sert, gozleri kirgin.",
    choices: {
      choice_0: {
        text: "🤗 \"Sadece arkadasim. Sana guveniyorum, sen de bana guven\"",
        feedback: "Sakince acikladin. Bir sure sonra yumusamaya basladi.",
      },
      choice_1: {
        text: "😠 \"Bu kadar kiskanc olma, boguyorsun beni!\"",
        feedback: "Sert konustun. Belki haklisin ama zamanlama kotu.",
      },
      choice_2: {
        text: "😞 \"Haklisin, bir daha olmaz\"",
        feedback: "Teslim oldun. Kisa vadede rahatladi ama uzun vadede...",
      },
    },
  },
  npcq_romance_jealousy_loyal: {
    text: "Arkadasin fark ettin ki son zamanlarda biraz mesafeli. Bir gun sonunda soruyorsun: \"Bir sorun mu var?\"",
    choices: {
      choice_0: {
        text: "💬 Sabırla dinle",
        feedback: "\"Bazen korkuyorum kaybetmekten\" dedi. Durust bir konusma oldu.",
      },
      choice_1: {
        text: "🎁 Surpriz bir sey yap — hediye veya mektup",
        feedback: "Gozleri parladi. Bazen kucuk jestler en buyuk seyleri soyler.",
      },
    },
  },
  npcq_romance_family_reaction: {
    text: "Ailen Arkadasin'i duydu. Annen merakla sordu: \"Anlat bakalim, nasil biri?\"",
    choices: {
      choice_0: {
        text: "😊 Acikca anlat — gizlemenin anlami yok",
        feedback: "Ailen sasirdi ama durst oldun. Bu onemli.",
      },
      choice_1: {
        text: "🤫 \"Sadece arkadas\" de — henuz hazir degil",
        feedback: "Gercegi sakladin. Simdilik rahat ama sonra zor olabilir.",
      },
      choice_2: {
        text: "😤 \"Bu benim ozel hayatim!\"",
        feedback: "Sinirlarini koydun. Ailen sasirdi ama mesaji aldi.",
      },
    },
  },
  npcq_romance_commitment: {
    text: "Arkadasin ile ozel bir aksam. \"Biz... resmi miyiz?\" diye soruyor yumusak bir sesle.",
    choices: {
      choice_0: {
        text: "❤️ \"Evet. Seninleyim.\"",
        feedback: "Kalbiniz ayni ritimde atiyor. Bu ozel.",
      },
    },
  },
  npcq_romance_mature_breakup: {
    text: "Arkadasin ile bir seyler degisti. Ikiniz de biliyorsunuz ama kimse soyleyemiyor. Sonunda bir aksam konusuyorsunuz.",
    choices: {
      choice_0: {
        text: "🤝 \"Guzel bir donenemdi. Tesekkur ederim.\"",
        feedback: "Aciti ama dogru olani yaptiniz. Bazi seyler bitmeli ki baskasi baslayabilsin.",
      },
      choice_1: {
        text: "😢 \"Gitme... lutfen\"",
        feedback: "Kalbin kirik. Ama zaman en iyi ilac derler...",
      },
    },
  },
  npcq_rival_first_challenge: {
    text: "Arkadasin sinifin ortasinda sana meydan okudu: \"Bahse girerim bu sinavi benden iyi yapamazsin.\" Herkes size bakiyor.",
    choices: {
      choice_0: {
        text: "🔥 \"Kabul! Goruruz.\"",
        feedback: "Meydan okuma kabul edildi. Sinifta heyecan dalga dalga yayiliyor.",
      },
      choice_1: {
        text: "😏 \"Benim kanitlamam gereken bir sey yok\"",
        feedback: "Sogukkanli cevap. Rakibin sasirdi ama saygisi artti.",
      },
    },
  },
  npcq_rival_group_sides: {
    text: "Arkadaslarin ikiye bolundu. Bazilari senin tarafinda, bazilari Arkadasin tarafinda. Okul koridorunda gerilim hissediliyor.",
    choices: {
      choice_0: {
        text: "🤝 \"Bu sacmalik, taraf falan yok\"",
        feedback: "Olgunlugun herkesi sasirtti. Gerilim biraz azaldi.",
      },
      choice_1: {
        text: "💪 Kendi grubunu guclendirmek icin ugras",
        feedback: "Saflar netlesti. Bu savas buyuyor.",
      },
    },
  },
  npcq_rival_showdown: {
    text: "Gun geldi. Arkadasin ile son karsilasma: spor turnuvasi, sinav sonucu veya sahne performansi — kim daha iyi?",
    choices: {
      choice_0: {
        text: "🏆 Her seyini ver — kazanmak icin",
        feedback: "Elinden gelenin en iyisini yaptin. Sonuc ne olursa olsun, kendini ashtin.",
      },
      choice_1: {
        text: "🤝 Fair play — iyi bir yarisma olsun",
        feedback: "Sportmen tavrin herkesin takdirini kazandi. Rakibin bile saygi duydu.",
      },
    },
  },
  npcq_rival_aftermath: {
    text: "Yarisma bitti. Arkadasin ile koridorda karsilastiniz. Ikiniznin de yuzunde yorgunluk var. Bir an sessizlik.",
    choices: {
      choice_0: {
        text: "🤝 Elini uzat: \"Iyi yarismaydaki\"",
        feedback: "Elini sikti. Belki dost olamazsiniz ama artik sayginiz var.",
      },
      choice_1: {
        text: "😤 Yandangecer gibi yap — hicbir sey soyleme",
        feedback: "Sessizlik devam ediyor. Rekabet henuz bitmedi.",
      },
    },
  },
  npcq_rival_peace_offer: {
    text: "Arkadasin sana yaklasti. \"Belki... surekli kavga etmek yerine birlikte calisabiliriz?\" Samimi gorunuyor.",
    choices: {
      choice_0: {
        text: "🕊️ \"Kabul. Deneyelim.\"",
        feedback: "Baris saglamak kazanmaktan zor ama daha degerli.",
      },
      choice_1: {
        text: "🤔 \"Guvenmem zor ama firsati veririm\"",
        feedback: "Temkinli ama acik kapisin. Zaman gosterecek.",
      },
    },
  },
  npcq_rival_escalation: {
    text: "Arkadasin arkadaslarindan birine seni kotulemis. Herkes duydu. Bu artik yarisma degil — kisisel.",
    choices: {
      choice_0: {
        text: "😡 Karsilik ver — daha sert sekilde",
        feedback: "Kavga buyudu. Simdi ikiniznin de itibarı zedelendi.",
      },
      choice_1: {
        text: "🧘 Sessiz kal — zamanla herkes gercegi gorur",
        feedback: "Sessizligin guclu bir cevap oldu. Insanlar senin tarafina gecmeye basladi.",
      },
    },
  },
  npcq_rival_respect: {
    text: "Yillar sonra Arkadasin ile karsilastiniz. \"Seni hep saygiyla hatirlayacagim\" diyor. Gercekten oyle gorunuyor.",
    choices: {
      choice_0: {
        text: "🤝 \"Ben de. Beni daha guclu yaptin.\"",
        feedback: "Bazen en iyi dersler rakiplerden gelir.",
      },
    },
  },
  npcq_rival_enemy: {
    text: "Arkadasin ile araniz tamamen bozuldu. Ayni ortamda bile nefes almak zor. Bu nefret seni icten icte yiyor.",
    choices: {
      choice_0: {
        text: "🧘 Burakit — nefret seni tuketiyor",
        feedback: "Icindeki ofkeyi biraktin. Rahatladiniz.",
      },
      choice_1: {
        text: "💀 \"Bu bitmedi...\"",
        feedback: "Nefret seni ele geciriyor. Bu yol karanlik.",
      },
    },
  },
  npcq_betray_first_doubt: {
    text: "Arkadasin'in soyledikleriyle yaptiklari tutmuyor. Bir gun plana gel dedi, gelmedi. Sonra baska biriyile goruldugu soyleniyor.",
    choices: {
      choice_0: {
        text: "🤔 \"Herkesin kotu gunu olabilir, gormezden gel\"",
        feedback: "Hos gorus. Ama icinde kucuk bir suphe kaldi.",
      },
      choice_1: {
        text: "🕵️ Dikkatli ol ve izle",
        feedback: "Gozlerini actin. Bir sonraki sefer daha dikkatli olacaksin.",
      },
    },
  },
  npcq_betray_gossip_heard: {
    text: "Biri sana yaklasti: \"Arkadasin herkese senin sirrini anlatti. Hem de guluyordu.\" Midena bir sey oturdu.",
    choices: {
      choice_0: {
        text: "😡 \"Ne?! Simdi gidip hesap soracagim!\"",
        feedback: "Sinirlisin ve haklisin. Ama once sakince dusunmek belki daha iyi olurdu.",
      },
      choice_1: {
        text: "🤔 \"Emin misin? Once dogrulayayim\"",
        feedback: "Sogukkanli kaldin. Kanitlari toplamak icin zaman kazandin.",
      },
      choice_2: {
        text: "😢 Ici cok aci ama gormezden gel",
        feedback: "Yaralar icine akiyor. Bu dayanilmaz ama sesini cikaramiyorsun.",
      },
    },
  },
  npcq_betray_investigation: {
    text: "Arkadasin'in mesajlarini, davranislarini, ortak arkadaslarin soylediklerini birlestiriyorsun. Resim netlesiyor. Ya gercekten ihanet ettiyse?",
    choices: {
      choice_0: {
        text: "📱 Ortak arkadaslarindan bilgi topla",
        feedback: "Parca parca gercek ortaya cikiyor. Simdi ne yapacaksin?",
      },
      choice_1: {
        text: "💔 Yeter, dogrudan konusacagim",
        feedback: "Cesur karar. Dogrudan gidiyorsun.",
      },
    },
  },
  npcq_betray_confrontation: {
    text: "Arkadasin ile yuz yuzesin. \"Sirrimi neden anlattín?\" Sessizlik. Sonra gozlerini kaciriyor.",
    choices: {
      choice_0: {
        text: "😤 \"Acikla. Simdi.\"",
        feedback: "Sesi titriyordu. Belki pismandir, belki korkmustur. Ama gercek ortada.",
      },
      choice_1: {
        text: "😢 \"Sana guveniyordum...\"",
        feedback: "Kelimeler yumruktan daha sert vurdu. Gozleri doldu.",
      },
    },
  },
  npcq_betray_reaction_manipulative: {
    text: "Arkadasin inkar ediyor: \"Ben oyle bir sey soylemedim, sana yalan soyluyorlar. Belki de gercek arkadaslarin onlar degil!\" Masumiyetini savunuyor.",
    choices: {
      choice_0: {
        text: "🧠 \"Kanitlarim var. Oyunu birak.\"",
        feedback: "Maskesi dustu. Sonunda gercek yuzu gordurun.",
      },
      choice_1: {
        text: "😰 Belki haklidir... kafam karisti",
        feedback: "Manipulasyona yenildín. Kendi gerceginden suphe ediyorsun.",
      },
    },
  },
  npcq_betray_reaction_regret: {
    text: "Arkadasin'in gozleri dolu: \"Yanliş yaptım. Neden yaptigimi bile bilmiyorum. Lutfen...\" Samimi gorunuyor.",
    choices: {
      choice_0: {
        text: "🤝 \"Zaman lazim. Ama dinliyorum.\"",
        feedback: "Af hemen gelmez. Ama kapi tamamen kapanmadi.",
      },
      choice_1: {
        text: "😤 \"Sozler yetmez. Ispat et.\"",
        feedback: "Hakli bir talep. Ama afin bedeli agir.",
      },
    },
  },
  npcq_betray_forgive: {
    text: "Haftalar gecti. Arkadasin surekli uzaktan bakiyor, mesaj atiyor, kucuk jestler yapiyor. Belki gercekten degismistir.",
    choices: {
      choice_0: {
        text: "💚 \"Herkes hata yapar. Ikinci sans.\"",
        feedback: "Affetmek gucluler icindir. Yeni bir sayfa acildi.",
      },
    },
  },
  npcq_betray_distance: {
    text: "Arkadasin ile artik arkadas degilsiniz. Ama duysman da degil. Koridorda kibar bir selam, o kadar.",
    choices: {
      choice_0: {
        text: "🤝 \"Selam. Nasilsin?\"",
        feedback: "Mesafeli ama medeni. Olgunlasmak bazen boyle olur.",
      },
    },
  },
  npcq_betray_cut_off: {
    text: "Arkadasin ile butun ipleri kopardín. Numarasini sildin, ortak gruplari biraktin. Temiz bir kesim.",
    choices: {
      choice_0: {
        text: "🚪 \"Ihanet affedilmez. Bitti.\"",
        feedback: "Sinirlarini korudun. Aciti ama daha guvenli hissediyorsun.",
      },
    },
  },
  cliff_npc_secret_promise: {
    text: "Arkadaslarindan biri seni kenara cekti. \"Yarin konusmamiz lazim. Onemli.\"",
    choices: {
      choice_0: {
        text: "🤔 \"Tamam, merak ettim!\"",
        feedback: "Merak icinde kaldin. Yarin ne soyleyecek acaba?",
      },
      choice_1: {
        text: "😤 \"Simdi soyle, bekleyemem\"",
        feedback: "Sabirsizligin fark edildi. \"Yarin\" dedi ve gitti.",
      },
    },
  },
  cliff_npc_secret_reveal: {
    text: "Arkadasin sonunda konusuyor: \"Ailem tasinmaya karar verdi. Bu sehirden gidiyoruz...\"",
    choices: {
      choice_0: {
        text: "😢 \"Hayir... gitme\"",
        feedback: "Bazi vedalar hayatin en zor kisimlaridir.",
      },
      choice_1: {
        text: "💪 \"Uzak da olsak arkadas kaliriz\"",
        feedback: "Mesafe dostlugun onundeki tek engel degil.",
      },
    },
  },
  cliff_exam_tomorrow: {
    text: "Yarin buyuk sinav var. Gece gec saate kadar ders mi calismalsin, yoksa dinlenip zihni tazele?",
    choices: {
      choice_0: {
        text: "📚 Son bir kez daha gozden gecir",
        feedback: "Geceyi ders calisarak gecirdin. Umarim sonuc iyi olur...",
      },
      choice_1: {
        text: "😴 Erken yat — dinlenmis zihin daha iyi calisir",
        feedback: "Rahat bir uyku cektin. Yarin taze bir kafayla gireceksin.",
      },
    },
  },
  cliff_exam_result_good: {
    text: "Sinav sonuclari aciklandi! Gece boyunca calistigi konulardan soru gelmis.",
    choices: {
      choice_0: {
        text: "🎉 Sonuclara bak",
        feedback: "Harika bir not! Gece calismak ise yaramis.",
      },
    },
  },
  cliff_exam_result_rested: {
    text: "Sinav sonuclari aciklandi. Dinlenmis bir kafayla girdiginde kendini iyi hissetmistin.",
    choices: {
      choice_0: {
        text: "📊 Sonuclara bak",
        feedback: "Fena degil! Dinlenmek de ise yaramis. Hem sagligindan da olmedin.",
      },
    },
  },
  cliff_family_announcement: {
    text: "Ailen aksam yemeginde \"Size bir haberimiz var\" dedi. Yuzlerinde garip bir ifade.",
    choices: {
      choice_0: {
        text: "😰 \"Ne oldu? Soyleyin...\"",
        feedback: "\"Yarin konusuruz\" dediler. Gece uyuyamadin.",
      },
      choice_1: {
        text: "😊 \"Iyi haber mi?\"",
        feedback: "Gulumsediler ama cevap vermediler. Merak icinde kaldin.",
      },
    },
  },
  cliff_family_news_reveal: {
    text: "Ailen sonunda haberi verdi: yeni bir kardes yolda! Hayatin degisecek.",
    choices: {
      choice_0: {
        text: "🎉 \"Harika! Abi/abla olacagim!\"",
        feedback: "Heyecanlisin! Buyuk sorumluluk ama buyuk mutluluk da.",
      },
      choice_1: {
        text: "😰 \"Ya dikkat bana azalirsa?\"",
        feedback: "Endiselen. Ama zamanla alisacaksiniz.",
      },
    },
  },
  cliff_fate_turning_point: {
    text: "Bugun garip bir gun. Her sey normal gibi ama icinde bir his var — \"yarin bir seyler degisecek.\" Buna hazir misin?",
    choices: {
      choice_0: {
        text: "🌟 \"Degisime hazirim\"",
        feedback: "Icindeki ses guclendi. Yarin ne getirecek bilmiyorsun ama korkun yok.",
      },
      choice_1: {
        text: "😟 \"Umarim kotu bir sey degildir\"",
        feedback: "Tedirginsin. Ama bazen en iyi seyler beklenmedik anda gelir.",
      },
    },
  },
  cliff_mysterious_letter: {
    text: "Cantanda bir mektup buldun. Uzerinde sadece \"Yarin parkta ol, saat 4\" yazıyor. El yazisi tanidik ama cikaramadin.",
    choices: {
      choice_0: {
        text: "🕵️ \"Gidecegim — kim oldugunu merak ediyorum\"",
        feedback: "Cesur bir karar. Yarin parkta ne bekliyor seni?",
      },
      choice_1: {
        text: "🗑️ \"Tuzak olabilir, gitmem\"",
        feedback: "Temkinli davrandin. Belki hakli, belki de bir firsati kacirdin.",
      },
    },
  },
  cliff_letter_meetup: {
    text: "Parka gittin. Bankta oturan kisiyi gordun — eski bir arkadasin! Yillardir gormemistin. \"Seni bulamaya calistim\" diyor.",
    choices: {
      choice_0: {
        text: "🤗 \"Inanamiyorum! Gel sarilayim!\"",
        feedback: "Bazen hayat en guzel surprizleri en beklenmedik anda verir.",
      },
      choice_1: {
        text: "🤔 \"Neden simdi?\"",
        feedback: "Mesafeli ama meraklisin. Hikayesini dinlemeye basladiniz.",
      },
    },
  },
  evt_burden_breakdown_forced: {
    text: "Risk seviyen kritik eşiği aştı. Zihinsel yorgunluk birikti ve ciddi bir mental çöküş yaşadın.",
    choices: {
      burden_breakdown_rest: {
        text: "Mecburi mola ver ve toparlan",
        feedback: "Zor bir çöküş yaşadın. Bir adım geri çekilip yeniden denge kurman gerekiyor.",
      },
    },
  },
  evt_crisis_exam_panic: {
    text: "Sınav haftası geldi ve hiçbir şeye hazır değilsin. Gece boyunca ders çalışmaya çalıştın ama sayfalar bulanıklaştı. Ellerinin titrediğini fark ettin.",
    choices: {
      crisis_exam_accept: {
        text: "Sınavı olduğu gibi kabul et, ne olursa olsun",
        feedback: "Sınava girdin ama kafan bomboştu. Kötü geçti. Ama en azından kendini parçalamayı bıraktın.",
      },
      crisis_exam_allnighter: {
        text: "Son bir gece daha zorla — ya hep ya hiç",
        feedback: "Sabaha kadar çalıştın. Sınavda idare ettin ama vücudun sana ağır bir fatura kesti.",
      },
    },
  },
  evt_crisis_family_pressure: {
    text: "Anne-baban arasındaki tartışma bu gece doruk noktasına ulaştı. Kapı çarpma sesleri, bağrışmalar... Odanda yorganın altına saklandın.",
    choices: {
      crisis_family_intervene: {
        text: "Araya gir ve onları sakinleştirmeye çalış",
        feedback: "Küçük omuzların büyük bir yük taşıdı. Biraz sakinleştiler ama bu senin taşıman gereken bir yük değildi.",
      },
      crisis_family_isolate: {
        text: "Kulaklığını tak ve dünyayı kapat",
        feedback: "Müziğin sesi bağrışmaları bastırdı. Uyuyakaldın. Sabah herkes normal davrandı — sanki hiçbir şey olmamış gibi.",
      },
    },
  },
  evt_crisis_social_exclusion: {
    text: "Grup sohbetinde herkesin seni konuştuğunu fark ettin. \"O gelmesin\" yazmışlar. Ekran karardı, içindeki bir şey de.",
    choices: {
      crisis_social_confront: {
        text: "Yüzlerine söyle — ne düşündüğünü bilsinler",
        feedback: "Cesaretini topladın ve konuştun. Bazıları özür diledi, bazıları arkasını döndü. Ama en azından sessiz kalmadın.",
      },
      crisis_social_withdraw: {
        text: "Sessizce uzaklaş — yalnız kalmayı tercih et",
        feedback: "O gece çok kitap okudun. Ama sayfalardaki kelimeler bir türlü anlam ifade etmedi. Yalnızlık sessiz bir ağrıdır.",
      },
    },
  },
  evt_crisis_identity: {
    text: "Aynaya baktın ve tanıyamadın. \"Ben kimim?\" sorusu kafanda yankılanıyor. Herkesin beklediği kişi mi, yoksa gerçekten olmak istediğin kişi mi?",
    choices: {
      crisis_identity_rebel: {
        text: "Herkesin beklentisini bir kenara koy — kendi yolunu çiz",
        feedback: "Saçını farklı kestirdin, tarzını değiştirdin. Herkes şaşırdı. Ama aynada sonunda tanıdığın birini gördün.",
      },
      crisis_identity_conform: {
        text: "Şimdilik akışa kapıl — herkes böyle yapıyor",
        feedback: "Normal davranmaya devam ettin. Maskeyi çıkarmadın. Belki bir gün... ama bugün değil.",
      },
    },
  },
  evt_crisis_insomnia: {
    text: "Üçüncü gece üst üste uyuyamadın. Gözlerin yanıyor, kafan zonkluyor. Saat 04:00 — tavan boş boş sana bakıyor.",
    choices: {
      crisis_insomnia_rest: {
        text: "Ertesi gün her şeyi iptal et ve uyu",
        feedback: "14 saat uyudun. Dünya döndü, sen kaçırdın. Ama vücudun sonunda nefes aldı.",
      },
      crisis_insomnia_push: {
        text: "Kahve iç ve güne devam et",
        feedback: "Gün boyunca zombi gibi dolaştın. Her şey bulanık. Birisi sana bir şey söyledi ama ne olduğunu hatırlamıyorsun.",
      },
    },
  },
  evt_crisis_failure_spiral: {
    text: "Her şey ters gidiyor. Sınavlar kötü, arkadaşlıklar soğuk, aile gergin. \"Ben hiçbir şeyi beceremiyorum\" düşüncesi kafana yerleşti.",
    choices: {
      crisis_failure_talk: {
        text: "Birine açıl — belki bir öğretmen, belki bir arkadaş",
        feedback: "\"Herkes bazen böyle hisseder\" dedi. Klişe geldi ama... gözlerindeki samimiyet gerçekti. Biraz rahatladın.",
      },
      crisis_failure_bottle: {
        text: "Kimseye bir şey belli etme — kendin halledeceksin",
        feedback: "Dışarıdan güçlü görünüyorsun. İçerideyse bir fırtına var. Ne kadar daha taşıyabilirsin?",
      },
    },
  },
  evt_crisis_perfectionism: {
    text: "Ödevini beşinci kez sildin ve baştan yazdın. Hâlâ yeterince iyi değil. Saat gece 2, gözlerinden yaşlar akıyor — neden bu kadar zor?",
    choices: {
      crisis_perfect_submit: {
        text: "\"Yeterince iyi\" de ve gönder",
        feedback: "Mükemmel değildi. Ama teslim ettin. Dünya yıkılmadı. Belki mükemmel olmak zorunda değilsin.",
      },
      crisis_perfect_redo: {
        text: "Bir kez daha dene — bu sefer olacak",
        feedback: "Altıncı versiyon... yedinci... Sonunda bitirdin. Mükemmel mi? Bilmiyorsun. Ama artık umursamıyorsun bile.",
      },
    },
  },
  evt_crisis_physical_collapse: {
    text: "Okul merdivenlerinde aniden başın döndü ve tutunmak zorunda kaldın. Yemek yemeyi, su içmeyi ne zaman bıraktığını hatırlamıyorsun.",
    choices: {
      crisis_physical_hospital: {
        text: "Revire git ve yardım iste",
        feedback: "Revirde serum taktılar. \"Düzgün beslenmen lazım\" dedi hemşire. Basit bir cümle ama içine işledi.",
      },
      crisis_physical_ignore: {
        text: "Geçer, sadece biraz yorgunum",
        feedback: "Güne devam ettin ama herkes solgun yüzünü fark etti. \"İyi misin?\" sorularına \"iyiyim\" dedin. Değildin.",
      },
    },
  },
  evt_crisis_childhood_overwhelm: {
    text: "Bugün okuldaki her şey çok fazla oldu. Öğretmen bağırdı, arkadaşın kırdı, ödev zor geldi. Eve gelince kapıyı kapatıp ağladın.",
    choices: {
      crisis_child_parent: {
        text: "Anne/babana koş ve sarıl",
        feedback: "Sıcak bir kucak, bir bardak süt ve \"her şey düzelecek\" sözü. Küçük kalbin biraz rahatladı.",
      },
      crisis_child_hide: {
        text: "Yatağının altına saklan ve yalnız kal",
        feedback: "Yatağın altında karanlıkta bekledin. Sonunda uyuyakaldın. Sabah biraz daha iyi hissettin ama kimseye anlatmadın.",
      },
    },
  },
  evt_momentum_helpful_street_thanks: {
    text: "Sokakta daha once destek oldugun biri seni gorup durduruyor: \"Iyi ki yardim etmistin, simdi ben ayaktayim.\"",
    choices: {
      helpful_street_thanks_accept: {
        text: "Gulumseyip tesekkuru kabul et",
        feedback: "Insanlarin guveni etrafinda sessiz bir guce donustu.",
      },
      helpful_street_thanks_redirect: {
        text: "Destege ihtiyaci olan baska birini yonlendir",
        feedback: "Iyilik zinciri seninle buyumeye devam etti.",
      },
    },
  },
  evt_momentum_helpful_unexpected_support: {
    text: "Gecmiste yardim ettigin bir tanidik, dar bir anda sana beklenmedik bir destek sunuyor.",
    choices: {
      helpful_support_accept: {
        text: "Destegi kabul edip tesekkur et",
        feedback: "Dunya bazen verdigin seyin fazlasiyla geri dondugu bir yer.",
      },
      helpful_support_share: {
        text: "Destegin bir kismini baska birine aktar",
        feedback: "Paylastikca agin buyudu ve ismin guvenle anildi.",
      },
    },
  },
  evt_momentum_pragmatic_short_offer: {
    text: "Planli ve tutarli tavrin dikkat cekmis. Mahalleden biri sana kisa sureli ama iyi odemeli bir is teklif ediyor.",
    choices: {
      pragmatic_offer_accept: {
        text: "Teklifi kabul et ve net bir plan cikar",
        feedback: "Hizli hesap yaptin, riski yonettin ve kazancin artti.",
      },
      pragmatic_offer_negotiate: {
        text: "Sartlari pazarlikla iyilestir",
        feedback: "Detaylari okuyup avantaji lehine cevirdin.",
      },
    },
  },
  evt_momentum_pragmatic_information_edge: {
    text: "Onceden yaptigin sistemli notlar sayesinde herkesten once kritik bir firsati fark ettin.",
    choices: {
      pragmatic_info_calm: {
        text: "Sogukkanli davran ve kontrollu ilerle",
        feedback: "Dogru bilgi, dogru zamanda seni bir adim one tasidi.",
      },
      pragmatic_info_press: {
        text: "Agresif bir zamanlama ile hamleyi hizlandir",
        feedback: "Dakik karar kar getirdi ama tempoyu sertlestirdi.",
      },
    },
  },
  evt_momentum_aggressive_reputation: {
    text: "Mahallede sert ve net durusun dilden dile yayilmis. Bazi insanlar geri adim atiyor, bazi kapilar ise aniden aciliyor.",
    choices: {
      aggressive_reputation_control: {
        text: "Gucu kontrol edip sinirlarini net ciz",
        feedback: "Korku degil saygi uyandiran bir cizgi yakaladin.",
      },
      aggressive_reputation_push: {
        text: "Baskiyi artirip daha sert bir tavir al",
        feedback: "Etkin artti ama iliskilerde catlaklar buyudu.",
      },
    },
  },
};
