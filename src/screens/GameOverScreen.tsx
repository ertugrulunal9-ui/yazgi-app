...
export const GameOverScreen: React.FC<GameOverScreenProps> = ({ theme, metrics, onRestart }) => {
  ...
  useEffect(() => {
    incrementCommunityRuns().then(setCommunityRunCount).catch(() => {});
  }, []);
  ...
  const handleShareLife = async () => {
    ...
    try {
      ...
      if (!sharingAvailable) {
        Alert.alert(t('messages.sharingUnavailable'), t('messages.sharingNotSupported'));
        return;
      }
      const uri = await shareCardRef.current?.capture?.();
      if (!uri) {
        Alert.alert(t('messages.sharingNotReady'), t('messages.cardCreationFailed'));
        return;
      }
      ...
      Alert.alert(t('messages.sharingFailed'), t('messages.cardSharingFailed'));
    } finally {
      setIsSharing(false);
    }
  };
  ...
  const TAB_LABELS: Record<GameOverTab, string> = {
    ozet: t('gameOver.tabs.ozet', undefined, 'Özet'),
    detay: t('gameOver.tabs.detay', undefined, 'Detay'),
    hikaye: t('gameOver.tabs.hikaye', undefined, 'Hikaye'),
  };

  const TABS: GameOverTab[] = ['ozet', 'detay', 'hikaye'];
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.appBg }}>
      <SafeAreaView style={{ flex: 1 }}>
            ...
            return (
              <TouchableOpacity
                ...
                style={{
                  ...
                  alignItems: 'center',
                  borderBottomWidth: 2,
                  borderBottomColor: isActive ? tierColor : 'transparent',
                }}
              >
                <Text style={{
                  color: isActive ? tierColor : theme.textSecondary,
        ...
        <ScrollView
          contentContainerStyle={{ flexGrow: 1, padding: metrics.pad }}
                ...
                <FadeInUpView delay={100}>
                  ...
                  >
                    <Text style={{
                      ...
                      {isSharing ? t('ui.cardPreparing') : t('buttons.shareLife')}
                    </Text>
                  </ShimmerButton>
                </FadeInUpView>
                  ...
                  <FadeInUpView delay={240}>
                    <View style={{
                      ...
                      {activeScars.map((scar, i) => (
                        <View key={scar.id} style={{ marginBottom: i < activeScars.length - 1 ? 10 : 0 }}>
                      ...
                      {!altEndingUnlocked && (
                        ...
                        >
                          <Text style={{ color: '#22c55e', fontWeight: '700', fontSize: 13 }}>
                            {altEndingUnlocking ? t('ui.adLoading') : t('buttons.watchAdAlternativeEnding')}
                          </Text>
                        </ShimmerButton>
                      )}
        ...
        <View style={{
          ...
          {communityRunCount > 0 && (
            <Text style={{
              ...
              {t('gameOver.communityCount', { count: communityRunCount }, 'Bu hayatı yaşayan {count}. kişisin')}
            </Text>
          )}
