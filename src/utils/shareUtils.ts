...
export const createTraitShareText = (traitName: string): ShareMilestone => ({
  type: 'TRAIT_ACQUIRED',
  text: pickTemplate(TRAIT_SHARE_TEMPLATES).replace('{traitName}', traitName),
  emoji: '\u26A1',
});

export const createAchievementShareText = (): ShareMilestone => ({
  type: 'SECRET_ACHIEVEMENT',
  text: pickTemplate(ACHIEVEMENT_SHARE_TEMPLATES),
  emoji: '\u{1F3C6}',
});

export const createRomanceShareText = (): ShareMilestone => ({
  type: 'FIRST_ROMANCE',
  text: pickTemplate(ROMANCE_SHARE_TEMPLATES),
  emoji: '\u{1F497}',
});

export const createFateShareText = (isPositive: boolean): ShareMilestone => {
  const outcome = isPositive ? 'gulumsedi' : 'sirtini dondu';
  return {
    type: 'FATE_MOMENT',
    text: pickTemplate(FATE_SHARE_TEMPLATES).replace('{outcome}', outcome),
