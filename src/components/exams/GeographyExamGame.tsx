...
import { GeographyQuestion, QuestionType as GeographyQuestionType, getQuestionPool } from '../../i18n/exams/geographyQuestions';

type QuestionType = GeographyQuestionType;
...
const getQuestionTypeTitle = (type: QuestionType): string => (
    tRuntime(`exams.geography.questionTypes.${QUESTION_TYPE_KEY_MAP[type]}`)
);
...
const GeographyExamGame: React.FC<GeographyExamGameProps> = ({
    ...
    if (!gameState || !setGameState) {
        return (
            <View style={styles.loadingContainer}>
                <Text style={styles.loadingText}>{tRuntime('exams.geography.loading')}</Text>
            </View>
        );
    }
    ...
    if (!currentQuestion) {
        return (
            <View style={styles.loadingContainer}>
                <Text style={styles.loadingText}>{tRuntime('exams.geography.preparing')}</Text>
            </View>
        );
    }
            ...
            {feedback === 'wrong' && (
                <View style={styles.explanationContainer}>
                    <Text style={styles.explanationText}>
                        {tRuntime('exams.geography.correctAnswer', { answer: currentQuestion.options[currentQuestion.correctIndex] })}
                    </Text>
                </View>
            )}
