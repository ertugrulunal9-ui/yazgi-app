...
import { TurkishQuestion, QuestionType as TurkishQuestionType, getQuestionPool } from '../../i18n/exams/turkishQuestions';

type QuestionType = TurkishQuestionType;
...
const getQuestionTypeTitle = (type: QuestionType): string => (
    tRuntime(`exams.turkish.questionTypes.${QUESTION_TYPE_KEY_MAP[type]}`)
);
...
const TurkishExamGame: React.FC<TurkishExamGameProps> = ({
    ...
    if (!gameState || !setGameState) {
        return (
            <View style={styles.loadingContainer}>
                <Text style={styles.loadingText}>{tRuntime('exams.turkish.loading')}</Text>
            </View>
        );
    }
    ...
    if (!currentQuestion) {
        return (
            <View style={styles.loadingContainer}>
                <Text style={styles.loadingText}>{tRuntime('exams.turkish.preparing')}</Text>
            </View>
        );
    }
            ...
            {feedback === 'wrong' && (
                <View style={styles.explanationContainer}>
                    <Text style={styles.explanationText}>
                        {tRuntime('exams.turkish.correctAnswer', { answer: currentQuestion.options[currentQuestion.correctIndex] })}
                    </Text>
                </View>
            )}
