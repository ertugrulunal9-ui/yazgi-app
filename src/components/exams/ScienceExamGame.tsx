...
import { ScienceQuestion, QuestionType as ScienceQuestionType, getQuestionPool } from '../../i18n/exams/scienceQuestions';

type QuestionType = ScienceQuestionType;
...
const getQuestionTypeTitle = (type: QuestionType): string => (
    tRuntime(`exams.science.questionTypes.${QUESTION_TYPE_KEY_MAP[type]}`)
);
...
const ScienceExamGame: React.FC<ScienceExamGameProps> = ({
    ...
    if (!gameState || !setGameState) {
        return (
            <View style={styles.loadingContainer}>
                <Text style={styles.loadingText}>{tRuntime('exams.science.loading')}</Text>
            </View>
        );
    }
    ...
    if (!currentQuestion) {
        return (
            <View style={styles.loadingContainer}>
                <Text style={styles.loadingText}>{tRuntime('exams.science.preparing')}</Text>
            </View>
        );
    }
            ...
            {feedback === 'wrong' && (
                <View style={styles.explanationContainer}>
                    <Text style={styles.explanationText}>
                        {tRuntime('exams.science.correctAnswer', { answer: currentQuestion.options[currentQuestion.correctIndex] })}
                    </Text>
                </View>
            )}
